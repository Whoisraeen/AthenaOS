...
fn execute_line(line: &[u8], cwd: &mut alloc::string::String, home: &str) {
    let cmd_str = match core::str::from_utf8(line) {
        Ok(s) => s.trim(),
        Err(_) => {
            write_err(b"rae-sh: invalid utf-8\n");
            return;
        }
    };
    if cmd_str.is_empty() {
    ...
    match verb {
        ...
        "cd" => {
            ...
            let ok = raekit::sys::readdir_at(new_path.as_str(), &mut dir_buf) != u64::MAX;
            if ok {
                *cwd = new_path;
            } else {
        ...
        "ls" => {
            ...
            let count = raekit::sys::readdir_at(path, &mut dir_buf);
            if count == u64::MAX {
...
/// >64 KiB sources run in raelangd — poll with yields), then print the
...
fn run_rae_script(src: &[u8]) {
    ...
    let id = raekit::sys::script_run(src, SCRIPT_CAP_ALL);
    ...
    for _ in 0..500 {
        let n = raekit::sys::script_status(id, &mut buf);
        if n == u64::MAX || n < 56 {
    ...
    write_err(b"rae: still running in raelangd (check later)\n");
}
...
fn read_file(path: &str) -> Option<alloc::vec::Vec<u8>> {
    ...
    let mut buf = [0u8; 512];
    loop {
        let n = raekit::sys::read(fd, &mut buf);
        if n == 0 || n == u64::MAX {
            break;
        }
...
pub extern "C" fn _start() -> ! {
    ...
    let home_owned = {
        ...
        if let Some(_) = raekit::sys::session_info(&mut info) {
            if let Some(h) = raekit::sys::session_home_from(&info) {
    ...
    loop {
        let mut ch = [0u8; 1];
        let n = raekit::sys::pty_slave_read(&mut ch);
        if n == 0 {
            raekit::sys::yield_now();
            continue;
        }
