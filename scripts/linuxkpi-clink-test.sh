#!/usr/bin/env bash
# C-link smoke test for raeen_linuxkpi (Path C).
#
# Proves the shim is genuinely linkable from C: builds libraeen_linuxkpi.a (the
...
echo "[clink] building staticlib (raeen_linuxkpi, --features clib) ..."
cargo rustc -p raeen_linuxkpi --target x86_64-unknown-none --release \
    --features clib --crate-type staticlib

A="$(realpath "$(find target/x86_64-unknown-none/release -name 'libraeen_linuxkpi.a' | head -1)")"
...
cc -nostdlib -nostartfiles -static -Wl,-e,probe_main \
    -o "$OUT/probe.elf" "$OUT/probe.o" -L"$(dirname "$A")" -lraeen_linuxkpi
...
echo "[clink] PASS: C driver TU links clean against libraeen_linuxkpi.a (0 unresolved)."
