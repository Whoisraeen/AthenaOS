//! athlangd — AthenaOS userspace Rae-script interpreter daemon.
//!
...
//! (`components/athlang`), two execution sites.
//!
...
/// The daemon's [`athlang::Host`], gated on the SAME `SCRIPT_CAP_*` bits
...
/// ath_abi constant, same as the clock app's direct use of 40).
...
impl athlang::Host for DaemonHost {
    fn call(
        &mut self,
        name: &str,
        args: &[athlang::Value],
...
fn run_job(id: u64, cap_mask: u64, src: &[u8]) {
    ...
    let result = core::str::from_utf8(src)
        .map_err(|_| athlang::RaeError::Lex(alloc::string::String::from("source is not UTF-8")))
        .and_then(|s| athlang::run_with_host(s, DAEMON_FUEL, &mut host));
...
pub extern "C" fn _start() -> ! {
    ...
    klog("[athlangd] up: draining queued Rae scripts (fetch/run/complete)");
    ...
    loop {
        ...
        if 32 + src_len > fetched as usize {
            klog("[athlangd] short job record — dropping");
