# AthenaOS — Final Completion Checklist

**Generated:** 2026-06-26  
**Authority:** `LEGACY_GAMING_CONCEPT.md` > `MasterChecklist.md` > this file  
...
| Storage | NVMe real SSD (incl. ESP BOOTLOG.TXT writes), AHCI, full AthFS suite |
...
- [~] AthFS mkfs (on-disk format) — QEMU-proven
...
- [~] FDE (AthFS encryption) — QEMU-proven
- [ ] **Live USB boots, installer formats Athena's NVMe, copies kernel, reboots without USB into working AthenaOS** (the full install-to-installed-disk path — the iron gate for Phase 3)
...
- [ ] **TPM 2.0 sealing of root keys** (Concept §AthFS encryption)
...
- [ ] AthFS on Athena NVMe root partition (currently ESPwrite-only on iron; AthFS volume on NVMe unproven)
- [ ] Network shares (SMB — not planned for 1.0)
- [ ] File associations / default apps

---

## §6 — Phase 6: AthGFX / Vulkan Demo (Year-1 open deliverable)

**This is the largest open Year-1 gap.** The Concept promises "Kernel + AthFS + AthGFX + AthUI hello world. Boots, draws, plays a single Vulkan demo." The demo today is software raster — `vkQueueSubmit`-equivalent on real silicon is not built.
...
- [ ] HDR (Concept §AthGFX first-class; not built)
- [ ] VRR (FreeSync/G-Sync) end-to-end on a real panel
- [ ] DPI / fractional scaling
- [ ] Color management (ICC profiles)
- [ ] Shader cache shared across Vulkan + AthGFX (struct exists; no GPU pipeline to populate)
...
- [~] SCHED_BODY audio thread (128-frame/2.67ms period) — structural
- [ ] **Sub-3ms round-trip verified on Athena** (Concept §AthAudio hard target — not yet measured)
...
- [~] AthUI (Label + Button + Frame + theme palette over software Canvas) — functional on QEMU
- [~] AthKit declarative SDK (`view!` macro, reactive state, app lifecycle, NavigationStack/TabView) — compiled
- [ ] **Skia integration wired** (Concept §Language Stack: "AthUI sits on Skia for 2D") — not wired
...
## §11 — AthBridge (Windows Compatibility) — HUMAN-GATED

> **Do not start net-new AthBridge guest-execution work without explicit owner assignment.**  
...
- [ ] **`notepad.exe` runs, types text, saves a file** (the first AthBridge iron milestone)
...
- [ ] Steam works (Concept: "Steam day one or there is no gaming OS") — blocked on AthBridge + GPU
...
- [ ] Swappable shell (replace AthShell with a competing one via first-class API)
...
- [~] AthShell default desktop (taskbar, start menu, tray w/ live RTC clock, notifications) — QEMU-proven
...
- [ ] **Non-developer can flash USB, boot Athena, install AthenaOS, log in, browse files — without reading a wiki** (the consumer install gate)
...
- [ ] AthenaOS Pro / Studio / Enterprise pricing tiers (business model)
...
- [ ] AthKit tools free for first-year developers
- [ ] Free signing for first-year developers
- [ ] World-class developer docs
- [ ] Swift app-development onramp (toolchain + AthKit bindings so a macOS app ports in a week)
...
- [~] Screen reader announce core (`announce_node` / `describe_focused`) — `[~]`; no real TTS→AthAudio sink
...
- [ ] G2 Switcher gate: G1 + run existing Windows apps via AthBridge + hardware telemetry + theme + driver management
...
|---|---|---|
| **Kernel core** | Hybrid, SCHED_BODY, capability-gated | `[x]` iron-proven |
| **SMP / ACPI / APIC** | 12 CPUs, full ACPI AML | `[x]` iron-proven |
| **AthFS** | CoW, snapshots, FDE, buckets | `[~]` QEMU-proven; iron NVMe root unproven |
| **AthGFX** | "Looks like Metal, performs like Vulkan" | `[ ]` SW raster only; real GPU submit not built |
| **AthAudio** | Sub-3ms round-trip | `[~]` HDA playback `[x]`; sub-3ms unverified |
| **AthUI / AthKit** | SwiftUI-style, GPU-accelerated, Skia+wgpu | `[~]` Skia/wgpu not wired; SW canvas works |
| **AthGuard** | Capability sandbox, attestation | `[~]` Core sound; fail-open sandbox (bring-up) |
| **AthNet** | WireGuard, TLS 1.3, QUIC | `[~]` TLS/WG host-KAT'd; iron net blocked on RX |
| **AthBridge** | Steam day one | `[~]` Compiled; DXBC→SPIR-V and real GPU needed |
| **AthPlay** | Steam/Epic/GOG unified | `[~]` Compiled; no live game launching |
