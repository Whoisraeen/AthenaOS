# Implementation Plan: RaeGFX & RaeUI Foundation

## Background & Motivation
With the foundational kernel components (scheduler, capability IPC, VFS, and user-space transitions) successfully implemented, RaeenOS is ready to transition from text-mode/serial output to a graphical desktop. The concept document outlines a gaming-first, native-feeling OS with sub-frame latency. To achieve this, we need a native graphics API (`RaeGFX`) and a declarative UI framework (`RaeUI`). This plan focuses on the immediate next milestones: drawing pixels to the screen via `RaeGFX` and establishing the core event loop for a `RaeUI` window.

## Scope & Impact
This plan covers:
1. **RaeGFX Initialization**: Exporting the kernel framebuffer capability to userspace and building a basic 2D line rasterizer.
2. **RaeUI Core**: Creating a foundational windowing abstraction, an event loop, and a basic widget (a button) that renders using RaeGFX.
...
## Proposed Solution
- **RaeGFX (`components/raegfx`)**: Implement a userspace library that takes a framebuffer memory map (provided via the `SYS_MMIO_MAP` syscall) and provides a 2D rendering canvas. It will include basic primitives: `clear`, `draw_pixel`, `draw_rect`, and `draw_line`.
- **RaeUI (`components/raeui`)**: Implement an event-driven UI framework on top of RaeGFX. It will introduce a `Window` struct, a `Widget` trait, and an `EventLoop` that polls for user input (via IPC channels from the keyboard driver) and triggers `render` passes.

## Alternatives Considered
- **Porting an existing UI library (e.g., Slint, Iced)**: While faster, this violates the RaeenOS core principle of a deeply integrated, proprietary native stack optimized for specific latency and architectural goals. We will build the foundational abstractions ourselves, potentially leveraging Skia/wgpu *under the hood* later as per the architecture doc, but the immediate goal is raw pixel manipulation to prove the IPC/Framebuffer pipeline.

## Implementation Plan

### Phase 1: RaeGFX 2D Rasterizer
...
### Phase 2: RaeUI Window & Event Loop
...
- Verify in QEMU that the framebuffer successfully transitions from the boot banner to the RaeUI window displaying a button.
