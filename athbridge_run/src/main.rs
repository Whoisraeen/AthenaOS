//! `athbridge_run` — the per-process AthBridge launcher (one `.exe`, one process).
//!
...
//! a well-known VFS path, spawns `athbridge_run`, and reaps its exit code; the
...
//!
//! Interim handoff (option (b), `docs/components/athbridge-process-model.md`
//! §2): the target is read from `athbridge::handoff::HANDOFF_PATH` — a no-ABI,
...
//!   [athbridge_run] launched pid=<self> target=<token> -> running
//! then the guest's own `[athbridge] guest ExitProcess(<c>) -> exit <c>` /
//! `[athbridge] smoketest: ... C++ exe ... PASS`, then `sys_exit(<c>)`.
...
// Pull in athkit for its #[global_allocator]; everything else goes through
// athbridge's own native syscall bindings.
#[allow(unused_imports)]
use athkit;

use athbridge::handoff::{self, Target, HANDOFF_MAX_BYTES, HANDOFF_PATH};
use athbridge::launcher::{run_target, target_image};
use athbridge::syscalls::{sys_close, sys_debug_print, sys_exit, sys_getpid, sys_open, sys_read};
...
/// `athbridge::handoff::HANDOFF_PATH`), so it holds OUR target. Returns the raw
...
pub extern "C" fn _start() -> ! {
    ...
    let blob = match read_handoff() {
        Some(b) => b,
        None => die(
            "[athbridge_run] FATAL: handoff target unreadable (no /run/athbridge/self.target)\n",
            RUN_ERR_NO_TARGET,
        ),
    };
    let target: Target = match handoff::decode(&blob) {
        Some(t) => t,
        None => die(
            "[athbridge_run] FATAL: handoff target malformed\n",
            RUN_ERR_NO_TARGET,
        ),
    };
    ...
    let pe_bytes: Option<Vec<u8>> = match &target {
        Target::Pe { path } => match read_pe_file(path) {
            Some(b) => Some(b),
            None => die(
                "[athbridge_run] FATAL: PE target unreadable\n",
                RUN_ERR_NO_TARGET,
            ),
        },
        _ => None,
    ...
    if image.is_empty() {
        die("[athbridge_run] FATAL: empty PE image\n", RUN_ERR_NO_TARGET);
    }

    log(&format!(
        "[athbridge_run] launched pid={pid} target={label} -> running\n"
    ));
