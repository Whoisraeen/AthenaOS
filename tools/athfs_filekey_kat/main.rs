...
//
// It reproduces the kernel's `athfs::EncryptionKey::derive` and
// `athfs::file_encryption_key` BYTE-FOR-BYTE using the shared
// `ath_crypto::sha256::hmac_sha256` (the exact HMAC-SHA256 the kernel HKDF
// calls), so this validates the real derivation, not a private copy. The kernel
// boot smoketest (`[athfs] per-file-key selftest ...`) proves the full
...
use ath_crypto::sha256::hmac_sha256;
...
/// Byte-identical mirror of kernel `athfs::file_encryption_key`: derive a
...
/// Byte-identical mirror of kernel `athfs::bucket_encryption_key` (no master key
