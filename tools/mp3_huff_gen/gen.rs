...
// public table data used by every conforming decoder.
//
...
fn tables() -> Vec<Tab> {
    vec![
    Tab{n:1,x:2,y:2,lin:0,
        hlen:&[1,3,2,3],
        hcod:&[1,1,1,0]},
    Tab{n:2,x:3,y:3,lin:0,
        hlen:&[1,3,6,3,3,5,5,5,6],
        hcod:&[1,2,1,3,1,1,3,2,0]},
    Tab{n:3,x:3,y:3,lin:0,
        hlen:&[2,2,6,3,2,5,5,5,6],
        hcod:&[3,2,1,1,1,1,3,2,0]},
    Tab{n:5,x:4,y:4,lin:0,
        hlen:&[1,3,6,7,3,3,6,7,6,6,7,8,7,6,7,8],
        hcod:&[1,2,6,5,3,1,4,4,7,5,7,1,6,1,1,0]},
    Tab{n:6,x:4,y:4,lin:0,
        hlen:&[3,3,5,7,3,2,4,5,4,4,5,6,6,5,6,7],
        hcod:&[7,3,5,1,6,2,3,2,5,4,4,1,3,3,2,0]},
    Tab{n:7,x:6,y:6,lin:0,
        hlen:&[1,3,6,8,8,9, 3,4,6,7,7,8, 6,5,7,8,8,9, 7,7,8,9,9,9, 7,7,8,9,9,10, 8,8,9,10,10,10],
        hcod:&[1,2,10,19,16,10, 3,3,7,10,5,3, 11,4,13,17,8,4, 12,11,18,15,11,2, 7,6,9,14,3,1, 6,4,5,3,2,0]},
    Tab{n:8,x:6,y:6,lin:0,
        hlen:&[2,3,6,8,8,9, 3,2,4,8,8,8, 6,4,6,8,8,9, 8,8,8,9,9,10, 8,7,8,9,10,10, 9,8,9,9,11,11],
        hcod:&[3,4,6,18,12,5, 5,1,2,16,9,3, 7,3,5,14,7,3, 19,17,15,13,10,4, 13,5,8,11,5,1, 12,4,4,1,1,0]},
    Tab{n:10,x:8,y:8,lin:0,
        hlen:&[1,3,6,8,9,9,9,10, 3,4,6,7,8,9,8,8, 6,6,7,8,9,10,9,9, 7,7,8,9,10,10,9,10, 8,8,9,10,10,10,10,10, 9,9,10,10,11,11,10,11, 8,8,9,10,10,10,11,11, 9,8,9,10,10,11,11,11],
        hcod:&[1,2,10,23,35,30,12,17, 3,3,8,12,18,21,12,7, 11,9,15,21,32,40,19,6, 14,13,22,34,46,23,18,7, 20,19,33,47,27,22,9,3, 31,22,41,26,21,20,5,3, 14,13,10,11,16,6,5,1, 9,8,7,8,4,4,2,0]},
...
fn verify(t:&Tab)->Option<String>{
    if t.hlen.len()!=t.x*t.y || t.hcod.len()!=t.x*t.y {
        return Some(format!("T{}: dim mismatch hlen={} hcod={} xy={}",t.n,t.hlen.len(),t.hcod.len(),t.x*t.y));
    }
    let mut bits:Vec<(String,usize,usize)>=Vec::new();
    for xi in 0..t.x { for yi in 0..t.y {
        let i=xi*t.y+yi; let l=t.hlen[i]; let c=t.hcod[i];
        if l==0 {continue;}
        if c>=(1u32<<l){return Some(format!("T{}: code {} > {} bits at ({},{})",t.n,c,l,xi,yi));}
        bits.push((format!("{:0w$b}",c,w=l as usize),xi,yi));
    }}
    for i in 0..bits.len(){for j in 0..bits.len(){
        if i!=j && bits[j].0.starts_with(&bits[i].0){
            return Some(format!("T{}: '{}'->({},{}) prefix of '{}'->({},{})",t.n,bits[i].0,bits[i].1,bits[i].2,bits[j].0,bits[j].1,bits[j].2));
        }}}
    None
}
...
fn main(){
    ...
    let mut bad=false;
    for t in &ts {
        if let Some(e)=verify(t){ eprintln!("FAIL {}",e); bad=true; }
    }
    if bad { eprintln!("VERIFICATION FAILED — not emitting"); std::process::exit(1); }
    eprintln!("All {} tables verified prefix-free.", ts.len());
    for t in &ts {
        let mut entries:Vec<(u32,u8,usize,usize)>=Vec::new();
        for xi in 0..t.x { for yi in 0..t.y { let i=xi*t.y+yi; let l=t.hlen[i]; if l==0{continue;} entries.push((t.hcod[i],l,xi,yi)); }}
        println!("pub static T{}: [HuffEntry; {}] = [",t.n,entries.len());
        let mut line=String::from("    ");
        for (k,(c,l,x,y)) in entries.iter().enumerate(){
            line.push_str(&format!("he!(0b{:b}, {}, {}, {}), ",c,l,x,y));
            if (k+1)%4==0 { println!("{}",line.trim_end()); line=String::from("    "); }
        }
        if line.trim().len()>0 { println!("{}",line.trim_end()); }
        println!("];");
        if t.lin>0 { println!("// T{} linbits={}",t.n,t.lin); }
    }
}
