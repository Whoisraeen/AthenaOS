// athsign — Ed25519 (RFC 8032) detached-signature tool for AthenaOS secure boot,
// atomic-update verification, and app-bundle code signing. Built on the shared
// `ath_crypto` Ed25519 so signer and on-device verifier are the same code.
//
// Subcommands:
//   athsign selftest
//       Run the RFC 8032 §7.1 known-answer test (proves the tool's crypto).
//   athsign keygen <passphrase>
...
//   athsign pubkey <seed-hex>
//       Print the public key (hex) for a 32-byte seed.
//   athsign sign <in-file> <seed-hex> <out-sig>
//       Write a 64-byte detached signature of <in-file> to <out-sig>.
//   athsign verify <in-file> <sig-file> <pubkey-hex>
...
fn cmd_selftest() {
    ...
    let pk = ath_crypto::ed25519::derive_public_key(&seed);
    let sig = ath_crypto::ed25519::sign(&seed, &msg);
    let pk_ok = hex_encode(&pk) == expect_pk;
    let sig_ok = hex_encode(&sig) == expect_sig;
    let verify_ok = ath_crypto::ed25519::verify(&pk, &msg, &sig);
    let forge_rejected = !ath_crypto::ed25519::verify(&pk, b"forged", &sig);
...
fn cmd_keygen(passphrase: &str) {
    ...
    let salt = b"athsign-keygen-1";
    let mut seed = [0u8; 32];
    ath_crypto::argon2id_derive(passphrase.as_bytes(), salt, 3, 8_192, 1, &mut seed);
    let pk = ath_crypto::ed25519::derive_public_key(&seed);
...
fn cmd_sign(in_file: &str, seed_hex: &str, out_sig: &str) {
    ...
    let sig = ath_crypto::ed25519::sign(&seed, &data);
