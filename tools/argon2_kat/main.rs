// Host harness for the SHARED `ath_crypto` crate — the exact code the kernel
// (FDE key derivation) and athid (account password hashing) use. Runs the
...
// embargoed. `ath_crypto` also carries these as `#[cfg(test)]` unit tests
// (`cargo test -p ath_crypto`); this binary is the standalone, dependency-free
...
fn main() {
    ...
    let got = ath_crypto::blake2b(64, b"abc");
    ...
    let mut tag = [0u8; 32];
    ath_crypto::argon2id_full(&[0x01; 32], &[0x02; 16], &[0x03; 8], &[0x04; 12], 3, 32, 4, &mut tag);
