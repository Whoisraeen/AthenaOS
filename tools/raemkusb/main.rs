...
//
//   cargo run --manifest-path tools/raemkusb/Cargo.toml -- --out target/install-usb.img
//   cargo run --manifest-path tools/raemkusb/Cargo.toml -- --scratch --out target/install.img
        ...
        match args[i].as_str() {
            ...
            "--size-mib" if i + 1 < args.len() => {
                size_mib = args[i + 1].parse().unwrap_or_else(|_| {
                    eprintln!("raemkusb: bad --size-mib");
                    exit(2)
                });
                i += 2;
            }
            other => {
                eprintln!("usage: raemkusb [--scratch] [--boot-image <kernel.uefi.img>] --out <path.img> [--size-mib <N>]");
...
fn inspect_image(image: &str) {
    let mut img = std::fs::read(image).unwrap_or_else(|e| {
        eprintln!("raemkusb: read {image}: {e}");
        exit(2);
    });
    let (s0, s1) = find_esp_range(&img).unwrap_or_else(|| {
        eprintln!("raemkusb: no ESP in {image}");
        exit(2);
    });
    ...
    let fs = fatfs::FileSystem::new(slice, fatfs::FsOptions::new()).unwrap_or_else(|e| {
        eprintln!("raemkusb: open ESP FAT failed: {e}");
        exit(2);
    });
...
fn build_inject(boot_image: &str, out: &str) {
    ...
    let mut src = std::fs::read(boot_image).unwrap_or_else(|e| {
        eprintln!("raemkusb: read boot image {boot_image}: {e}");
        eprintln!("  (build it first: cargo run -p xtask --release -- build --release)");
        exit(2);
    });
    if src.len() < SECTOR * 3 {
        eprintln!("raemkusb: boot image too small to be a GPT disk");
        exit(2);
    }
    let (s0, s1) = find_esp_range(&src).unwrap_or_else(|| {
        eprintln!("raemkusb: no EFI System Partition found in {boot_image}'s GPT");
        exit(2);
    });
    ...
    let (bootx64, kernel, bootlog) = {
        ...
        let fs = fatfs::FileSystem::new(slice, fatfs::FsOptions::new()).unwrap_or_else(|e| {
            eprintln!("raemkusb: open source ESP FAT failed: {e}");
            exit(2);
        });
        let root = fs.root_dir();
        let bootx64 = read_fatfs_file(&root, &["EFI", "BOOT"], "BOOTX64.EFI").unwrap_or_else(|| {
            eprintln!("raemkusb: EFI/BOOT/BOOTX64.EFI not found in source image");
            exit(2);
        });
        let kernel = read_fatfs_file(&root, &[], "kernel-x86_64").unwrap_or_else(|| {
            eprintln!("raemkusb: kernel-x86_64 not found in source image");
            exit(2);
        });
    ...
    eprintln!(
        "raemkusb: extracted boot tree — BOOTX64.EFI {} B, kernel-x86_64 {} B{}",
        bootx64.len(),
        kernel.len(),
        match &bootlog {
    ...
    }
    {
        ...
        let fs = fatfs::FileSystem::new(slice, fatfs::FsOptions::new()).unwrap_or_else(|e| {
            eprintln!("raemkusb: open dest ESP FAT failed: {e}");
            exit(2);
        });
        let root = fs.root_dir();
    ...
    if !ok {
        eprintln!("raemkusb: FAT32 readback verification FAILED");
        exit(2);
    }

    std::fs::write(out, &img).unwrap_or_else(|e| {
        eprintln!("raemkusb: write {out}: {e}");
        exit(2);
    });

    println!(
        "raemkusb: wrote {} ({} MiB) — FAT32 ESP with EFI/BOOT/BOOTX64.EFI + kernel-x86_64 + INSTALL.NOW (readback OK)\n\
...
fn write_fatfs_file<IO: fatfs::ReadWriteSeek>(dir: &fatfs::Dir<IO>, name: &str, data: &[u8]) {
    let mut f = dir
        .create_file(name)
        .unwrap_or_else(|e| panic!("raemkusb: create {name}: {e}"));
    f.truncate().ok();
    f.write_all(data)
        .unwrap_or_else(|e| panic!("raemkusb: write {name}: {e}"));
    f.flush().ok();
}
...
fn build_scratch(out: &str, size_mib: u64) {
    ...
    if total_sectors < 4096 {
        eprintln!("raemkusb: --size-mib too small");
        exit(2);
    }
    let mut img = vec![0u8; total_sectors as usize * SECTOR];
    ...
    println!(
        "raemkusb: wrote {} ({} MiB, --scratch): GPT + FAT32 ESP (LBA {}..{}, {} sectors) + INSTALL.NOW",
