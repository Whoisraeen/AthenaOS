...
> "Steam works day one via AthBridge — non-negotiable; without Steam there is no
> PC gaming OS."
...
runs on AthenaOS's own syscalls, with Wine/Proton as a _reference_ rather than a
...
|---|---|---|
| Wine source | ~3.5M C | the surface AthBridge must *functionally* replace |
| AthBridge today | ~72k Rust | real, working, ~2% of the functional surface by area |
| AthBridge 1.0 budget | ~800k Rust | single largest first-party module (`MasterChecklist.md` sizing table) |
...
## 2. Where AthBridge actually is (proven, not aspirational)
...
| Registry shim backed by versioned config | `[~]` | `win_registry.rs`; survives reboot, rides AthFS snapshot/rollback |
...
AthenaOS" promise is *real for genuine compiler output*. What's missing is breadth
...
AthBridge uses **Wine's in-process model**, which is the right call for AthenaOS:

- **Guest VA == host VA.** The PE is `sys_mmap`-mapped into the launcher process's
  own address space; AthenaOS user pages are executable, so guest code runs
...
- **One loader process per `.exe`** (`handoff.rs` + `launcher.rs`): a guest
  ...
  structural "each app is its own AthenaOS process" model, and it's what makes the
  capability sandbox (per-app virtual `C:\` = per-app AthFS bucket) coherent.
...
- **Keep from-scratch Rust** for the OS-coupled core: `ntdll` (NT syscalls → AthenaOS
  syscalls), `kernel32` process/thread/sync/heap, the loader, TEB/PEB, SEH. These
  *must* be tightly fused to AthenaOS internals; a Wine port would fight the kernel.
...
- But the **`MasterChecklist.md` sizing analysis** says: *"Aggressive reuse of
  ...
  AthBridge alone past 2M LOC,"* and Phase 11 still lists `[ ] DXVK port` /
  `[ ] VKD3D-Proton port`.
...
AthGFX has a real Vulkan submit path on the amdgpu bring-up. So this fork is a
...
- `HWND` → a AthShell compositor surface (so Win32 windows get glass/HDR/VRR for free),
- `WM_PAINT` → a `gdi32` raster into that surface (SW raster now; AthGFX-accelerated later).
...
6. `CreateWindowEx` → AthShell compositor surface; `WM_PAINT` → `gdi32` SW raster.
7. **Acceptance:** `notepad.exe` runs, types, saves (Phase 11.3 gate #1).

**Phase D — games (gated on AthGFX real-GPU submit):**
...
10. Vulkan surface from AthGFX → D3D runtime → **Steam runs** (the non-negotiable test).
...
- **HUMAN-GATED.** Net-new AthBridge *guest-execution* work requires explicit owner
...
- **Every guest-visible primitive is capability-gated and sandboxed** — a Windows app
  sees a virtual `C:\` that is its own AthFS bucket; no global registry, no shared
...
2. **Porting wineserver.** Use native AthenaOS IPC + capabilities (§6.1).
