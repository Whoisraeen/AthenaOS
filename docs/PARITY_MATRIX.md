...
Owner: athena-parity. This is the EXTENSIVE per-capability companion to
...
- Media pillar: athmedia from-scratch PNG/JPEG/WAV decoders + EXIF (165 KATs,
...
- Web/PWA foundation (all host-KAT'd pure libs): athnet HTTP/1.1 client (`1a4be7c`)
  + live TCP/DNS transport (`253467a`); ath_json RFC 8259 (`b559025`); ath_pwa W3C
  manifest (`1b67e01`); ath_markdown CommonMark subset (`f900146`).
...
(`0c4abb2`), settings redesign (`86d36c2`/`d190f8c`), athweb engine (`0cd527e`).
...
| Overview / expose | macOS Mission Control | live thumbnails + drag-between-spaces, GPU-cheap | `[ ]` missing (IN FLIGHT) | `athshell/animations.rs` has a `DesktopOverview` action enum but no live thumbnail/compositor mechanism yet |
| Virtual desktops / Spaces | macOS Spaces, Win Virtual Desktops | per-monitor workspaces + per-space wallpaper | `[ ]` missing (IN FLIGHT) | `athshell/virtual_desktops.rs` (1334 lines) exists as a module + `WorkspaceSwitch` animation, but is NOT instantiated in live `DesktopShell` (declared `pub mod` only, same dead-code pattern as the search index); compositor has no per-space scanout/workspace concept |
| Stage-Manager-style grouping | macOS Stage Manager | optional auto-grouping, off by default | `[ ]` missing | none |
| App switcher | macOS Cmd-Tab / Win Alt-Tab | live previews, type-to-filter | `[~]` partial | `athshell` cycle_alt_tab; lead added live-thumbnail switcher chrome (concurrent-owned shell render); no type-to-filter |

## B. Search & launch

| Capability | Best today + why | Target | AthenaOS status | Evidence |
|---|---|---|---|---|
| Instant local search | macOS Spotlight (indexed, instant, math/units) | sub-100ms index across files+apps+settings | `[~]` partial | TWO indexes: (1) DEAD athshell `search_indexer` (TF-IDF/trigram/calc, never instantiated); (2) LIVE kernel `search_index.rs` w/ full syscall surface (add/remove/query/stats, wired+smoketested) but NOTHING POPULATES IT and nothing QUERIES it yet. Live Start search = app-name substring only. THE GAP IS WIRING, not engine |
| Command palette (run actions) | rofi / VSCode palette / mac Spotlight actions | one palette runs ACTIONS, not just finds files | `[~]` partial | `athshell/command_palette.rs` LIVE (instantiated in `DesktopShell` line 1597, rendered line 1787; fuzzy match + command/app registry + seeded settings-actions). QEMU-booted `0b026f1`. GAP: not wired to `search_indexer` for file content; iron unproven |
| Web/answer in search | Spotlight, Win Search | optional, privacy-respecting | `[ ]` missing | transport now exists (athnet HTTP/1.1 + live TCP/DNS, `1a4be7c`/`253467a`) but no search-bar web action wired |

## C. File management

| Capability | Best today + why | Target | AthenaOS status | Evidence |
|---|---|---|---|---|
| Tabs / split panes | Win11 Explorer tabs + mac Finder tabs | tabs + split + dual-pane | `[~]` partial | TABS LIVE: `apps/files` uses `ath_files::TabSet` (host-KAT'd tab/history model; up to 8 tabs, +/close, per-tab back/forward, line ~650/744/1870). GAP: no split / dual-pane yet; iron-unproven |
| Fuzzy in-folder search | Finder / Explorer | wired to the shared index | `[~]` partial | TWO indexes exist: athshell `search_indexer` is DEAD (declared, never instantiated) AND a LIVE kernel `kernel/src/search_index.rs` (init+smoketest in main.rs 1095; syscalls add/remove/query/stats wired in `syscall.rs` 1908-1926; procfs stats). GAP: nothing POPULATES the kernel index (no AthFS crawler) and Files/palette don't QUERY it |
| Quick Look / preview | macOS Quick Look (spacebar) | spacebar preview for img/text/pdf/media | `[~]` partial | `apps/files` Quick Look renders REAL PNG pixels via `athmedia` (`8550989`); athmedia now also decodes JPEG (`461985e`) + EXIF orientation (`720e99e`) + WAV, so a wired QuickLook can show jpeg too. GAP: Files app not yet calling jpeg/EXIF path; no text/pdf/video; no spacebar gesture; iron-unproven |
| Batch rename | macOS Finder rename | pattern + counter + find/replace | `[~]` partial | LIVE: `apps/files` batch-rename dialog over host-KAT'd `ath_files::batch_rename_target` (pattern + counter, line 1735/1768/1796). GAP: no find/replace mode; iron-unproven |
| Trash / Recycle Bin | both | undoable delete + restore | `[~]` partial | LIVE: `apps/files` Trash = CoW move into `<home>/.Trash` bucket via `SYS_RENAME` (trash/restore/empty over host-KAT'd `ath_files::{trash_target,restore_target}`, line 995-1038). GAP: no global undo, no auto-purge policy; iron-unproven |
| Tags / smart folders | macOS Finder | saved searches + tags | `[ ]` missing | none |
| File associations / default apps | both | per-type default + open-with | `[ ]` missing | confirmed: no default-app/open-with registry in `apps/`, `athshell`, `athstore`, or kernel. Files app has no "open with" path; double-click has no MIME->app resolution. Daily-driver blocker (can't open a downloaded file by clicking it) |
| Removable media mount/eject | both | auto-mount + eject | `[~]` partial | USB-MSC enumerate blocked on iron |
| Per-app data buckets | AthFS (Concept differentiator) | isolation by default | `[~]` partial | `kernel/src/athfs.rs` buckets |
...
| Capability | Best today + why | Target | AthenaOS status | Evidence |
|---|---|---|---|---|
| Region/window/scroll capture | macOS screenshot + Flameshot | region+window+scroll, hotkey | `[~]` partial | Capture ABI 274-276 LANDED (`Cap::ScreenCapture`-gated, safe-mode-refused; `CaptureHeader` + pixels); athmedia PNG encoder saves real `.png` (`89cce24`); athshell region-capture overlay Super+Shift+S (`0c4abb2`, concurrent-owned shell). GAP: no scroll capture, markup, or iron proof |
...
| Capability | Best today + why | Target | AthenaOS status | Evidence |
|---|---|---|---|---|
| Handoff / file beam | macOS Handoff/AirDrop | AthSync-backed handoff + beam | `[~]` partial | `components/athsync` compiles (E2E); no live transport |
...
| Capability | Best today + why | Target | AthenaOS status | Evidence |
|---|---|---|---|---|
| Snapshots + rollback | macOS Time Machine | CoW snapshots + one-click rollback | `[~]` partial (iron-proven engine, no UI) | `athfs.rs` snapshot/rollback iron-proven; no Settings UI |
...
| Code signing / notarization | Gatekeeper / SmartScreen | Ed25519 bundle sign + unverified-dev UX | `[~]` partial | `athshield` Ed25519 signing |
...
> and `ath_tokens::active_palette() == HIGH_CONTRAST` via `a11y::set_high_contrast` (proven by
...
| High contrast / color filters | both | palette swap via tokens | `[~]` partial (was `[ ]`; SHIPPED 2026-06-21) | LIVE forced-colors swap: `a11y::toggle_high_contrast`/`set_high_contrast` -> `ath_tokens::active_palette() == HIGH_CONTRAST`; on-switch Super+Alt+H + Control Center tile; compositor color filters (`a11y_filter_set`, Super+Alt+C). Proven by `a11y::run_onswitch_smoketest` + host KAT. GAP: broaden surface coverage; iron-unproven |
...
| Package manager | brew / winget / apt | AthStore CLI + repos | `[~]` partial | `athstore` compiles |
...
| System info / About | both | one panel from /proc/athena | `[~]` partial | data exists; no UI |
...
| GameOS / Big Picture | none | couch shell, controller-only | `[~]` partial | `athshell/gameos.rs` |
...
| IME / layouts | both | CJK + layouts | `[ ]` missing | confirmed: `usb_hid.rs::bridge_scancode` ships raw scancodes; the scancode->char map is fixed US-QWERTY (no AZERTY/QWERTZ/Dvorak), `athlocale` has NO keyboard-layout data, no IME framework. Switcher blocker for non-US users; QEMU-verifiable for alt-LAYOUTS (CJK IME is bigger, defer) |
...
| Capability | Best today + why | Target | AthenaOS status | Evidence |
|---|---|---|---|---|
| App store + sideload + PWA | App Store / Store / Flatpak | AthStore + sideload + PWA | `[~]` partial | `athstore` shell; ath_pwa parses W3C Web App Manifest (`1b67e01`, install-to-desktop foundation) but no install flow wired; sideload missing |
| Windows app compat | none (the moat) | AthBridge (Wine/Proton lineage) | `[~]` partial | `athbridge` LDR/SEH host-KAT'd; GPU/Steam blocked |
...
| Capability | Best today + why | Target | AthenaOS status | Evidence |
|---|---|---|---|---|
| Calculator | both | real f64 calc | `[~]` partial | `apps/calculator` + `ath_calc` (17 KATs) |
| Notes | both | quick notes | `[~]` partial | LIVE: `apps/notes` real app — sidebar `.md`/`.txt` list in `<home>/Notes`, edit buffer + live `ath_markdown` preview (Tab toggle, Ctrl+S save, New/Delete), themed via `SYS_THEME_GET`. GAP: iron-unproven, no sync |
...
| Photos / image viewer | both | viewer + basic edit | `[~]` partial | `apps/photos` LIVE over athmedia PNG/JPEG + EXIF orientation (`720e99e`); QEMU-booted. GAP: no edit, no zoom/pan UI proven, iron-unproven |
| Media player | both | symphonia-backed player | `[~]` partial | `apps/music` LIVE over athmedia WAV decoder -> mixer PCM (`f836954`); QEMU-booted. GAP: WAV-only (no mp3/flac/aac), no video, no playlist UI depth |
...
LANE DISCIPLINE: a concurrent opus session owns athshell (launcher/switcher
chrome), shell_runner (hotkeys), athweb (web engine), gameos, and the GPU stack.
...
athmedia, athnet client surface.

### MY-LANE wave (disjoint — runnable once image is green):

1. [DAILY] **Notes app** (`apps/notes`) on ath_markdown. NEW crate, zero
...
3. [SWITCHER] **Files Quick Look: JPEG + EXIF + thumbnails** (`apps/files`). Wire
   athmedia jpeg/EXIF (already decoding) into the existing PNG QuickLook path +
   a thumbnail grid. My-lane (`apps/files` is mine). Boot-verifiable.
4. [DAILY/SWITCHER] **GIF + WebP decoder, then Video player** (athmedia +
   `apps/music`->media). GIF/WebP are host-KAT-only pure decoders (extend
   athmedia, 165-KAT pattern); video player app is boot-verifiable. My-lane.
   Sequence: decoders first (host-KAT), then player UI.
5. [SWITCHER] **Web-fetch / PWA-install demo** on athnet HTTP + ath_pwa +
   ath_json + ath_markdown. A small `apps/` demo: fetch a URL, parse a manifest,
   install-to-launcher stub, render markdown. Pure my-lane libs. Web FETCH is
   host-KAT-able; the launcher-install hook touches athshell -> SEQUENCE-AFTER
   concurrent (stub the registration locally, hand the wire-up to athshell).
6. [SHIP GATE] **a11y: high-contrast palette + AudioSpeechSink** (`kernel/src/
   a11y.rs` + athmedia/athaudio). High-contrast = token palette swap (my-lane,
...
- Wire Photos/Music/PWA-install into the LAUNCHER (athshell) — concurrent owns
  athshell render; provide the app-registry rows, let them wire the tiles.
...
- Search index -> palette/Start (athshell `search_indexer` still dead) —
  athshell-owned; flag for sequencing.
...
- Two-session de-confliction — the shared image is red from concurrent GameBar
  WIP; my-lane waves wait on their fix-commit + a green build. The athshell/
  shell_runner/athweb/gameos/gpu lane boundary needs the owner's coordination so
