...
**Author:** athena-accessibility (read-only audit; only this doc was written)
...
- **`kernel/src/a11y.rs` (1236 lines) is real and wired:** accessibility tree built from the
  ...
  smoketests, `/proc/athena/a11y`, Concept docstring), `init()` called from `kernel_main` after
...
- **AT ABI is live & cap-gated:** `SYS_A11Y_SNAPSHOT` (277) + `SYS_A11Y_ACTION` (278) dispatched
  ...
  `copy_to_user`. `A11yNode`/`A11ySnapshotHeader` wire reprs in `ath_abi`.
...
- **athui widget semantics exist:** `components/athui/src/accessibility.rs` has real role
...
- **Contrast math exists & is tested:** `ath_tokens::contrast_ratio` (WCAG luminance) +
  `HighContrastPalette` mirror, unit-tested at >= 4.5 / >= 7.0.
- **Reduced-motion is honored (partially):** `shell_runner::reduced_motion()` reads
  `config_registry "/a11y/reduced_motion"` and gates an animation; `athshell::animations` has
  `set_reduce_motion`; `ath_tokens` has `MOTION_INSTANT`/`REDUCED_MOTION_DURATION_MS`.
- **Standalone a11y library:** `components/athaccessibility` (2268 lines) — rich AT data models
...
1. The widget-tier provider seam exists in BOTH the kernel (`publish_window_widgets`) and athui
...
athui widget role inference + `provider_nodes_for_window`. Status `[~]` in PARITY_MATRIX §J.
...
- **No reader navigation commands / gestures (P1):** no "next heading/link/form-field" verbs
  driven by live keys (the verbs exist in the unused `athaccessibility` crate, not the live path).

**Owner:** widget provider wiring = **athena-ui** (publish) + **athena-shell-apps** (per-app
nodes); TTS sink = **athena-accessibility** + **athena-audio** (iron). 
**Acceptance:** focusing a control in a real app makes `/proc/athena/a11y` show a named child
...
**EXISTS:** athui `focus_next/prev` + `focusable_nodes()` + `FocusRing::from_node`; per-surface
...
- **No focus trapping contract in modals (P1):** modal flag exists in athui traits; the
...
**Owner:** **athena-accessibility** (focus-order contract + ring spec + the "no-mouse" audit) +
**athena-shell-apps** (Tab order per surface) + **athena-ui** (per-widget focusability).
...
**Owner:** hotkey + Control-Center tile = **athena-shell-apps**; smooth-pan/lens =
**athena-gfx** + **athena-accessibility** policy. 
...
**EXISTS:** `ath_tokens` `HighContrastPalette` mirror + HC cyan focus ring + the compositor
`A11Y_FILTER_HIGH_CONTRAST` per-pixel filter; athui `HighContrastPalette` + `high_contrast_colors()`.
...
**Owner:** **athena-accessibility** (the flag + `active_palette()` contract) + **athena-ui** /
**athena-shell-apps** (read the active palette). theme_engine currently has **no** HC flag.
...
gates an animation (shell_runner:3949); `athshell::animations::set_reduce_motion`; `ath_tokens`
`MOTION_INSTANT` / `REDUCED_MOTION_DURATION_MS = 0`; athui `animation.rs` reference.
...
**Owner:** **athena-accessibility** (the audit + single source-of-truth flag) + **athena-gfx**
(compositor animation sites) + **athena-shell-apps** (shell/app animations).
...
**EXISTS:** `ath_tokens::contrast_ratio` (WCAG relative-luminance) + unit tests asserting
...
- **No measured-from-screenshot counterpart (P2):** rendered pixels differ from token intent
  (blur/blend over glass); needs **athena-visual-qa** measured ratios (heed the QEMU screendump
  stripe artifact — measure on host-render / iron).

**Owner:** **athena-accessibility** (token audit) + **athena-visual-qa** (measured).
...
### (g) Text scaling
**EXISTS:** athui layout computes bounds; ath_tokens type ramp exists.
...
- **No layout-fit audit (P2):** a scaled pass must not clip/overlap (checkable since athui
  computes bounds).

**Owner:** **athena-ui** (layout reads the factor) + **athena-accessibility** (the flag + fit
audit) + **athena-shell-apps** (settings toggle).
...
## BLOAT FLAG — `components/athaccessibility` (anti-bloat house rule)

`components/athaccessibility` is a **2268-line workspace member that NOTHING imports** (no
Cargo dependency, no `use`, no `init()` caller, no R10 contract, no procfs). It duplicates the
data shape now live in `kernel/src/a11y.rs` + `athui::accessibility` and adds speculative
...
Do not invest further in it as-is — the live path is the kernel a11y.rs + athui seam.
...
(`a11y::publish_window_widgets` / `athui::provider_nodes_for_window`) with **zero callers**, so
every app is one anonymous window to a screen reader. Wire athui to publish each window's widget
subtree (real role/label/bounds) into the kernel tree, starting with one real app (Settings or
Files).
- **Owner:** athena-ui (publish path) + athena-shell-apps (per-app nodes).
- **Acceptance:** focusing a control in a live app makes `/proc/athena/a11y` list a NAMED child
  node under that window with correct role/bounds; `describe_focused()` returns
...
- **Owner:** athena-shell-apps (shortcuts + Control-Center tile + Settings) — calls existing
...
already-universal `ath_tokens::DARK/LIGHT` read path). This closes the only fully-`[ ]` row in
PARITY_MATRIX §J.
- **Owner:** athena-accessibility (flag + `active_palette()` contract) + athena-ui /
  athena-shell-apps (read it).
...
(taskbar/start/tray/notifications/control-center) driven by athui `focus_next/prev`, a
...
- **Owner:** athena-accessibility (contract + audit) + athena-shell-apps (per-surface Tab order) +
  athena-ui (focusability).
...
Wire `ath_tokens::contrast_ratio` into an enumerated boot/host audit over every load-bearing
painted pair; print PASS/FAIL naming the worst pair + ratio. Cheapest real win, zero
locked-file risk, ships a ship-gate proof no competitor ships. Pair with athena-visual-qa for
the measured-from-screenshot counterpart.
- **Owner:** athena-accessibility + athena-visual-qa (measured).
...
| Widget role inference | built (host-KAT) | `athui/src/accessibility.rs:role_from_widget_kind`, `provider_nodes_for_window` |
| Contrast math | built (unit-tested) | `ath_tokens::contrast_ratio` + `HighContrastPalette` |
| Reduced-motion hook | partial | `shell_runner::reduced_motion()` + `animations::set_reduce_motion` + `MOTION_INSTANT` |
| Focus ring (drawing) | partial (gameos only) | `gameos.rs:draw_focus_ring` (4-signal); `athui FocusRing` |

The remaining work is **integration, user-reach, and audits** — not new engines. The one place
to be careful of duplication is `components/athaccessibility` (see Bloat Flag): build on the live
`kernel/src/a11y.rs` + `athui` seam, not that orphaned crate.
