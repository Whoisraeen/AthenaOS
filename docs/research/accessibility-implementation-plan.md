...
**Owner of this doc:** raeen-accessibility
...
**Why this is a ship gate (raeen-parity #7, flagged SHIP GATE):** a UI that rivals macOS and
...
| High-contrast / reduced-motion | HC themes + "show animations" off | Increase contrast + Reduce motion | per-DE | Token-level palette swap via `rae_tokens` + a global mode flag every animation already reads (`MOTION_INSTANT` / `set_reduce_motion`). Honored everywhere, machine-audited. |
| Contrast compliance | manual | manual | manual | **`rae_tokens::contrast_ratio` wired into a FAIL-able boot audit** — the OS proves WCAG AA on its own palette at boot. Nobody else does this. |

---

## 1. ACCESSIBILITY TREE (foundation — 19.1)

**Owner:** raeen-accessibility (kernel tree) + raeen-ui (widget provider). **Interface:**
...
- **Widget tier (raeen-ui populates the seam):** `components/raeui/src/accessibility.rs`
  ...
  only and every app is one anonymous "Window" node.** raeen-ui makes
...
### Which crate owns it
`kernel/src/a11y.rs` (NEW) owns the window-tier tree + cap-gated API + procfs. `raeui` owns the
widget-tier provider data. They meet at the `set_widget_provider` seam + the `rae_abi` `A11yNode`
repr(C).

### Files touched
- NEW `kernel/src/a11y.rs`; `kernel/src/main.rs` (init after `compositor::init`);
  `kernel/src/procfs.rs` (register `/proc/raeen/a11y`); `kernel/src/syscall.rs` (2 dispatch arms).
- `components/rae_abi/src/lib.rs` (Cap variant + 2 syscall numbers + `A11yNode` repr) —
  **raeen-architect only, `[interface]` commit, bump `ABI_VERSION`, update
  `docs/SYSCALL_TABLE.md` same commit**.
- `components/raeui/src/accessibility.rs` (raeen-ui fills the `infer_*` stubs + `Accessible`
...
**Owner:** raeen-accessibility (engine) + raeen-ui (widget labels via §1 provider) + raeen-kernel
(focus-change event). **Audio tail:** raeen-audio (iron-gated).
...
1. **Tree-walk + focus-announce core (host-KAT-able, build this):** given an `AccessNode` tree +
   ...
   `raeui::accessibility::AccessibilityTree::describe_focused()` ("label, role, value, state,
   hint"). Promote/mirror that as the kernel-side announcer over `a11y.rs` nodes so it works for
   the live (kernel) tree, not just the dead raeui tree. This is pure string logic — deterministic
...
- `kernel/src/compositor.rs` — **MINIMAL, serializes against WM/screenshot waves:** bump a focus
  ...
  WM waves, not concurrently. (If raeen-kernel prefers, expose it as a no-arg `note_focus_change()`
  that `focus_surface` calls — same one-line touch.)
- `components/raeui/src/accessibility.rs` (raeen-ui: real labels so the announcer says something).
...
**Owner:** raeen-gfx (compositor seam) + raeen-accessibility (focus-follows policy).
...
- **Focus-follows (raeen-accessibility policy):** on a focus-change event (§2), set
...
- `kernel/src/compositor.rs` — **the magnifier sampled-blit in the scanout step + the atomics +
  ...
  after WM/overview/screenshot settle, or hand raeen-gfx a merge window. Call this out explicitly.
...
**Owner:** raeen-accessibility (focus order + ring contract) + raeen-ui (per-widget focusability)
+ raeen-shell-apps (Tab order in each surface) + raeen-kernel (input toggles).

### Focus model — reuse what exists
- `raeui::accessibility::AccessibilityTree` ALREADY has `focus_next` (Tab), `focus_previous`
...
### What to build
1. **A consistent focus-ring contract** drawn from tokens: the ring color/glow = `rae_tokens`
   ...
   `HighContrastPalette.focus_ring` (cyan `0xFF00FFFF`, already in raeui). Focus is **never
...
5. **Input-layer toggles (raeen-kernel):** sticky keys / slow keys / key-repeat live in
   `kernel/src/input.rs` (foundation §9 names them). Additive, host-KAT-able state machines.

### Modes (tie to theme engine / tokens)
- **Reduced-motion:** the hook ALREADY EXISTS — `raeshell::animations` has `reduce_motion` +
  `set_reduce_motion`, and `rae_tokens::REDUCED_MOTION_DURATION_MS = 0` / `MOTION_INSTANT`. Add a
  **single global flag in `kernel/src/theme_engine.rs`** (an `AtomicBool`, mirrored to userspace
  ...
  honors it.
- **High-contrast:** `raeui::HighContrastPalette` exists; promote it to a `rae_tokens`
  high-contrast `Palette` variant so the swap is one token lookup the whole stack already reads
  (every surface already pulls `rae_tokens::DARK`/`LIGHT`). theme_engine holds the
...
- **Scalable text:** a global text-scale factor (e.g. 1.0/1.25/1.5/2.0) in theme_engine; layout
  ...
  overlapping bounds in the tree). raeui layout already computes bounds, so this is checkable.

### Files touched
- `components/raeui/src/accessibility.rs` (focusability/labels — additive).
...
- `components/raeshell/*` + `apps/*` (Tab order per surface) — **shell/app surfaces; serialize
  against the shell wave in flight.**
- `components/rae_tokens/src/lib.rs` (HC palette variant + a `MOTION_INSTANT`/reduced helper if
...
**Owner:** raeen-accessibility + raeen-visual-qa (measured values from real screenshots).

### Mechanism
`rae_tokens::contrast_ratio(fg_argb, bg_argb) -> f32` ALREADY EXISTS (rae_tokens/src/lib.rs:340,
...
- This is a kernel/host audit over the static token palette. raeen-visual-qa supplies the
  *measured* counterpart from real screenshots (rendered pixels can differ from token intent due
  to blur/blend); file mismatches back to raeui/shell-apps with the exact widget + measured ratio.

### Files touched
- NEW host-KAT + boot smoketest: `kernel/src/a11y.rs::run_contrast_audit()` (or a small
  `a11y_contrast` submodule) calling `rae_tokens::contrast_ratio` over the palette pair list.
- `components/rae_tokens/src/lib.rs` already has the fn + tests (no change needed beyond possibly
...
- **iron-gated:** none for token math; raeen-visual-qa's *measured-from-screenshot* ratios are the
...
| Piece | Owner(s) | Touches `compositor.rs`? | Touches `raeui`? | Serialize against |
|---|---|---|---|---|
...
| §5 Contrast audit | accessibility + visual-qa | no | no (rae_tokens) | nothing — parallel-safe |
...
## 7. NEEDS-INTERFACE (raeen-architect, land FIRST in an `[interface]` commit)
...
1. `Cap::Accessibility { rights: Rights }` (READ = read tree, WRITE = dispatch actions) in
   `rae_abi` + `kernel/src/capability.rs`. Additive enum -> **bump `ABI_VERSION`** + update
   `docs/SYSCALL_TABLE.md` same commit.
2. `SYS_A11Y_SNAPSHOT` + `SYS_A11Y_ACTION` (architect assigns next free numbers; last allocated
   ...
   `docs/SYSCALL_TABLE.md`). Number in `rae_abi` + dispatch arm in `syscall.rs` same commit.
3. `repr(C) A11yNode { id, parent, role, state, x, y, w, h, actions, name[48] }` in `rae_abi`
...
1. **§1 Accessibility tree (kernel `a11y.rs` window tier).** *First — everything depends on it.*
   ...
   - Sub-slice 1a (parallel, raeen-ui): fill `raeui::accessibility::infer_role/infer_label`
...
window-tier tree** (after raeen-architect lands the §7 interface), with **§5 the contrast audit
...
`[~]` rows. raeen-ui's `infer_*` fill (1a) runs alongside so the tree names real widgets.
