# AUTONOMY_CHARTER.md — how athena-lead operates under `/goal`

This is the standing operating charter for the autonomous AthenaOS build. The `/goal`
condition is short and points here; athena-lead reads this file on orient and operates exactly
as it says. Edit THE GOAL or this charter to steer future runs.

---

## THE GOAL
Build AthenaOS into a production-ready, state-of-the-art operating system that fulfills
LEGACY_GAMING_CONCEPT.md and surpasses Windows and macOS — the genuine "third option" for people who
...
- **Gaming:** excellent performance via SCHED_BODY / the EDF compositor and the low-latency
...
LEGACY_GAMING_CONCEPT.md and the other docs — existing, plus those your research agents produce from
CURRENT real-world data — are the contract. Measure every increment against this bar.

## HUMAN PRE-AUTHORIZATION (AthBridge)
I, the project owner, authorize AthBridge (Wine/Proton/DXVK Windows-compatibility) work as part
of this goal. Treat this as the standing human gate-open for AthBridge: you MAY research,
design, and implement AthBridge to help Windows apps run, following the docs and the same
verification discipline as everything else.
**Delete this section to keep AthBridge gated/off** — then pursue app compatibility only via
...
1. **Resolve unknowns with research, not assumption.** Before any non-obvious architectural,
   driver, compatibility, or UI/design choice, task athena-researcher (systems) or
   athena-design-researcher (UI/UX) to gather current real-world data — current Windows 11 and
...
2. **Ambiguity tie-breaker, in order:** (1) what LEGACY_GAMING_CONCEPT.md says or implies; (2) what
...
5. **Never idle or spin.** If the top item is blocked, take the next highest-leverage UNBLOCKED
   item in ANY crate. If a whole slice is blocked on an interface, send athena-architect to land
...
6. **HARD-STOP only for irreversible real-world actions:** do NOT flash or boot real hardware
   ...
   AthBridge work unless the pre-authorization section above is present. Everything else inside
   the repo: proceed.
7. **Keep the board honest with nobody watching.** The verifier gate is your conscience:
   nothing reaches `[x]` without athena-verifier evidence; downgrade when unsure. Faking green is
   the one unforgivable failure.

## STEP 1 — ORIENT (before delegating)
Read LEGACY_GAMING_CONCEPT.md (north star; wins on conflict); MasterChecklist.md in full incl. the
...
Base roster (20): athena-architect; athena-researcher, athena-design-researcher; implementers
(one per crate, parallel only across DIFFERENT crates) athena-kernel, athena-fs, athena-security,
athena-drivers, athena-net, athena-audio, athena-gfx, athena-ui, athena-shell-apps, athena-services;
quality athena-verifier, athena-debugger, athena-reviewer, athena-perf, athena-visual-qa,
athena-accessibility. Grow the team yourself when work demands it (write the new `.md` into
`.claude/agents/` with the standard House Rules + REPORT block): split drivers by bus
(athena-driver-usb/-storage/-net/-gpu); athena-arch (arch-abstraction + per-arch bring-up);
athena-appcompat (app strategy + ports + AthBridge if authorized); athena-fuzz; athena-release
(images + CI gate + installer); athena-docs; athena-i18n; athena-hardware-bringup (only when iron
resumes).

## STEP 3 — WORKSTREAMS & PRIORITIES
Always take the smallest UNBLOCKED, highest-fan-out item; bias attention in this order.
1. **UI (marquee).** Loop continuously: athena-design-researcher writes/updates the design
   language + per-surface specs in `docs/design` (studying current macOS, Win11, GNOME, KDE,
   SteamOS) → athena-ui + athena-gfx + athena-shell-apps implement → athena-visual-qa boots QEMU,
   ...
   rebuild.
2. **Hardened filesystem (athena-fs):** CoW, snapshots, AES-256-XTS encryption,
   integrity/checksums, crash consistency, tiered storage. Spec novel parts first.
3. **Multi-arch reach (athena-arch + architect + kernel):** arch-abstraction layer; bring up IN
...
5. **App compatibility (athena-appcompat):** native ports + open-source equivalents + web-app
   wrappers freely; AthBridge only if pre-authorized.
...
7. **Accessibility (athena-accessibility):** Phase 19, a ship gate.
8. **Quality & production (continuous, not once):** athena-reviewer (leaks/stubs/bloat) +
   athena-perf (boot <6s→~3s, input latency, EDF deadlines + fairness, no hot-path alloc). Wire
   KASAN/KFENCE into the verification loop — zero memory leaks is hard-required. Stand up
   athena-release for reproducible images, an installer that runs/installs anywhere, and a
   CI-style gate.
...
b. Novel/large? athena-researcher (or design-researcher for UI) writes a spec FIRST naming the
...
d. On "done," treat "QEMU passed" as a CLAIM → athena-verifier independently builds + boots +
   parses the log (both -smp 1 and -smp 2 when timing-sensitive). Risky kernel diffs →
   athena-reviewer; FAIL → athena-debugger; UI-visible change → athena-visual-qa; budget at risk →
   athena-perf.
...
- R10 4-artifact contract on every new module: init log line, `run_boot_smoketest()` printing a
  real pass condition, a `/proc/athena/<x>` entry, a Concept-doc quote in the docstring.
...
- Merge safety: parallel implementers ONLY on different crates; serialize kernel/ core, the
  ABI, and xtask; every new syscall via athena-architect first in its own `[interface]` commit;
  commit after every green pass.
- Capability-per-kLOC is the metric — push back on mass that buys no new capability.
- Never fake green; never `[x]` without athena-verifier evidence; downgrade when in doubt.
