...
|---|---|---|
| **Measured boot** (PCRs) | *Did the exact, untampered OS image boot, in order?* | `/proc/athena/measured_boot`, `kernel::measured_boot`, `ath_crypto::pcr` |
| **Process integrity** | *Is this process's code unmodified — no injected hooks, no debugger, W^X intact?* | `SYS_AC_*` syscalls, `/proc/athena/anticheat`, `kernel::anticheat` |
...
[`ath_crypto::pcr`](../components/ath_crypto/src/pcr.rs) core; the kernel drives it from
...
`ath_crypto::pcr::verify_log(log, pcr_index, &golden)` — it replays the log and returns
...
detection — surfaced via `/proc/athena/anticheat` and folded into the attestation quote
...
- A native app requests it through the `SYS_AC_*` syscalls, which check the caller's
  `Cap`.
- A **sandboxed Wasm app** ([`athwasm`](../components/athwasm/)) cannot issue syscalls
  ...
  [`athwasm::pkg`](../components/athwasm/src/pkg.rs) manifest, and the call traps unless
...
| Layer | Status |
|---|---|
| Software measured-boot core (`ath_crypto::pcr`) + kernel `measured_boot` | **Live + host-KAT'd + real-Ryzen verified** (two-stage PCR[4]/PCR[8]) |
...
- **Built + proven:** measured-boot two-stage PCR chain (real-Ryzen verified), the
  ...
  `/proc/athena/{measured_boot,anticheat}`, the capability gate.
