# RaeenOS Settings Catalog & UI Map

The complete settings surface for the userspace **Settings** app — every toggle,
slider, and picker RaeenOS should expose, at parity with Windows 11 Settings,
macOS System Settings, and GNOME/KDE, plus the RaeenOS-native categories
(Gaming, RGB/Vibe, RaeShield, RaeBridge, Drivers).
...
- **Runs entirely in userspace** (RaeUI/RaeKit app, `apps/settings/`). No setting
...
- **Every write is capability-gated** via RaeShield: changing security/driver/network
...
- **Sync** optional per-category via RaeSync/RaeID (like iCloud/Microsoft-account
...
| Atomic update slots (A/B), rollback to previous build | RaeenOS-native | Phase 3.6 update slots | 🟡 |
...
| Refresh rate + **VRR / adaptive sync** | gaming-first | compositor VRR pacer | 🟡 |
...
| **Routing matrix** (VoiceMeeter-class input→fx→output) | RaeenOS-native | raeaudio AudioRouter | 🟡 |
...
| **Game controllers** (DualSense/Xbox: test, remap, deadzones, rumble, LED) | gaming-first | input (DualSense/Xbox) | 🟡 |
...
| Mobile device / phone link | | RaeSync | ⬜ |
...
| **Firewall** (per-app, inbound/outbound rules, zones) | | RaeShield, sandbox net classes | 🟡 |
...
| **Vibe Mode presets** (Cyberpunk Night, Ghibli Morning, Bauhaus…) | RaeenOS-native: wallpaper+colors+sound+fonts+cursor+anim as one | theme_engine | ⬜ |
...
| **Live wallpaper** (GPU, pause when occluded) | RaeenOS-native | live_wallpaper | 🟡 |
| **Glassmorphism** (blur radius, transparency, vibrancy) | RaeenOS-native | compositor | 🟡 |
...
| **App permissions** (per-app: camera/mic/location/files/…) | → §12 | RaeShield, rae_manifest | ✅ |
| Optional features / components | | app_bundle | ⬜ |
| **App bundle dependency view** (hashed deps) | RaeenOS-native | app_bundle (SYS_BUNDLE_VERIFY) | 🟡 |
| Offline maps / per-app storage / app data buckets | | raefs data buckets | 🟡 |
| App store settings (RaeStore: auto-update, sources) | | raestore | ⬜ |

## 8. Accounts & Users

| Setting | Notes | Backing | Status |
|---|---|---|---|
| **RaeID** account (sign in, profile) | like MS account / Apple ID | RaeID | 🟡 |
| Local user accounts (add, type, admin) | | RaeID, capability | ⬜ |
| Sign-in options: password, PIN, **biometrics**, security key | | RaeID, RaeShield, TPM | ⬜ |
| Auto-login / lock policy / dynamic lock | | RaeShield | ⬜ |
| Family / parental controls / screen time | | RaeID | ⬜ |
| **Settings sync** (which categories sync) | | RaeSync | ⬜ |
| Work/school / enterprise enrollment (MDM) | | RaeID | ⬜ |
...
## 10. Gaming (RaeenOS flagship)

| Setting | Notes | Backing | Status |
|---|---|---|---|
| **Game Mode / SCHED_GAME** (auto-prioritize foreground game) | hard real-time class | scheduler SCHED_GAME | 🟡 |
| Game Bar / in-game overlay (FPS, perf, chat) | | compositor | ⬜ |
| **Captures** (clips, screenshots, hotkeys, bitrate) | | compositor, raeplay | ⬜ |
| **GameOS / Couch mode** (big-picture UI) | RaeenOS-native | raeplay GameOS | 🟡 |
| Per-game profiles (power, RGB, resolution, fan) | RaeenOS-native | overclock, rgb, power_supply | 🟡 |
...
| **RaeBridge / Windows-game compatibility** (per-title) | Proton-lineage | raebridge | 🟡 |
...
## 12. Privacy & Security (RaeShield)

| Setting | Notes | Backing | Status |
|---|---|---|---|
| **Per-app permissions** (camera, mic, location, files, network, devices) | mac/iOS-style consent | RaeShield, rae_manifest, perm_prompt | ✅ |
| **App sandbox level** (Trusted/AppSandbox/Strict) per app | RaeenOS-native | sandbox.rs | 🟡 |
...
| Firewall (→ §5) | | RaeShield | 🟡 |
| Find my device / remote wipe | | RaeID, RaeSync | ⬜ |
| Activity history / diagnostics data view + delete | | audit | 🟡 |
| Anti-theft / lockdown mode | | RaeShield | ⬜ |
...
| **CPU power limits per game/app** | RaeenOS-native | overclock | 🟡 |
...
## 15. Storage & Filesystem (RaeFS)

| Setting | Notes | Backing | Status |
|---|---|---|---|
| Disks & volumes (manage, format, mount) | | raefs, gpt, block_io | 🟡 |
| **Snapshots** (list, create, **one-click rollback**) | RaeenOS-native | raefs snapshots/CoW | ✅ |
| **Tiered storage** (hot/cold placement policy) | | raefs tiered storage | 🟡 |
| **Per-app data buckets** (quota, isolation) | RaeenOS-native | raefs data buckets | 🟡 |
| Trash / recycle bin (size, auto-empty) | | raefs | ⬜ |
| Drive optimization / TRIM | | nvme, raefs | ⬜ |
| Network drives / file shares | | raenet | ⬜ |
| Versioned config history (the registry itself) | RaeenOS-native | config_registry snapshots | ✅ |
...
| Setting | Notes | Backing | Status |
|---|---|---|---|
| **Developer mode** (sideload unsigned, verbose logs) | | rae_manifest, RaeShield | 🟡 |
| Unverified-developer sideload UX (warn, not block) | RaeenOS-native | rae_manifest | 🟡 |
| Terminal / shell defaults (RaeShell) | | raeshell terminal | 🟡 |
...
| Window manager API (tile/stack/float/hybrid swap) | RaeenOS-native | compositor | ⬜ |
| Swappable shell (replace RaeShell) | RaeenOS-native | raeshell | ⬜ |
...
- **Covered beyond the big-3:** Gaming (§10), RGB/Vibe (§6), per-app sandbox + two-tier
  ...
  versioned settings with rollback (§0) — these are RaeenOS-native and have **no direct
...
- **macOS-specific we map:** Night Shift → Night Light §2; True Tone/Display calibration
  ...
  Focus → §16; Continuity/Handoff → RaeSync §4/§8.
- **Linux/GNOME-KDE we map:** Online Accounts → RaeID §8; Color profiles → §2; Power
  profiles → §14; Wayland/X knobs → N/A (RaeGFX/compositor own this).
...
2. **Shell of the app** (RaeUI): left nav = these categories, search bar over the schema,
   per-panel capability gating via RaeShield.
