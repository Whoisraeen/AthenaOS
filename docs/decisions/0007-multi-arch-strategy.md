...
- Status: accepted
- Date: 2026-06-21
- Owner: athena-lead (autonomous); spec by athena-researcher
...
athena-researcher's spec (docs/research/multi-arch-abstraction.md) found: NO `arch::`
...
2. **Introduce a monomorphized `arch::` abstraction** (free-fns/associated-types, NOT `dyn` —
   ...
   stay shared. The user/syscall ABI stays arch-neutral (NO `ath_abi`/ABI_VERSION change).
...
4. **Sequencing — groundwork now, refactor deliberately:** the heavy Slice 0 kernel refactor
   ...
   Slice 0a (athena-architect: `arch/mod.rs` `#[cfg]` re-export skeleton + seam contract +
   `BootContext` replacing `bootloader_api::BootInfo` + R10 smoketest slot) → Slice 0b
   (athena-kernel: relocate seam files, no logic change) → Slice 1 (arch-neutral MM newtypes,
