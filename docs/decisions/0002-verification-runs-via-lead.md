...
## Context
In this unattended session, subagent Bash execution is auto-DENIED (sandbox). raeen-verifier
...
log path to raeen-verifier, which parses it (Read/Grep) and renders the hard PASS/FAIL verdict and
...
## How to reverse
Re-enable subagent Bash permissions; then raeen-verifier runs the full cycle itself per ADR 0001.
