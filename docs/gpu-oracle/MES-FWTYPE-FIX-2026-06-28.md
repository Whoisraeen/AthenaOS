...
ftype=33  sz=127040   MES pipe0 ucode   <-- RaeenOS sent 76 (RS64_MES) -> REJECTED
ftype=34  sz=131072   MES pipe0 data    <-- RaeenOS sent 77 -> REJECTED
ftype=81  sz=104016   MES KIQ ucode     <-- RaeenOS sent 78 -> REJECTED
ftype=82  sz=131072   MES KIQ data      <-- RaeenOS sent 79 -> REJECTED
...
RaeenOS used the `RS64_MES`/`RS64_KIQ` enum values 76/77/78/79 (correct for *some* gfx11
...
3. (Optional follow-up) re-add SDMA 71/72 — amdgpu loads them; RaeenOS excluded them after
...
running INSTR_PNTR. **The MES microengine is now executing its scheduler loop on RaeenOS.**
0 panics.
...
```
RaeenOS emitted op **0x26** for MAP_QUEUES — the wrong PACKET3 opcode (commit dde2ff0 fixes it to
0xA2). With it, RaeenOS's header becomes 0xc005a200 and its sel word 0x34080000 — BYTE-IDENTICAL to
...
(bit31), RaeenOS pushed a now-byte-correct MAP_QUEUES, the KIQ wptr-poll addr (q.fence+1040) matches
...
Dump the WORKING MES-KIQ HQD registers (me=3, pipe=1) and diff against what RaeenOS programs in
...
and diffed RaeenOS's MES bring-up function-by-function:
- `mes_v11_0_queue_init_register` (the KIQ HQD write sequence): RaeenOS's `build_mes_queue_init_register`
...
- `mes_v11_0_mqd_init` (the MQD VALUES): RaeenOS's `build_mes_mqd` matches — header 0xC0310800,
...
- `_DEFAULT` base constants verified: CP_HQD_PQ_CONTROL_DEFAULT=0x00308509 (contributes the working
  ...
  RaeenOS uses a 64KB SCHED ring -> 13, Linux 32KB -> 12; benign).
...
runtime call/order RaeenOS omits — the static source can't show what the static source doesn't list.
...
```
Confirms: (a) structure == RaeenOS's order (enable -> KIQ mqd -> [KIQ maps SCHED] -> hw_init does
...
Gaps spotted (downstream of the gate, but real): RaeenOS sends ONE set_hw_resources; amdgpu sends
...
to capture the EXACT value it rings the KIQ doorbell with (RaeenOS rings dword-count=7 via
...
that RaeenOS omits (no HDP/cache flush exists anywhere in raeen_amdgpu; check if the daemon's KIQ-ring
...
- Doorbell encoding: RULED OUT. Linux `mes_v11_0_ring_set_wptr` does `WDOORBELL64(doorbell_index,
  ring->wptr)` — a 64-bit write of the dword wptr. RaeenOS's daemon `ring_doorbell` (amdgpud/src/main.rs:508)
...
**EXHAUSTIVE verdict: every static + structural thing RaeenOS does matches amdgpu** (fw types, MAP_QUEUES
...
introspection, not more source diffing):
  1. **MES pipe1 (KIQ) not in its service loop.** Heartbeat kiq(pipe1)=0x6038109 proves pipe1 booted
     ...
     NEXT: read pipe1's INSTR_PNTR (GRBM-select me=3/pipe=1) on RaeenOS — running PC vs stuck — and
  ...
  3. The mes_v11_0_enable 504us **MES-ready WAIT** — does RaeenOS wait for the MES to report ready
...
(b) Read the WORKING KIQ's INSTR_PNTR to see if 0x15be is ALSO where amdgpu's idle KIQ parks (then it's
    fine and the gate is doorbell-wake) vs deeper (then RaeenOS's KIQ is genuinely stalled). Blocked by
...
umr-read CP_MES_INSTR_PNTR for pipe1: it reads **0x15be** — the SAME PC as RaeenOS's KIQ. (Reads are
flaky: GFXOFF still intermittently gates even with the param, so most samples are 0, but 0x15be came
through on pipe1.) So **0x15be is the KIQ's NORMAL idle wait-loop PC**, not a stall. RaeenOS's KIQ is
...
KIQ never acts on it. NEXT (concrete):
  1. After ringing the KIQ doorbell on RaeenOS, read CP_HQD_PQ_WPTR (me=3/pipe=1) — did the doorbell
  ...
  2. Check CP_PQ_WPTR_POLL_CNTL.EN (or the per-queue wptr-poll enable): the MES KIQ likely POLLS its
     ...
     notes the wptr_poll_addr is "only used if CP_PQ_WPTR_POLL_CNTL.EN=1". If RaeenOS never enables
...
NEVER processed RaeenOS's KIQ ring — not the MAP_QUEUES, not the WRITE_DATA. This DEFINITIVELY rules
...
**Prime suspect: VFIO/KVM passthrough.** The whole debug loop runs RaeenOS in a KVM-VFIO VM on the
...
**THE decisive next test: a `--safe` bare-metal flash of RaeenOS on the Athena (boot natively, not in
...
closer than the VM suggests. If it stays 0xcafedead on bare metal too, it's a real RaeenOS bug and the
...
- GRBM_GFX_CNTL = dword 0xa900 (0xc=me3/pipe0, 0xd=me3/pipe1, 0=restore). CP_MES PC = dword 0xc800.
  ...
  0x28xx (a different block, written me=0) — RaeenOS matches.
- KIQ HQD setup (GRBM=0xd, ts 43.9075): CP_HQD writes at d=0x320b..0x321a — DOORBELL_CONTROL(0x3218)=
  0x40000060, PQ_CONTROL(0x321a)=0xd8308011 (QUEUE_SIZE=0x11=1MB ring! RaeenOS uses 4KB=9),
  PERSISTENT_STATE(0x320d)=0xbe05501, ACTIVE(0x320b)=1, MQD_BASE(0x3209/a)=0x80_7fca3000. Mostly
  matches RaeenOS; the 1MB-vs-4KB KIQ ring size is the one notable diff.
- DOORBELL VALUES (doorbell BAR 0xdc000000): KIQ doorbell @byte 0x60 rung with **0x100** (256), then
  ...
  and rings the PADDED wptr. RaeenOS rang the raw 7-dword count.
...
FIX TRIED (commit 20f551a): pad kiq_map to 256 dwords (type-2 NOPs) so RaeenOS rings the KIQ doorbell
...
interaction the MES needs; (2) the 1MB KIQ ring (QUEUE_SIZE 0x11) vs RaeenOS's 4KB — try matching it;
...
Found a real gap: RaeenOS set only the gfx CP_RB_DOORBELL_RANGE [0x458,0x7f8]; it had NO
...
base 0x1260) — covers MES SCHED (0x58) + KIQ (0x60). RaeenOS omitted it entirely. Added it (commit
...
covers it — yet pipe1 never wakes to drain its ring. Remaining wake candidates (NOT yet tried):
  1. IH (interrupt handler) ring routing — if the MES wakes via a doorbell INTERRUPT through the IH
     ring, and RaeenOS's IH ring isn't routing the MES/CP doorbell interrupt, pipe1 never gets the
     wake event. (RaeenOS programs the IH ring base/rptr/wptr early — verify the doorbell-monitor IRQ
     source is enabled + routed to the MES.)
  2. A per-pipe doorbell/EOP-interrupt enable on the KIQ HQD that RaeenOS's mqd/queue_init omits.
  3. set_hw_resources dependency: maybe the MES only arms its doorbell monitor AFTER set_hw_resources
     ...
     direct CP_HQD queue serviced by CP/MEC hardware; re-examine whether RaeenOS's KIQ is actually
...
Hammered umr on the working amdgpu (gfxoff=0) and checked RaeenOS for each:
