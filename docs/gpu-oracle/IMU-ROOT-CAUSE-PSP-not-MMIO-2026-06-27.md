...
|---|---|---|
| `GFX_IMU_CORE_STATUS` | **0x03** | CBUSY=1 RUNNING (RaeenOS gets 0x02 = PWAIT, not running) |
...
- **Total MMIO writes during a full cold amdgpu init = 6858.** The IMU ucode alone is
  132352 B = ~33k dwords. If amdgpu streamed it via `GFX_IMU_I_RAM_DATA` (as RaeenOS
...
- The PSP mailbox (MP0 `C2PMSG`) **is** in the trace at dword `0x16080+` (byte `0x58200`,
  matching RaeenOS's `cmd@0x58200`). So the firmware load is the PSP doing it.

## Conclusion → the real gate
The PSP **loads and starts** the IMU (and RLC/CP/MES/SDMA) via autoload. amdgpu never
touches GFX_IMU over MMIO. **RaeenOS's `bringup.rs` IMU MMIO direct-load (I-RAM/D-RAM
streaming + CORE_CTRL/RESET pokes) is a dead-end workaround that fights the hardware.**
The IMU sits at CORE_STATUS=0x02 (PWAIT) because RaeenOS's **PSP autoload is broken** —
...
Diff **RaeenOS's PSP `LOAD_IP_FW` sequence vs amdgpu's** — both are MMIO-visible in this
...
3. Fix RaeenOS's PSP autoload (`amdgpud`/`bringup.rs` PSP path) to match — the 0xffff0006
...
Code-level diff of RaeenOS's PSP path against amdgpu + the trace, narrowed to the exact bug:

- **The PSP ring wptr stride is CORRECT** — RaeenOS `PSP_RB_FRAME_DWORDS=16` (`0x10`),
...
- **Therefore the rejection is a PSP firmware-AUTHENTICATION failure**: IMU/RLC/CP blobs
  authenticate (status 0), MES/SDMA blobs do NOT. The bytes RaeenOS hands `LOAD_IP_FW`
  for MES (`extract_mes_ucode_data`) and SDMA are the wrong signed region — the PSP
  signs/validates a specific span and RaeenOS extracts a different one.
...
- **Staging address** — where RaeenOS stages each blob before `LOAD_IP_FW` (`fw_pri` MC
