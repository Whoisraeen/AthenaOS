...
pub enum PasswordAlgorithm {
    /// Real memory-hard Argon2id (RFC 9106) via `ath_crypto`. The default and
...
/// RFC 9106 memory-hard KDF in `ath_crypto`); the legacy path exists only so
...
impl PasskeyCeremony {
    ...
    pub fn verify_auth_response(
        ...
        // rejected. `ath_crypto::p256_ecdsa::verify` hashes with SHA-256
        ...
        if !ath_crypto::p256_ecdsa::verify(&stored.public_key, &signed, &response.signature) {
            return Err(AuthError::InvalidSignature);
        }
...
mod password_tests {
    ...
    fn manager_credential_get_set_roundtrip() {
        ...
        let uid = mgr
            ...
            .id;
        mgr.add_password(uid, b"correct-horse", *b"athid-test-salt!", 0)
            .unwrap();
        let cred = mgr
    ...
    fn change_password_honors_brute_force_lockout() {
        ...
        let uid = mgr
            ...
            .id;
        mgr.add_password(uid, b"orig-pass", *b"athid-test-salt!", 0)
            .unwrap();
        let now = 2000u64;
        ...
        for _ in 0..MAX_FAILED_ATTEMPTS {
            assert_eq!(
                mgr.change_password(uid, b"wrong", b"new-pass", *b"athid-test-salt2", now),
                Err(AuthError::InvalidCredentials)
            );
        }
        // 2. The account is now locked; even the CORRECT old password is refused.
        assert_eq!(
            mgr.change_password(uid, b"orig-pass", b"new-pass", *b"athid-test-salt2", now),
        ...
        let later = now + LOCKOUT_DURATION_SECS + 1;
        assert!(mgr
            .change_password(uid, b"orig-pass", b"new-pass", *b"athid-test-salt2", later)
            .is_ok());
    }
