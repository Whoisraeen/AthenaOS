...
impl HttpsClient {
    ...
    pub fn request(&mut self, req: HttpsRequest) -> Result<HttpsResponse, HttpsError> {
        ...
        // [`tls13::TlsByteTransport`] backed by the athkit socket syscalls and
...
// dependency. A userspace daemon passes CSPRNG bytes (getrandom/athkit) -- that is
...
pub mod tls13 {
    ...
    /// athkit socket syscalls; the host KATs implement it with a deterministic
    ...
    // -----------------------------------------------------------------------
    // Host KATs (cargo test -p athnet --features tls13): verified fetch +
