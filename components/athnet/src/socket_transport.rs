...
//!
//! Why a backend trait instead of calling `athkit::sys` directly:
//! * athnet links into the **kernel** (no_std) and into the **host test**
//!   harness (std). athkit installs a `#[global_allocator]` + `#[panic_handler]`
//!   for a userspace ELF and emits the raw `syscall` instruction — neither is
//!   valid in those two link contexts. Pulling athkit in here would break both.
...
//!   real impl in three lines (forwarding to `athkit::sys::{tcp_connect,
...
//!
//! The result: `athnet::http_get(url, &backend)` performs a real `http://`
...
/// caller implements this by forwarding to the athkit wrappers; the host KATs
/// implement it with an in-memory mock.
///
/// ```ignore
/// // In a userspace app/daemon that links athkit:
/// struct RaekitSyscalls;
/// impl athnet::socket_transport::SocketSyscalls for RaekitSyscalls {
///     fn dns_resolve(&self, host: &str) -> Option<[u8; 4]> {
///         athkit::sys::dns_resolve(host)
///     }
///     fn tcp_connect(&self, ip: [u8; 4], port: u16) -> Option<u64> {
///         athkit::sys::tcp_connect(ip, port)
///     }
///     fn send(&self, fd: u64, buf: &[u8]) -> isize { athkit::sys::sock_send(fd, buf) }
///     fn recv(&self, fd: u64, buf: &mut [u8]) -> isize { athkit::sys::sock_recv(fd, buf) }
///     fn close(&self, fd: u64) { athkit::sys::sock_close(fd) }
/// }
/// ```
