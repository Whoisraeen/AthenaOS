...
/// `apps/music` player streams the result through `athkit::audio_submit`
...
/// R10 artifact — the athmedia MP3 status line for `/proc/athena/media` (or wherever the
...
pub fn run_boot_smoketest() -> alloc::string::String {
    ...
    format!(
        "[athmedia] mp3 synth: frames=1 peak={:.4} nonsilent={} zero_silent={} -> {}",
        peak,
        nonsilent,
        zero_silent,
...
mod tests {
    ...
    fn mp3_boot_smoketest_passes() {
        ...
        assert!(line.starts_with("[athmedia] mp3 synth:"));
    ...
    fn aac_boot_smoketest_passes() {
        ...
        assert!(line.starts_with("[athmedia] aac-lc:"));
