...
//! (so the exact same proven path runs in-kernel, exercising `ath_crypto` under
...
pub fn loopback(host_seed: &[u8; 32], host_pub: &[u8; 32]) -> Result<(), &'static str> {
    ...
    let probe = keys.cipher_s2c.seal(0, b"athssh-loopback-probe", 0);
    if client_s2c.open(0, &probe).ok().as_deref() != Some(&b"athssh-loopback-probe"[..]) {
    ...
    let areq = build_publickey_request("athena", SERVICE_CONNECTION, &auth_pub, Some(&asig));
    let step = srv
    ...
    if step.event
        != (SessionEvent::Authenticated {
            user: alloc::string::String::from("athena"),
        })
    {
