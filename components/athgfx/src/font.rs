...
//! `athgfx/src/lib.rs` (orphan — never compiled into the crate). The WIRED font
//! engine is the standalone `components/athfont` crate (more complete: hinting
//! interpreter, COLR/CPAL, shaper, the crisp filled rasterizer + `builtin`
//! embedded faces). The one text API is `athgfx::Canvas::draw_text_aa`
//! (`athgfx/src/text.rs`), which composites athfont coverage source-over.
//! See `docs/QUARANTINED_MODULES.md` and `docs/design/typography-rendering.md`
//! §3.2. New text work extends athfont, never this file.
//!
//! ---
