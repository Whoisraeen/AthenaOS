...
//! bindings (wiring [`raeweb`]'s DOM to the interpreter) and the event loop come after
...
//! with `cargo test -p rae_js` — no QEMU, no image build.
//!
//! ```
//! let program = rae_js::parse("var x = 1 + 2 * 3;").expect("valid");
