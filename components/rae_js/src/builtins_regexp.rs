...
//! never-panic, ReDoS-safe [`rae_regex`] engine (Thompson NFA / Pike VM) — there is no
...
//!   RegExp wrapping a compiled [`rae_regex::Regex`] (compiled at construction; a bad
...
//!
//! ## Flag mapping — honest scope (rae_regex's flavor vs JS's)
//! rae_regex is a deliberately-bounded subset (see its crate docs): a Thompson NFA with no
...
//! - `i` (ignoreCase): rae_regex has **no** case-folding mode, so we implement `i`
...
//!   all-matches path (`rae_regex::Regex::find_all`) and advance `lastIndex`.
//! - `s` (dotAll): rae_regex's `.` already excludes only `\n`; full dotAll is **not**
//!   distinguished (best-effort) — recorded on the object but does not change matching.
//! - `m` (multiline): rae_regex `^`/`$` are start/end-of-text only (no per-line) — recorded
...
//!   the `$&`/`$1`..`$9`/`$$` substitution subset that [`rae_regex::Regex::replace_all`]
...
//! [`rae_regex::Regex::new`]'s `Err`, never `unwrap`). A regex method on a non-string /
//! non-regex receiver → `TypeError`. Pathological input is bounded by rae_regex's own
//! linear-time guarantee plus the interpreter's step budget. No script — valid or hostile —
//! panics the host. Run the FAIL-able KATs with `cargo test -p rae_js`.
...
fn compile(source: &str, flags: &str) -> Result<RegExpData, RuntimeError> {
    ...
    // JS: an empty literal `//` has source `(?:)`. rae_regex accepts an empty pattern.
    let effective = if i {
...
/// Escape a plain string so it matches literally as a regex (every rae_regex metacharacter
...
/// rae_regex reports byte offsets; the rest of the interpreter indexes strings by `char`
...
pub(crate) fn string_replace(
    ...
    let matches: Vec<rae_regex::Match> = if global {
        rd.re.find_all(text)
    } else {
...
// ─── Host KATs (cargo test -p rae_js) — FAIL-able by construction ────────────────
