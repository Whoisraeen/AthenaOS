//! Host KATs for the rae_js tree-walking interpreter. FAIL-able by construction: each
//! asserts a concrete [`JsValue`] / console output, so a semantics regression turns a
//! green run red. Run with `cargo test -p rae_js`. (`#![cfg_attr(not(test), no_std)]`
...
fn uri_encoding() {
    ...
    // a malformed % sequence throws (JS would raise URIError; rae_js uses Error).
