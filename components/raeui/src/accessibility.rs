...
/// raegfx canvas. This is the single focus-ring renderer every shell chrome
...
/// [`rae_tokens::active_focus_ring`] so when high-contrast forced-colors mode is
...
pub fn draw_focus_ring(
    ...
    let ring = rae_tokens::active_focus_ring(normal_ring);
...
// The numeric tags are inlined (not imported) so raeui keeps its lean dep set;
// `provider_role_code_kat` asserts they stay in lockstep with the ABI.

// rae_abi A11Y_ROLE_* values (mirrored — kept in lockstep by the host KAT).
...
/// The numeric ABI role code for a role (maps to `rae_abi::A11Y_ROLE_*`).
...
/// accessibility tree. Mirrors the `rae_abi::A11yNode` field set (minus the
...
mod tests {
    ...
    fn role_codes_match_documented_abi() {
        // Lockstep guard against the documented rae_abi A11Y_ROLE_* / _STATE_* /
        // _ACTIONBIT_* values (raeui doesn't link rae_abi; the kernel a11y.rs
