...
pub enum AmlValue {
    ...
    /// AMI firmware uses this for SuperIO/EC index-data register files.
    IndexField {
        index: AmlHandle,
        data: AmlHandle,
        flags: FieldFlags,
...
impl AmlValue {
    ...
    pub fn type_of(&self) -> AmlType {
        match self {
            ...
            AmlValue::BankField { .. } => AmlType::FieldUnit,
    ...
    pub fn as_integer(&self, context: &AmlContext) -> Result<u64, AmlError> {
        match self {
             ...
             */
            AmlValue::Field { .. } | AmlValue::IndexField { .. } | AmlValue::BankField { .. } => self.read_field(context)?.as_integer(context),
    ...
    pub fn as_buffer(&self, context: &AmlContext) -> Result<Arc<Spinlock<Vec<u8>>>, AmlError> {
        match self {
            ...
            AmlValue::Field { .. } | AmlValue::IndexField { .. } | AmlValue::BankField { .. } => self.read_field(context)?.as_buffer(context),
    ...
    pub fn as_string(&self, context: &AmlContext) -> Result<String, AmlError> {
        match self {
            ...
            AmlValue::Field { .. } | AmlValue::IndexField { .. } | AmlValue::BankField { .. } => self.read_field(context)?.as_string(context),
    ...
    pub fn read_field(&self, context: &AmlContext) -> Result<AmlValue, AmlError> {
        match self {
            ...
            AmlValue::IndexField { index, data, flags: _, offset, length } => {
                let bit_shift = (*offset % 8) as usize;
                let byte_count = ((bit_shift as u64 + *length + 7) / 8).min(8);
                let base_byte = *offset / 8;
                let mut raw: u64 = 0;
                for i in 0..byte_count {
                    write_field_unit_shared(context, *index, base_byte + i)?;
                    let byte = context.namespace.get(*data)?.read_field(context)?.as_integer(context)?;
                    raw |= (byte & 0xFF) << (8 * i);
                }
            ...
            AmlValue::BankField { region, bank, bank_value, flags, offset, length } => {
                write_field_unit_shared(context, *bank, *bank_value)?;
                read_region_field(context, *region, *flags, *offset, *length)
            }
            _ => Err(AmlError::IncompatibleValueConversion { current: self.type_of(), target: AmlType::FieldUnit }),
        }
    }
    ...
    pub fn write_field(&mut self, value: AmlValue, context: &mut AmlContext) -> Result<(), AmlError> {
        match self {
            ...
            AmlValue::IndexField { index, data, flags: _, offset, length } => {
                let (index, data, offset, length) = (*index, *data, *offset, *length);
                let raw = value.as_integer(context)?;
                if offset % 8 != 0 {
                    // A sub-byte-aligned IndexField write needs a read-modify-
                    // write of the surrounding bytes; real firmware aligns its
                    // index/data units, so reject honestly instead of
                    // corrupting neighbouring bits.
                    return Err(AmlError::FieldRegionAccessUnsupported);
                }
                let byte_count = ((length + 7) / 8).min(8);
                let base_byte = offset / 8;
                for i in 0..byte_count {
                    write_field_unit_shared(context, index, base_byte + i)?;
                    let byte = (raw >> (8 * i)) & 0xFF;
                    let mut data_field = context.namespace.get(data)?.clone();
                    data_field.write_field(AmlValue::Integer(byte), context)?;
                }
                Ok(())
            }
            ...
            AmlValue::BankField { region, bank, bank_value, flags, offset, length } => {
                let (region, bank, bank_value, flags, offset, length) =
                    (*region, *bank, *bank_value, *flags, *offset, *length);
                write_field_unit_shared(context, bank, bank_value)?;
                write_region_field(context, region, flags, offset, length, value)
            }
            _ => Err(AmlError::IncompatibleValueConversion { current: self.type_of(), target: AmlType::FieldUnit }),
...
fn validate_field_region(context: &AmlContext, region: AmlHandle) -> Result<(), AmlError> {
    if let AmlValue::OpRegion { region, .. } = context.namespace.get(region)? {
        match region {
            RegionSpace::SystemMemory | RegionSpace::SystemIo | RegionSpace::PciConfig => Ok(()),
            _ => Err(AmlError::FieldRegionAccessUnsupported),
        }
    } else {
...
fn read_region_field(
    ...
    validate_field_region(context, region)?;
    /*
     ...
     */
    let access_size = u64::max(field_min_access_size(&flags)?, length.next_power_of_two());
...
fn write_region_field(
    ...
    let mut field_value = match flags.field_update_rule()? {
        FieldUpdateRule::Preserve => {
            read_region_field(context, region, flags, offset, length)?.as_integer(context)?
        }
...
/// selector fields are writable here (which is what real firmware uses).
fn write_field_unit_shared(context: &AmlContext, handle: AmlHandle, value: u64) -> Result<(), AmlError> {
    let (region, flags, offset, length) = match context.namespace.get(handle)? {
        AmlValue::Field { region, flags, offset, length } => (*region, *flags, *offset, *length),
        other => {
    ...
    let access_size = u64::max(field_min_access_size(&flags)?, length.next_power_of_two());
    // Selector fields normally span their whole register; preserve the
    // neighbouring bits with a read-modify-write when they don't.
    let mut field_value =
        if length == access_size { 0 } else { context.read_region(region, offset, access_size)? };
    field_value.set_bits(0..(length as usize), value);
    context.write_region_shared(region, offset, access_size, field_value)
}
