    ...
    'c: 'a,
{
    ...
    let reserved_field =
        opcode(opcode::RESERVED_FIELD).then(crate::pkg_length::raw_pkg_length()).map(|((), length)| Ok(length as u64));
    ...
    let named_field = name_seg().then(crate::pkg_length::raw_pkg_length()).map_with_context(move |(name_seg, length), context| {
        try_with_context!(
            context,
            context.namespace.add_value_at_resolved_path(
    ...
    'c: 'a,
{
     ...
     * (first NameString = the index field-unit, second = the data field-unit)
     */
    let index_and_data = name_string().then(name_string()).map_with_context(|(index_name, data_name), context| {
        let (_, index) =
            try_with_context!(context, context.namespace.search(&index_name, &context.current_scope));
        let (_, data) = try_with_context!(context, context.namespace.search(&data_name, &context.current_scope));
        (Ok((index, data)), context)
    });
    ...
    ext_opcode(opcode::EXT_DEF_INDEX_FIELD_OP)
        .then(comment_scope(
            DebugVerbosity::Scopes,
            "DefIndexField",
            pkg_length().then(index_and_data).then(take()).feed(|((list_length, (index, data)), flags)| {
                move |mut input: &'a [u8], mut context: &'c mut AmlContext| -> ParseResult<'a, 'c, ()> {
                    /*
                     * FieldList := Nothing | <FieldElement FieldList>
                     */
                    ...
                    while list_length.still_parsing(input) {
                        let (new_input, new_context, field_length) =
                            index_field_element(index, data, FieldFlags::new(flags), current_offset)
                                .parse(input, context)?;
                        input = new_input;
                        context = new_context;
                        current_offset += field_length;
    ...
    'c: 'a,
{
     ...
     * (first NameString = the operation region, second = the bank-selector
     * field-unit; BankValue := TermArg => Integer)
     */
    let region_and_bank = name_string().then(name_string()).map_with_context(|(region_name, bank_name), context| {
        let (_, region) =
            try_with_context!(context, context.namespace.search(&region_name, &context.current_scope));
        let (_, bank) = try_with_context!(context, context.namespace.search(&bank_name, &context.current_scope));
        (Ok((region, bank)), context)
    });
    ...
    ext_opcode(opcode::EXT_DEF_BANK_FIELD_OP)
        .then(comment_scope(
            ...
            "DefBankField",
            pkg_length()
                .then(region_and_bank)
                .then(term_arg())
                .map_with_context(|((list_length, (region, bank)), bank_value), context| {
                    let bank_value = try_with_context!(context, bank_value.as_integer(context));
                    (Ok((list_length, region, bank, bank_value)), context)
                })
                .then(take())
                .feed(|((list_length, region, bank, bank_value), flags)| {
                    move |mut input: &'a [u8], mut context: &'c mut AmlContext| -> ParseResult<'a, 'c, ()> {
                        /*
                         * FieldList := Nothing | <FieldElement FieldList>
                         */
