...
impl AmlContext {
    ...
    /// IO and PCI-config write accessors take `&self`, so those spaces are
    /// writable here; SystemMemory (whose handler writes are `&mut self`) is
    /// rejected with [`AmlError::FieldRegionAccessUnsupported`] — real
    /// firmware's selector registers live in IO/PCI-config space.
    pub(crate) fn write_region_shared(
        &self,
        region_handle: AmlHandle,
        offset: u64,
        length: u64,
