    ...
    'c: 'a,
{
    ...
    comment_scope(
        ...
        choice!(
            ...
            binary_integer_op(opcode::DEF_MULTIPLY_OP, "DefMultiply", |l, r| l.wrapping_mul(r)),
            binary_integer_op(opcode::DEF_NAND_OP, "DefNAnd", |l, r| !(l & r)),
            binary_integer_op(opcode::DEF_OR_OP, "DefOr", |l, r| l | r),
            binary_integer_op(opcode::DEF_NOR_OP, "DefNOr", |l, r| !(l | r)),
            binary_integer_op(opcode::DEF_XOR_OP, "DefXOr", |l, r| l ^ r),
            // Mod-by-zero is a fatal error per spec §19.6.83; evaluating to 0
            // keeps table load alive (matches our lenient load-time posture).
            binary_integer_op(opcode::DEF_MOD_OP, "DefMod", |l, r| if r == 0 { 0 } else { l % r }),
            def_not(),
            def_size_of(),
            def_deref_of(),
            def_index(),
...
/// resolved for correctness; no ownership state is needed without contention.
    ...
    'c: 'a,
{
    ext_opcode(opcode::EXT_DEF_ACQUIRE_OP)
        .then(comment_scope(
            ...
            "DefAcquire",
            super_name().then(take_u16()).map(|(_mutex, _timeout)| Ok(AmlValue::Integer(0))),
        ))
        .map(|((), result)| Ok(result))
}
    ...
    'c: 'a,
{
    ext_opcode(opcode::EXT_DEF_WAIT_OP)
        .then(comment_scope(
            DebugVerbosity::Scopes,
            "DefWait",
            super_name().then(term_arg()).map(|(_event, _timeout)| {
        ...
        .then(comment_scope(
            ...
            term_arg().then(term_arg()).then(target()).map_with_context(
                move |((left_arg, right_arg), target), context| {
                    let left = try_with_context!(context, left_arg.as_integer(context));
                    let right = try_with_context!(context, right_arg.as_integer(context));
                    let result = AmlValue::Integer(f(left, right));
...
where
    'c: 'a,
{
    opcode(opcode::DEF_NOT_OP)
        .then(comment_scope(
            DebugVerbosity::AllScopes,
            "DefNot",
            term_arg().then(target()).map_with_context(|(operand, target), context| {
                let value = try_with_context!(context, operand.as_integer(context));
        ...
        .then(comment_scope(
            ...
            term_arg().map_with_context(|operand, context| {
                let size = match operand {
                    AmlValue::Buffer(ref bytes) => bytes.lock().len() as u64,
                    AmlValue::String(ref s) => s.len() as u64,
                    AmlValue::Package(ref elements) => elements.len() as u64,
                    ref other => {
                        return (
                            Err(Propagate::Err(AmlError::TypeCannotBeSliced(other.type_of()))),
                            context,
                        )
                    }
                };
...
/// what we produce. This covers the real firmware pattern
/// `DerefOf(Index(DataPackage, n))` (AMD computes region offsets this way);
/// reference-writeback (`Store` through a DerefOf) is not supported.
fn def_deref_of<'a, 'c>() -> impl Parser<'a, 'c, AmlValue>
where
    'c: 'a,
{
    opcode(opcode::DEF_DEREF_OF_OP)
        .then(comment_scope(DebugVerbosity::AllScopes, "DefDerefOf", term_arg()))
        .map(|((), value)| Ok(value))
}
    ...
    'c: 'a,
{
    opcode(opcode::DEF_INDEX_OP)
        .then(comment_scope(
            ...
            "DefIndex",
            term_arg().then(term_arg()).then(target()).map_with_context(
                |((source, index), target), context| {
                    let idx = try_with_context!(context, index.as_integer(context)) as usize;
                    let result = match source {
                        AmlValue::Package(ref elements) => match elements.get(idx) {
                            Some(element) => element.clone(),
                            None => {
                                return (
                                    Err(Propagate::Err(AmlError::IndexOutOfBounds)),
                                    context,
                                )
                            }
                        },
                        AmlValue::Buffer(ref bytes) => {
                            ...
                            match bytes.get(idx) {
                                Some(&byte) => AmlValue::Integer(byte as u64),
                                None => {
                                    return (
                                        Err(Propagate::Err(AmlError::IndexOutOfBounds)),
                                        context,
                        ...
                        AmlValue::String(ref s) => match s.as_bytes().get(idx) {
                            Some(&byte) => AmlValue::Integer(byte as u64),
                            None => {
                                return (
                                    Err(Propagate::Err(AmlError::IndexOutOfBounds)),
