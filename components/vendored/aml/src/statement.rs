    ...
    'c: 'a,
{
    ext_opcode(opcode::EXT_DEF_RELEASE_OP)
        .then(comment_scope(
            ...
            "DefRelease",
            crate::name_object::super_name().map(|_mutex| Ok(())),
        ))
        .discard_result()
}
    ...
    'c: 'a,
{
    ext_opcode(opcode::EXT_DEF_SLEEP_OP)
        .then(comment_scope(
            DebugVerbosity::Scopes,
            "DefSleep",
            term_arg().map_with_context(|ms, context| {
                let ms = try_with_context!(context, ms.as_integer(context));
                context.handler.sleep(ms);
                (Ok(()), context)
            }),
        ))
        .discard_result()
    ...
    'c: 'a,
{
    ext_opcode(opcode::EXT_DEF_STALL_OP)
        .then(comment_scope(
            DebugVerbosity::Scopes,
            "DefStall",
            term_arg().map_with_context(|us, context| {
                let us = try_with_context!(context, us.as_integer(context));
                context.handler.stall(us);
                (Ok(()), context)
            }),
        ))
        .discard_result()
