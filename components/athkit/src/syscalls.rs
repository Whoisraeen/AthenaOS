...
//
// Run with `cargo test -p athkit`. Exercises the FAIL-able wire decoders for
...
mod tests {
    ...
    fn header_size_matches_const() {
        ...
        // the ath_abi struct size the whole format silently corrupts.
    ...
    fn round_trips_two_records() {
        ...
        assert_eq!(hits[0].path, "/home/athena/Documents/resume.txt");

        assert_eq!(hits[1].id, 9);
        assert_eq!(hits[1].kind, Kind::File);
        assert!(hits[1].is_folder);
        assert_eq!(hits[1].name, "Vacation_Photos");
        assert_eq!(hits[1].path, "/home/athena/Pictures/Vacation_Photos");
    }
