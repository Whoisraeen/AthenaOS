...
// the `tools/ui_screenshot` renderer linking `apps/files` -> `athkit`) gets
...
/// Convenience re-exports for app development. `use athkit::prelude::*;`
...
pub mod sys {
    ...
    /// Create a directory (ath_abi SYS_MKDIR). rdi=path_ptr, rsi=path_len, rdx=mode.
    pub const SYS_MKDIR: u64 = 96;
    /// Remove a file/empty dir (ath_abi SYS_UNLINK). rdi=path_ptr, rsi=path_len.
    pub const SYS_UNLINK: u64 = 97;
    /// Move/rename (ath_abi SYS_RENAME). rdi=old_ptr, rsi=old_len, rdx=new_ptr, r10=new_len.
    pub const SYS_RENAME: u64 = 98;
    pub const SYS_CONFIG_GET: u64 = 50;
    pub const SYS_CONFIG_SET: u64 = 51;
    // ── Rae scripting (ath_abi scripting block, SYSCALL_TABLE.md Block 14) ──
    ...
    // ── Local search index (ath_abi search block 54-57, docs/SYSCALL_TABLE.md) ──
    ...
    /// Resolved search query (ath_abi::syscall::SYS_SEARCH_QUERY_RESOLVED, 281) —
    ...
    /// Live theme read (ath_abi::syscall::SYS_THEME_GET). rdi=out ptr, rsi=cap.
    pub const SYS_THEME_GET: u64 = 266;
    /// Feed PCM into the AthAudio mixer (ath_abi::syscall::SYS_AUDIO_SUBMIT).
    /// rdi=samples ptr (*const i16), rsi=frame_count, rdx=format_flags.
    pub const SYS_AUDIO_SUBMIT: u64 = 267;

    // ── Clipboard history (ath_abi::syscall::SYS_CLIP_HIST_*) ──
    ...
    // ── Screen capture (ath_abi::syscall::SYS_CAPTURE_*, 274-276) ──
    ...
    /// Screen-capture failure sentinel (mirrors `ath_abi::syscall::CAPTURE_ERR`).
    pub const CAPTURE_ERR: u64 = u64::MAX;

    // ── Accessibility tree (ath_abi::syscall::SYS_A11Y_*, 277-278) ──
    ...
    /// Accessibility-syscall failure sentinel (mirrors `ath_abi::syscall::A11Y_ERR`).
    pub const A11Y_ERR: u64 = u64::MAX;
    /// `A11yNodeInfo::role` tags (mirror `ath_abi::syscall::A11Y_ROLE_*`).
    pub const A11Y_ROLE_DESKTOP: u32 = 1;
    pub const A11Y_ROLE_WINDOW: u32 = 2;
    pub const A11Y_ROLE_BUTTON: u32 = 3;
    /// `A11yNodeInfo::state` bits (mirror `ath_abi::syscall::A11Y_STATE_*`).
    pub const A11Y_STATE_FOCUSED: u32 = 1 << 0;
    pub const A11Y_STATE_VISIBLE: u32 = 1 << 1;
    pub const A11Y_STATE_MINIMIZED: u32 = 1 << 6;
    /// `A11yNodeInfo::actions` bits (mirror `ath_abi::syscall::A11Y_ACTIONBIT_*`).
    pub const A11Y_ACTIONBIT_FOCUS: u32 = 1 << 0;
    pub const A11Y_ACTIONBIT_ACTIVATE: u32 = 1 << 1;
    /// `SYS_A11Y_ACTION` action selectors (mirror `ath_abi::syscall::A11Y_ACTION_*`).
    ...
    /// Pixel-format tags (mirror `ath_abi::syscall::CAPTURE_FMT_*`).
    pub const CAPTURE_FMT_ARGB32: u32 = 0;
    pub const CAPTURE_FMT_BGRA32: u32 = 1;
    /// `SYS_CAPTURE_START` flags (mirror `ath_abi::syscall::CAPTURE_FLAG_*`).
    pub const CAPTURE_FLAG_CONTINUOUS: u64 = 1 << 0;

    // ── Absolute cursor position (ath_abi::syscall::SYS_INPUT_CURSOR, 279) ──
    ...
    // ── Live surface origin (ath_abi::syscall::SYS_SURFACE_ORIGIN, 280) ──
    ...
    /// `ath_abi::syscall::SURFACE_ORIGIN_ERR`).
    pub const SURFACE_ORIGIN_ERR: u64 = u64::MAX;

    /// Clipboard-history failure sentinel (mirrors `ath_abi::syscall::CLIP_ERR`).
    pub const CLIP_ERR: u64 = u64::MAX;
    /// `ClipEntry::format` tags (mirror `ath_abi::syscall::CLIP_FMT_*`).
    ...
    /// Mirrors `ath_abi::THEME_DEFAULT_ACCENT` / `ath_tokens::RAEBLUE` (athkit
    /// deliberately does not depend on ath_abi to keep the app crate graph thin).
    pub const THEME_DEFAULT_ACCENT: u32 = 0xFF_4E_9C_FF;

    /// Live desktop theme, mirroring `ath_abi::ThemeInfo` (32 bytes, `#[repr(C)]`,
    ...
    /// This is the seed an app feeds into `ath_tokens::derive_accent`.
    ...
    /// One history entry returned by [`clip_hist_get`]: the `ath_abi::ClipEntryHeader`
    ...
    pub fn clip_hist_get(index: u32) -> Option<ClipEntry> {
        const HDR: usize = 32; // size_of::<ath_abi::ClipEntryHeader>()
    ...
    pub fn capture_read(id: u64) -> Option<CapturedImage> {
        const HDR: usize = 16; // size_of::<ath_abi::CaptureHeader>()
    ...
    /// One accessibility node, mirroring `ath_abi::A11yNode` (96 bytes). The
    ...
    pub fn a11y_snapshot() -> Option<alloc::vec::Vec<A11yNodeInfo>> {
        const HDR: usize = 16; // size_of::<ath_abi::A11ySnapshotHeader>()
        const STRIDE: usize = 96; // size_of::<ath_abi::A11yNode>()
    ...
    // ── Userspace TCP/UDP sockets + DNS (ath_abi net block 121-125 + 264) ─────
    // Thin, never-panicking wrappers over the EXISTING socket syscalls. These
    // are the live-network seam the HTTP client (`athnet::http1`) rides on, the
    ...
    /// when this returns; larger ones run in `athlangd` — poll
...
mod hello_athkit_app_tests {
    ...
    //! live shell the handler fires a syscall (`athkit::syscalls`); host-side those
        ...
        fn on_event(&mut self, event: &AppEvent) -> Option<ViewNode> {
            match event {
                AppEvent::Action { id } if *id == SAVE_ACTION => {
                    // The click handler. Live: fires a syscall via athkit::syscalls.
                    self.clicks += 1;
                    Some(self.on_launch())
                }
                _ => None,
