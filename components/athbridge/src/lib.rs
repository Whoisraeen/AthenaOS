...
//!
//! See `docs/components/athbridge.md` for the design.
...
// object's semantics broke (`docs/components/athbridge-wine-strategy.md` §6.1).
//
...
// cross-*process* namespace are the `athbridge_server` broker's job (slice 2)
...
pub use pe_loader::{athena_error_to_win32, AthenaOsError};

/// Convenience: set `last_error` on a session from a AthenaOS error.
pub fn set_last_error_from_athena(ctx: &mut FullCompatSession, err: &pe_loader::AthenaOsError) {
    ctx.last_error = pe_loader::athena_error_to_win32(err);
}

/// Boot smoketest for the synchronization-object model (the in-process half of
/// the `athbridge_server` broker — `docs/components/athbridge-wine-strategy.md`
...
/// `docs/components/athbridge-wine-strategy.md` §7). Builds a throwaway session
...
/// One-line snapshot of the registry-thunk layer for `/proc/athena/*`.
...
/// One-line snapshot of the synchronization-object model for `/proc/athena/*`.
    ...
    let _ = core::fmt::Write::write_fmt(
        ...
        format_args!(
            "AthBridge sync objects (broker §6.1, in-process)\nself_test: {}\n\
             ...
             note: cross-process + true blocking = athbridge_server daemon (slice 2)\n",
            if ok { "PASS" } else { "FAIL" }
        ),
    );
    s
