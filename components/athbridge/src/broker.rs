//! `athbridge_server` — cross-process synchronization-object namespace (broker §6.1).
//!
...
//! `docs/components/athbridge-server-design.md`.
//!
...
/// `athbridge_server` daemon; every AthBridge process talks to it on
/// open/create only.
...
/// One-line `/proc/athena/*` snapshot of the broker namespace (Slice 1).
...
// docs/components/athbridge-process-model.md §"Cross-process sync broker"
...
pub struct SharedSyncState {
    ...
    /// fsync-parity contract (docs/.../athbridge-process-model.md Invariant 4).
