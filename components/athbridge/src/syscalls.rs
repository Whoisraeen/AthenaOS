...
/// guest `gs:[0x30]` reads the TEB self-pointer (ath_abi syscall 282).
pub const SYS_SET_GS_BASE: u64 = 282;
/// `SYS_MPROTECT(addr, len, prot)` — change protection flags on already-mapped
/// user pages (ath_abi syscall 283). The W^X RW→RX flip the loader needs.
pub const SYS_MPROTECT: u64 = 283;

/// `prot` bits shared with `SYS_MMAP` (ath_abi `PROT_*`). `PROT_READ |
