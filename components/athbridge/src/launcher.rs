...
//! naturally." This is the shared load path the `athbridge_run` binary calls so
...
/// loop — `athbridge_run::_start` does the `SYS_OPEN`/`SYS_READ`).
...
pub unsafe fn run_target(target: &Target, image: Vec<u8>, command_line: &str) -> ! {
    ...
    let session = match FullCompatSession::new(
        ...
        Ok(s) => Box::new(s),
        Err(_) => die(
            "[athbridge_run] FATAL: session construction failed\n",
            LAUNCH_ERR_SESSION,
        ),
    };
    if install_host_context(session).is_some() {
        die(
            "[athbridge_run] FATAL: duplicate host context\n",
            LAUNCH_ERR_DUP_CTX,
        );
    }
    if !host_context_installed() {
        die(
            "[athbridge_run] FATAL: host context not installed\n",
            LAUNCH_ERR_DUP_CTX,
        );
    }
    ...
    let loaded = match load_pe_executable(&image) {
        Ok(l) => l,
        Err(e) => die(
            &format!("[athbridge_run] FATAL: load: {e:?}\n"),
            LAUNCH_ERR_LOAD,
        ),
    };
    if loaded.stubbed_imports != 0 {
        die(
            &format!(
                "[athbridge_run] FATAL: {} imports UNRESOLVED (would fail-loud before entry)\n",
    ...
    match target {
        ...
        Target::BundledExit42 => {
            ...
            // "[athbridge] guest ExitProcess(42) -> exit 42 PASS". A real `.exe`
