...
//! (`docs/components/athbridge-process-model.md` §"fast-path") becomes a
...
//! the `athbridge-wine-strategy.md` §8 gate. Wiring `SyscallFutexOps` to issue
...
/// instance backs `/proc/athena/athbridge_syncbroker`; the smoketest asserts the
...
/// `/proc/athena/athbridge_syncbroker` — the cross-process sync engine's capability
