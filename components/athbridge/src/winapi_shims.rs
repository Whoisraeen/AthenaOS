...
fn with_ctx<R>(f: impl FnOnce(&mut FullCompatSession) -> R) -> R {
    ...
    let result = unsafe {
        match (*HOST_CTX.ctx.get()).as_mut() {
            ...
            None => {
                HOST_CTX.lock.store(false, Ordering::Release);
                die("[athbridge] FATAL: Win32 shim called before install_host_context\n");
            }
        }
    };
    HOST_CTX.lock.store(false, Ordering::Release);
...
pub unsafe extern "win64" fn shim_exit_process(exit_code: u32) -> ! {
    ...
    if REAL_EXE_MILESTONE_ARMED.swap(false, Ordering::SeqCst) {
        ...
        match kind {
            2 => {
                ...
                if stdout_ok && exit_code == 0 {
                    syscalls::sys_debug_print(
                        b"[athbridge] smoketest: real MSVC /MT printf exe -> stdout \"printf says hi 42 from a real exe\" + exit 0 PASS\n",
                    );
                } else {
                    let msg = alloc::format!(
                        "[athbridge] smoketest: real MSVC /MT printf exe -> reached printf but stdout_ok={} (got {} bytes) exit={} FAIL\n",
                        stdout_ok,
                        captured.len(),
                        exit_code
                    );
            ...
            3 => {
                ...
                if stdout_ok && exit_code == 0 {
                    syscalls::sys_debug_print(
                        b"[athbridge] smoketest: real MSVC /MT C++ exe -> static-ctor ran + main + exit 0 PASS\n",
                    );
                } else {
                    ...
                    let msg = alloc::format!(
                        "[athbridge] smoketest: real MSVC /MT C++ exe -> static-ctor ran + main + exit 0 (ctor_line_seen={} stdout_ok={} got {} bytes exit={}) FAIL\n",
                        ctor_seen,
                        stdout_ok,
                        captured.len(),
                        exit_code
            ...
            _ => {
                ...
                if stdout_ok && exit_code == 0 {
                    syscalls::sys_debug_print(
                        b"[athbridge] smoketest: real MSVC /MT exe -> reached main + stdout \"real windows exe\" + exit 0 PASS\n",
                    );
                } else {
                    let msg = alloc::format!(
                        "[athbridge] smoketest: real MSVC /MT exe -> reached main but stdout_ok={} (got {} bytes) exit={} FAIL\n",
                        stdout_ok,
                        captured.len(),
                        exit_code
                    );
    ...
    if EXIT42_SENTINEL_ARMED.swap(false, Ordering::SeqCst) {
        ...
        let msg = alloc::format!(
            "[athbridge] guest ExitProcess({}) -> exit {} {}\n",
            exit_code,
            exit_code,
            verdict
        );
        ...
        let msg = alloc::format!(
            "[athbridge] guest ExitProcess({}) -> exit {} (reaped by parent)\n",
            exit_code,
            exit_code
        );
        syscalls::sys_debug_print(msg.as_bytes());
...
// docs/athbridge-real-crt-abi.md.
// ---------------------------------------------------------------------------
...
pub unsafe extern "win64" fn shim_terminate_process(_process: u64, exit_code: u32) -> ! {
    let msg = alloc::format!(
        "[athbridge] guest TerminateProcess({}) -> exit {}\n",
        exit_code,
        exit_code
    );
    syscalls::sys_debug_print(msg.as_bytes());
...
pub unsafe extern "win64" fn shim_rtl_unwind_ex(
    ...
    syscalls::sys_debug_print(
        b"[athbridge] FATAL: RtlUnwindEx invoked before main (live SEH dispatch is HUMAN-GATED Phase 11.2)\n",
    );
    syscalls::sys_exit(0xDEAD)
}
...
pub unsafe extern "win64" fn shim_raise_exception(
    ...
    let msg = alloc::format!(
        "[athbridge] FATAL: RaiseException(code={:#x}) before main (SEH dispatch HUMAN-GATED Phase 11.2)\n",
        code
    );
    syscalls::sys_debug_print(msg.as_bytes());
    syscalls::sys_exit(0xDEAD)
...
// ---------------------------------------------------------------------------
//
// These back the `athbridge_server` broker's in-process object model
// (`docs/components/athbridge-wine-strategy.md` §6.1). The previous code never
...
// close that gap (`docs/components/athbridge-wine-strategy.md` §7 / Phase A.3).
...
// docs/components/athbridge-readiness.md "coverage hit list"). Every entry is
...
//     cross-thread *blocking* is deferred to the athbridge_server broker/futex
//     (see docs/components/athbridge-server-design.md). Mirrors the existing
...
pub unsafe extern "win64" fn shim_debug_break() {
    syscalls::sys_debug_print(b"[athbridge] DebugBreak (ignored: no debugger)\n");
}
