...
//! `docs/components/athbridge-process-model.md` §2 option (b): the parent
//! writes the per-spawn launch target to a well-known VFS path *before*
//! spawning `athbridge_run`, and `athbridge_run` reads it at startup to learn
...
/// spawning `athbridge_run`, and the launcher reads at startup.
///
...
///
/// It lives under the session home (`/home/athena/`) deliberately: that tree is
...
pub const HANDOFF_PATH: &[u8] = b"/home/athena/.rae-launch-target";
...
/// The launch target `athbridge_run` resolves at startup.
///
