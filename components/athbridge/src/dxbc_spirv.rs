...
fn translate_inner(
    ...
    let is_fragment = matches!(stage, athgfx::ShaderStage::Fragment);
...
pub fn translate(dxbc: &[u8], _opts: TranslateOpts) -> Result<Translated, ShaderError> {
    ...
    let stage = match program_type {
        0 => athgfx::ShaderStage::Fragment,
        1 => athgfx::ShaderStage::Vertex,
        2 => athgfx::ShaderStage::Geometry,
        3 => athgfx::ShaderStage::TessControl,
        4 => athgfx::ShaderStage::TessEvaluation,
        5 => athgfx::ShaderStage::Compute,
        other => return Err(ShaderError::UnsupportedShaderModel(other)),
    };
    // Slice 1 supports only vertex + fragment.
    if !matches!(
        stage,
        athgfx::ShaderStage::Vertex | athgfx::ShaderStage::Fragment
    ) {
...
/// filesystem. The kernel-side `[athbridge]` smoketest calls `run_self_test()`.
...
pub fn run_self_test() -> bool {
    ...
    if t.stage != athgfx::ShaderStage::Vertex {
        return false;
    }
    if t.spirv.len() < 20 {
        return false;
    ...
    if p.stage != athgfx::ShaderStage::Fragment {
        return false;
    }
