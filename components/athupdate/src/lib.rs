...
//!
//! See `docs/components/athupdate.md` for the design.
#![no_std]
...
impl DeltaEngine {
    ...
    ///
    /// `patch_data` is the serialized [`DeltaOp`](ath_diff::DeltaOp) stream (see
    /// [`crate::serialize_delta`]). This routes through the real `ath_diff`
    ...
    pub fn apply_patch(&self, base_data: &[u8], delta: &DeltaPackage) -> Result<Vec<u8>> {
        ...
        let new_image =
            ath_diff::apply_delta(base_data, &ops).map_err(|_| UpdateError::CorruptedPackage)?;
        let actual = ath_hash::sha256(&new_image);
