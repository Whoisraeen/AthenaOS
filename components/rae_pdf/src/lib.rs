...
//! sit on. It is the inverse of raeprint's `pdf.rs` **generator**: that module
...
//!   [`rae_deflate::zlib_decompress`], `/Length` direct or indirect. Other filters
...
//!   - decompressed stream size (delegated to rae_deflate's [`rae_deflate::MAX_OUTPUT`]),
...
impl<'a> Lexer<'a> {
    ...
    fn parse_literal_string(&mut self) -> Result<Object, PdfError> {
        ...
        loop {
            ...
            if out.len() > rae_deflate::MAX_OUTPUT {
    ...
    fn parse_hex_string(&mut self) -> Result<Object, PdfError> {
        ...
        loop {
            ...
            nibbles.push(v);
            if nibbles.len() > rae_deflate::MAX_OUTPUT {
                return Err(PdfError::TooLarge);
            }
        }
...
fn decode_stream(s: &Stream, doc: Option<&Document>) -> Result<Vec<u8>, PdfError> {
    ...
    for f in &filters {
        match f.as_str() {
            "FlateDecode" | "Fl" => {
                data = rae_deflate::zlib_decompress(&data).map_err(|_| PdfError::BadStream)?;
                if data.len() > rae_deflate::MAX_OUTPUT {
                    return Err(PdfError::TooLarge);
                }
            }
...
// ════════════════════════════════════════════════════════════════════════════
// Host KAT suite — `cargo test -p rae_pdf`. FAIL-able by construction.
// ════════════════════════════════════════════════════════════════════════════
#[cfg(test)]
mod tests;
