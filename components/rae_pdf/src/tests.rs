//! Host KAT suite for rae_pdf — `cargo test -p rae_pdf`.
//!
...
// raeprint's generator relies on). This keeps the KATs honest: we assemble a real
...
/// rae_deflate's compressor so the reader's FlateDecode path is exercised).
fn flate_content_obj(content: &[u8]) -> Vec<u8> {
    let comp = rae_deflate::zlib_compress(content);
    let mut o = Vec::new();
    o.extend_from_slice(
        format!(
...
fn xref_stream_pdf_parses() {
    ...
    let comp = rae_deflate::zlib_compress(&table);
