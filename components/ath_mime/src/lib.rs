...
//! the bottom (`cargo test -p ath_mime`) are the primary proof — no QEMU, no
//! image build.
//!
...
//! file (via `ath_toml`) is likewise a follow-up; [`Registry`] is a pure,
...
/// `ath_toml`) is a follow-up slot; this type carries no I/O.
///
...
impl Registry {
    ...
    pub fn with_defaults() -> Self {
        ...
        r.set("text/html", "athweb", &["athweb", "text_editor", "files"]);
        r.set(
            "image/svg+xml",
            "photos",
        ...
        r.set("application/x-athpkg", "athstore", &["athstore", "files"]);
...
// ===========================================================================
// Host KATs — the primary, FAIL-able proof. `cargo test -p ath_mime`.
//
...
mod tests {
    ...
    fn zip_container_refined_by_extension() {
        ...
        // A .athpkg is actually our package container; if a publisher ships it as
        // a zip-on-the-wire, the extension refines it to x-athpkg.
        assert_eq!(
            resolve_mime("game.athpkg", Some(zip)),
            MimeType("application/x-athpkg")
        );
    }
    ...
    //
    // ath_mime sniffs the LEADING BYTES of arbitrary/downloaded files: a pure
    ...
    fn prop_magic_beats_mislabeled_extension() {
        ...
        // (a .json/.png/.athpkg whose bytes happen to start with PK\x03\x04).
