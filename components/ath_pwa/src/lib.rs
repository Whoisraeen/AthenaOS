...
//!   [`ath_json`] core. A manifest that does not identify the app (no `name` and
...
/// launcher wiring lives in athshell/start_menu and is intentionally NOT done
...
///
/// ```
/// use ath_pwa::{parse_manifest, DisplayMode};
...
pub fn parse_manifest(input: &str) -> Result<WebAppManifest, ManifestError> {
    let json = ath_json::parse(input).map_err(|_| ManifestError::BadJson)?;
...
// ── Host KATs (the FAIL-able proof: `cargo test -p ath_pwa`) ─────────────────
