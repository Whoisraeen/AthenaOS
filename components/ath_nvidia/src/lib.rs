//! ath_nvidia — pure NVIDIA GPU bring-up logic for the `nvidiad` daemon.
//!
//! AthenaOS's native NVIDIA driver, built to the same discipline as
//! [`ath_amdgpu`](../ath_amdgpu): the parts of the bring-up that are *pure
...
//! `cargo test -p ath_nvidia` (the host-KAT pattern) rather than only being
