// ════════════════════════════════════════════════════════════════════════════
// Host KAT suite — `cargo test -p rae_docx`. FAIL-able by construction.
//
...
fn written_docx_has_required_opc_parts() {
    ...
    // The container must carry the three required OPC parts (open via rae_zip
    // directly so we assert on the package, not just the parsed model).
    let ar = rae_zip::Archive::open(&bytes).expect("written bytes are a valid zip");
    assert!(
