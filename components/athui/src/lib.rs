//! AthUI v0 — minimal widget toolkit drawn through athgfx::Canvas.
//!
...
/// so the design-language handoff name `athui::tokens` resolves; apps should
/// consume these instead of redefining a private `const ACCENT` / palette.
pub use ath_tokens as tokens;
...
//
// The earlier athui shape had a `Window { root, canvas }` wrapper. Keep it
