...
//! bin that links raekit's `#[panic_handler]`, so `cargo test` inside it trips
...
//! toggles to `std` under `cfg(test)` (the `rae_tokens` pattern) gives us a
//! clean, FAIL-able proof: `cargo test -p rae_calc`.
//!
...
//! arithmetic (no panic).
//!
//! Pure logic, host-KAT'd (`cargo test -p rae_calc`).
...
///
/// ```
/// assert_eq!(rae_calc::eval("2+3*4"), Ok(14.0));
/// assert_eq!(rae_calc::eval("(2+3)*4"), Ok(20.0));
/// assert!(rae_calc::eval("5/0").is_err());
/// ```
...
///
/// ```
/// assert_eq!(rae_calc::eval_int("0xFF & 0x0F"), Ok(0x0F));
/// assert_eq!(rae_calc::eval_int("1 << 8"), Ok(256));
/// assert_eq!(rae_calc::eval_int("0b1010 | 0b0101"), Ok(0x0F));
/// ```
...
// ── Host KATs (the FAIL-able proof: `cargo test -p rae_calc`) ────────────────
