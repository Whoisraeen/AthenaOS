...
//! and zips it with [`rae_zip::ZipWriter`] — it does **not** hand-roll ZIP/DEFLATE.
//!
...
//! (`cargo test -p rae_xlsx`) is the load-bearing proof; tweak any expected value
...
//! [`XlsxWriteError`]. [`rae_zip::ZipWriter`] bounds the archive itself. XML text is
...
impl From<rae_zip::ZipError> for XlsxWriteError {
    fn from(_: rae_zip::ZipError) -> Self {
        XlsxWriteError::Zip
    }
}
...
impl WorkbookBuilder {
    ...
    /// worksheet per sheet) and zips them via [`rae_zip::ZipWriter`]. Returns `Err`
