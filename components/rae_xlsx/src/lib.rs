...
//! `xl/_rels/workbook.xml.rels`. This crate REUSES [`rae_zip`] for the archive layer
...
//! building, mirroring rae_docx / rae_png:
//!   - each decompressed XML part is capped at [`MAX_PART_XML`] (rae_zip also
...
//! (`cargo test -p rae_xlsx`): it hand-assembles a minimal valid `.xlsx` in-test and
...
/// ([`XlsxError::TooLarge`]) rather than parsed. (rae_zip's own
/// [`rae_zip::MAX_ENTRY_SIZE`] / [`rae_zip::MAX_RATIO`] guards fire first on a
...
impl Workbook {
    ...
    ///
    /// Reads the ZIP central directory via [`rae_zip`], requires `xl/workbook.xml`
...
// ════════════════════════════════════════════════════════════════════════════
// Minimal XML tokenizer (identical strategy to rae_docx — well-formed,
...
// ─── Small helpers (shared with rae_docx's conventions) ─────────────────────
