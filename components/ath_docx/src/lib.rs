...
//! **WordprocessingML**. This crate REUSES [`ath_zip`] for the archive layer — it
...
//! building, mirroring ath_png's dimension caps:
//!   - the decompressed `document.xml` is capped at [`MAX_DOCUMENT_XML`] (ath_zip
...
//! (`cargo test -p ath_docx`): it hand-assembles a minimal valid `.docx` in-test and
...
/// Word bodies are a tiny fraction of this even for very long documents. (ath_zip's
/// own [`ath_zip::MAX_ENTRY_SIZE`] / [`ath_zip::MAX_RATIO`] guards fire first on a
...
impl Document {
    ...
    ///
    /// Reads the ZIP central directory via [`ath_zip`], requires `word/document.xml`
...
// inverse of [`Document::open`]: it REUSES [`ath_zip::ZipWriter`] for the OPC ZIP
...
impl Document {
    ...
    ///
    /// Produces the minimal valid Office Open XML package via [`ath_zip::ZipWriter`]:
