...
//! WebAuthn (`raeid::webauthn`, COSE **RS256** = alg `-257`, the algorithm
...
//! and by `raeid::webauthn`'s genuine RS256 WebAuthn assertion KAT.
...
mod tests {
    ...
    // before embedding (the same key material `raeid::webauthn`'s RS256 KAT
    ...
    // `raeid::webauthn` feeds it.
    const MSG: [u8; 69] = [
        // authData: SHA-256("raeenos.local") || flags=0x05 || signCount=5(BE)
