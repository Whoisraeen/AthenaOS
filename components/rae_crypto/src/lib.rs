...
//! nobody rolls their own (the account-password path in `raeid` previously
...
/// primitive for secure channels (raesync, TLS, WireGuard).
pub mod x25519;
...
/// One MAC namespace shared by `rae_otp` (2FA codes) and the protocol stacks.
pub mod hmac;

/// ChaCha20-Poly1305 AEAD (RFC 8439) — the shared authenticated cipher for
/// secure channels (raesync, TLS, WireGuard). `seal` / `open`.
...
/// `p256`/`ecdsa` crates; unblocks `raeid::webauthn`'s ES256 path (the
...
/// kernel's DNSSEC / X.509 path and by `raeid::webauthn`'s COSE **RS256**
