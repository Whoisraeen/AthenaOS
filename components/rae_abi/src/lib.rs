...
//! `RAEEN_AGENT=opus` AND the commit message carries the `[interface]` tag.
//!
//! # Versioning
//!
...
pub mod syscall {
    ...
    /// (`E_RAEFS_*`, high bits set) on failure.
    pub const SYS_RAEFS_SNAPSHOT_CREATE: u64 = 101;
    /// `RESTORE(snap_id)` — atomically roll the live FS back to `snap_id`.
    /// `rax` = 0 on success, error sentinel on failure.
    pub const SYS_RAEFS_SNAPSHOT_RESTORE: u64 = 102;
    /// `DELETE(snap_id)` — drop a snapshot and reclaim its CoW block refs.
    /// `rax` = 0 on success, error sentinel on failure.
    pub const SYS_RAEFS_SNAPSHOT_DELETE: u64 = 103;
    ...
    /// Claims the job (Queued → Running). The `raelangd` daemon half.
    ...
    // default: 0 = pure computation). The kernel host and `raelangd` gate
    ...
    //
    // Kernel side: kernel/src/syscall.rs + compositor.rs. raekit wrappers:
    // raekit::sys::capture_{start,read,stop}.
    ...
    // kernel/src/syscall.rs. raekit wrappers: raekit::sys::a11y_{snapshot,action}.
    ...
    /// `components/raeui::accessibility::AccessibilityRole` so the kernel can
    /// serialize raeui's live widget tree into this wire repr with NO lossy
    /// remapping (each raeui role maps to exactly one tag here). `Desktop` (the
    /// synthetic root) and `Unknown` have no raeui counterpart (root/fallback).
    ...
    /// announces (`AccessibilityTraits` in raeui) plus the window-tier states the
    ...
    /// before calling. Mirrors raeui `AccessibilityAction`.
    ...
    /// existing `SYS_POLL_MOUSE`/`raekit::sys::poll_mouse` event stream. Reads a
    ...
    /// remaining buffer (the documented raekit decoder does both, never panicking
    ...
    /// compositor's native front-buffer + `rae_tokens` ARGB). `CAPTURE_FMT_BGRA32`
    ...
    /// entry is never evicted). Mirrors `raeshell::ClipboardManager::max_history`.
    ...
    /// opaque. Mirrors `raeshell::clipboard::ClipboardFormat`.
    ...
    /// Mirrors `kernel::theme_engine::RAEBLUE` / `rae_tokens::RAEBLUE`; kept on
    ...
    // userspace shim components/raeen_linuxkpi (Composer) calls these.
    ...
    /// `raeen_drm::kms::atomic_commit`) registers its display scanout framebuffer
    ...
    /// 281 search_query_resolved · 282-283 raebridge MSVC-CRT · 284-290
...
pub struct ThemeInfo {
    ...
    /// the seed every surface feeds into `rae_tokens::derive_accent`. Equals
    ...
    pub fg_argb: u32,
    /// `1` if the active palette is dark, `0` if light — picks `rae_tokens::DARK`
...
// and `raeui::accessibility::AccessibilityTree` use (parent ids, not nested),
...
/// chosen to map 1:1 onto `raeui::accessibility`'s `AccessibilityRole`,
/// `AccessibilityTraits`, and `AccessibilityAction` so the kernel serializes
/// raeui's live tree with no lossy remapping.
#[repr(C)]
...
// against the remaining capacity (see the raekit decoder). The const layout is
// the single contract both the kernel encoder and the raekit decoder quote;
...
// `(id, kind)` record share ONE source of truth. raekit re-exports the same
// values in its `sys` module (it deliberately doesn't depend on rae_abi).
...
const _: () = {
    ...
    assert!(syscall::SYS_RAEFS_SNAPSHOT_DELETE == 103);
    assert!(syscall::SYS_RAEFS_SNAPSHOT_DELETE < syscall::SYS_DRIVER_REGISTER);
    ...
    // and raekit decoder both quote this single number.
