...
//! so the panel cannot call `athkit::sys::clip_hist_*` directly. Instead it is a
...
//! (the userspace `athkit::sys::clip_hist_*` wrappers are the equivalent surface
...
/// `athkit::sys::ClipEntry` fields the panel needs (format + pinned flag +
...
/// Mirrors `athkit::sys::CLIP_FMT_*` so a future image/url/files entry reads
...
/// `athkit::sys::clip_hist_*` userspace wrappers reach).
...
impl ClipboardPanel {
    ...
    pub fn render(&self, canvas: &mut athgfx::Canvas) {
        ...
        use ath_tokens::{RADIUS_LG, RADIUS_XS, SPACE_2, SPACE_3, SPACE_4};
        ...
        canvas.draw_text_aa(
            content_x as i32,
            (header_y
                + (HEADER_H.saturating_sub(ath_tokens::TYPE_SUBTITLE.line_height as usize)) / 2)
                as i32,
            "Clipboard",
        ...
        canvas.draw_text_aa(
            clear_x,
            (header_y
                + (HEADER_H.saturating_sub(ath_tokens::TYPE_CAPTION.line_height as usize)) / 2)
                as i32,
            clear_label,
            ath_tokens::TYPE_CAPTION,
            p.state_danger,
        ...
        if self.incognito {
            ...
            canvas.draw_text_aa(
                ...
                (cursor_y
                    + (SECTION_H.saturating_sub(ath_tokens::TYPE_CAPTION.line_height as usize)) / 2)
                    as i32,
        ...
        for (i, (_hist_idx, row)) in self.rows.iter().enumerate() {
            ...
            let chip_h = ath_tokens::TYPE_CAPTION.line_height as usize + 4;
            ...
            canvas.draw_text_aa(
                preview_x,
                (ry + (ROW_H.saturating_sub(ath_tokens::TYPE_BODY.line_height as usize)) / 2)
                    as i32,
                preview_text,
                ath_tokens::TYPE_BODY,
                p.text_primary,
    ...
    fn draw_section_header(
        ...
        y: usize,
        label: &str,
        p: &ath_tokens::Palette,
    ) {
        canvas.draw_text_aa(
            x as i32,
            (y + (SECTION_H.saturating_sub(ath_tokens::TYPE_CAPTION.line_height as usize)) / 2)
                as i32,
            label,
            ath_tokens::TYPE_CAPTION,
            p.text_secondary,
    ...
    pub fn design_proof() -> ClipPanelProof {
        ...
        panel.set_rows(alloc::vec![
            ...
            ClipRow {
                format: 4,
                pinned: false,
                preview: String::from("https://athena.os"),
            },
        ]);
        panel.open();
        ...
        let want_accent = ath_tokens::derive_accent(crate::active_accent(), crate::PALETTE).base;
...
fn truncate_to_width(canvas: &athgfx::Canvas, text: &str, max_w: usize) -> String {
    if max_w == 0 {
        return String::new();
    }
    let full = canvas.measure_text_aa(text, ath_tokens::TYPE_BODY, athgfx::text::FontFamily::Sans);
