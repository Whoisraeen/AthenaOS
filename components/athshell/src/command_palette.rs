...
//! now agree on one index (athena-kernel's review fix). If no kernel source is
...
const SETTINGS_ACTIONS: &[SettingsAction] = &[
    ...
    SettingsAction {
        ...
        keywords: &["script", "rae", "athlang", "run", "automation", "macro"],
    },
];
...
pub(crate) const PALETTE_GLASS_TIER: ath_tokens::GlassTier = ath_tokens::GLASS_POPOVER_DARK;
...
impl CommandPalette {
    ...
    pub fn render(&self, canvas: &mut athgfx::Canvas) {
        ...
        use ath_tokens::{RADIUS_LG, RADIUS_SM, RADIUS_XS, SPACE_2, SPACE_3, SPACE_4};
        ...
        let margin = ath_tokens::SPACE_6 as usize;
        ...
        // Leading magnifier — a REAL athgfx line-icon (not the bitmap-font '>'),
        ...
        let text_y = (field_y
            + (field_h.saturating_sub(ath_tokens::TYPE_SUBTITLE.line_height as usize)) / 2)
            as i32;
        ...
        if self.results.is_empty() {
            ...
            canvas.draw_text_aa(
                (field_x + SPACE_3 as usize) as i32,
                (list_y + (row_h.saturating_sub(ath_tokens::TYPE_BODY.line_height as usize)) / 2)
                    as i32,
        ...
        for (i, r) in self.results.iter().take(8).enumerate() {
            ...
            // Leading line-icon per result kind — the SAME shipped athgfx icon set
            ...
            if !r.subtitle.is_empty() {
                let sub_y =
                    (ry + SPACE_2 as usize + ath_tokens::TYPE_BODY.line_height as usize) as i32;
            ...
            let tag_y = (ry
                + (row_h.saturating_sub(ath_tokens::TYPE_CAPTION.line_height as usize)) / 2)
                as i32;
            canvas.draw_text_aa(
                tag_x,
                tag_y,
    ...
    pub(crate) fn selection_ink(p: &ath_tokens::Palette) -> u32 {
        p.bg_base
    }

    /// Map a result to the closest shipped `athgfx` line-icon — the SAME icon set
    ...
    fn result_line_icon(r: &SearchResult) -> athgfx::icon::Icon {
...
mod tests {
    ...
    /// bytes athkit's `search::decode_results` parses. Proves the host that wires
    /// the kernel source (and the athkit userspace path) reads the format right.
    #[test]
    fn kernel_result_blob_decodes() {
        ...
        // Decode mirroring the kernel layout (the logic athkit shares).
    ...
    fn selected_row_is_dark_on_accent_and_legible() {
        ...
        let dark_on_accent = ath_tokens::contrast_ratio(ink, a.base);
        assert!(
            dark_on_accent >= 4.5,
        ...
        let white_on_accent = ath_tokens::contrast_ratio(0xFF_FF_FF_FF, a.base);
        assert!(
    ...
    /// FAIL-able: every result kind maps to a real shipped `athgfx` line-icon
