...
//!
//! **Cohesion (Phase 1):** this surface reads the LIVE `ath_tokens` — the same
...
//!   the cohesion smoketest + `/proc/athena/gaming`.
//!
...
//! record (athshell can't depend on the kernel crate). Each field is a
...
//! (`SYS_GAME_PROFILE_SET` -> `GET` round-trip -> `APPLY`), so `/proc/athena/games`
...
// colour comes from `ath_tokens` over the DARK palette (couch defaults to dark
...
const PALETTE: &Palette = &ath_tokens::DARK;
...
fn accent() -> ath_tokens::AccentRamp {
    ath_tokens::derive_accent(crate::active_accent(), PALETTE)
}
...
#[inline]
fn couch_text_w(canvas: &athgfx::Canvas, s: &str, style: TypeStyle) -> usize {
...
fn draw_focus_ring(
    ...
    let glow = ath_tokens::elev_focus(acc.glow).color;
...
// 32767, hat 0-7/8, button bitmap); athshell can't depend on the kernel crate,
...
/// A decoded pad report snapshot — a athshell-side mirror of
/// `kernel::hid_gamepad::PadInput` (athshell can't depend on the kernel crate).
...
// syscalls 58–61, `/proc/athena/games`). athshell can't depend on the kernel
...
pub struct ProfileEditor {
    /// The stable kernel profile id being edited (e.g. "athplay:steam:730" or a
...
pub struct GameOsShell {
    ...
    /// binds via `bind_pad`. Exposed for `/proc/athena/gaming` so the bound pad
    ...
    /// syscalls (SET/GET/APPLY) so `/proc/athena/games` reflects every edit.
...
impl GameOsShell {
    ...
    /// True while a cross-fade is running (for `/proc/athena/gaming`).
    #[must_use]
    ...
    pub fn render_osk(&self, canvas: &mut athgfx::Canvas) {
    ...
    /// saturating layout).
    pub fn render_hint_bar(&self, canvas: &mut athgfx::Canvas) {
    ...
    pub fn render_sidebar(&self, canvas: &mut athgfx::Canvas) {
        let acc = accent();
    ...
    pub fn render_top_bar(&self, canvas: &mut athgfx::Canvas) {
        canvas.fill_rect(
            SIDEBAR_W,
            0,
    ...
    pub fn render_home(&self, canvas: &mut athgfx::Canvas, ox: usize, oy: usize) {
    ...
    pub fn render_library(&self, canvas: &mut athgfx::Canvas, ox: usize, oy: usize) {
        let games = self.visible_games();
        let section_x = ox + GRID_MARGIN;
    ...
    pub fn render_game_detail(&self, canvas: &mut athgfx::Canvas, game_index: usize) {
        let games = self.visible_games();
        if game_index >= games.len() {
            return;
        }
        ...
        let mut row =
            |canvas: &mut athgfx::Canvas, y: &mut usize, label: &str, value: &str, vc: u32| {
    ...
    pub fn render_profile_editor(&self, canvas: &mut athgfx::Canvas) {
    ...
    pub fn render_quick_menu(&self, canvas: &mut athgfx::Canvas) {
        let acc = accent();
        let item_h = ROW_H;
    ...
    pub fn render_now_playing(&self, canvas: &mut athgfx::Canvas, np: &NowPlaying) {
        let bar_h = ROW_H;
    ...
    fn render_settings(&self, canvas: &mut athgfx::Canvas) {
        let acc = accent();
        ...
        let mut kv = |canvas: &mut athgfx::Canvas, y: usize, k: &str, v: &str, vc: u32| {
    ...
    fn render_search_overlay(&self, canvas: &mut athgfx::Canvas) {
        let acc = accent();
...
impl GameOsShell {
    fn render_carousel(&self, canvas: &mut athgfx::Canvas, ox: usize, oy: usize) {
        let acc = accent();
...
/// change. Read by the boot smoketest and `/proc/athena/gaming`.
#[must_use]
...
/// The default controller glyph-set tag for `/proc/athena/gaming` (Phase 2). The
...
/// home context — for `/proc/athena/gaming`. Built with the SAME `context_hints`
...
/// Exposed for `/proc/athena/gaming` and the padbind smoketest — the pure
...
pub fn run_couch_smoketest(
    ...
    let expected = ath_tokens::derive_accent(couch_active_seed(), PALETTE).base;
    ...
    // `athgfx::text::run_boot_smoketest`, so without this the engine would not
...
mod glyph_tests {
    ...
    fn profile_flag_bits_match_kernel() {
        // The athshell mirror flag bits MUST match kernel::game_profile::FLAG_*.
