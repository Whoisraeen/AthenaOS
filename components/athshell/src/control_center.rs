...
//!
//! Cohesion (spec §5): every colour resolves from `ath_tokens` through the LIVE
...
use alloc::vec::Vec;
use ath_tokens::{RADIUS_LG, RADIUS_MD, RADIUS_SM, SPACE_1, SPACE_2, SPACE_3, SPACE_4};
...
/// frost still reads beneath the interaction flash (ath_tokens' `with_alpha` is
...
// the panel now paints the shared `ath_tokens::GLASS_PANEL_DARK` tier.)
...
impl TileKind {
    /// The real line-icon (`athgfx::icon::Icon`) this tile draws — replacing the
    ...
    /// Round-2 #1). The icon system landed in `athgfx::icon`; this is the wiring
    /// that consumes it. Map per the gfx agent's recommendation.
    #[must_use]
    pub const fn icon(self) -> athgfx::icon::Icon {
...
pub enum TileBackend {
    ...
    /// flips `ath_tokens::active_palette()`). The Accessibility tile's primary
...
/// Focus is distinct from hover (athena-accessibility sign-off requirement).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Interact {
    /// Resting.
    None,
...
//    primitives; see the athgfx follow-up list in the module REPORT) ───────────
...
/// vectors by [`draw_speaker`] / [`draw_sun`] — the `athgfx::icon` set has no
...
impl ControlCenter {
    ...
    fn fixed_content_height(&self) -> usize {
        ...
        h += SPACE_3 as usize + ath_tokens::TYPE_SUBTITLE.line_height as usize + SPACE_2 as usize;
    ...
    pub fn tile_text_boxes(cy: usize) -> (usize, usize, usize, usize) {
        ...
        let label_bot = label_y + ath_tokens::TYPE_LABEL.line_height as usize;
    ...
    fn draw_tile_card(
        ...
        match interact {
            ...
            Interact::Hover => {
                canvas.fill_rounded_rect(x, y, w, h, radius, ath_tokens::GLASS_FROST_LIGHTEN)
            }
            _ => {}
        }
    }
    ...
    /// A dedicated `text.on-accent` token would be cleaner — NOTED for athena-ui
    /// (not added this pass, ath_tokens is frozen for this slice).
    #[must_use]
    ...
    pub fn render(&self, canvas: &mut athgfx::Canvas) {
        ...
        let p = crate::active_palette();
        let hc = ath_tokens::high_contrast();
        ...
        let panel_fill = if hc {
            ...
            ath_tokens::GLASS_PANEL_DARK.tint
        };
        ...
        cy += ath_tokens::TYPE_SUBTITLE.line_height as usize + SPACE_2 as usize;
    ...
    fn render_tile_grid(
        ...
        let mut cy = y;
        let mut col = 0usize;
        let inner_r = ath_tokens::concentric(RADIUS_MD, SPACE_2) as usize;
        ...
        for (i, t) in self.tiles[start..end].iter().enumerate() {
            ...
            if !t.expandable && !t.disabled {
                ...
                let pill_r = ath_tokens::radius_pill(pill_h as u32) as usize;
                ...
                canvas.fill_rounded_rect(
                    ...
                    knob,
                    knob,
                    ath_tokens::radius_pill(knob as u32) as usize,
            ...
            // LETTER placeholders — the icon system landed in `athgfx::icon`,
    ...
    fn render_expand_panel(
        ...
        if self.expand_rows.is_empty() {
            canvas.draw_text_aa(
                ...
                (ry + (EXPAND_ROW_HEIGHT - ath_tokens::TYPE_BODY.line_height as usize) / 2) as i32,
                "Scanning...",
        ...
        for row in self.expand_rows.iter().take(visible) {
            ...
            let text_y = ry + (EXPAND_ROW_HEIGHT - ath_tokens::TYPE_BODY.line_height as usize) / 2;
            canvas.draw_text_aa(
    ...
    fn render_slider(
        ...
        let track_r = ath_tokens::radius_pill(4) as usize;
    ...
    fn render_media_card(
        ...
        canvas.draw_text_aa(
            text_x as i32,
            (y + SPACE_2 as usize + ath_tokens::TYPE_LABEL.line_height as usize) as i32,
            &self.media.artist,
            ath_tokens::TYPE_CAPTION,
            p.text_secondary,
            athgfx::text::FontFamily::Sans,
    ...
    fn render_rgb_chips(&self, canvas: &mut athgfx::Canvas, x: usize, y: usize, w: usize) -> usize {
        let a = accent();
        let chip_h = 32usize;
        ...
        for (i, (_name, swatch)) in RGB_EFFECTS.iter().enumerate() {
            ...
            let chip_r = ath_tokens::radius_pill(chip_h as u32) as usize;
    ...
    fn render_perf_segmented(
        ...
        let seg_w = w / n;
        let pill_r = ath_tokens::radius_pill(seg_h as u32) as usize;
        // Track = a luminous frosted pill (Round-4 P0 #1), not a dark slate:
        // popover tint + frost composited over the panel.
        canvas.fill_rounded_rect(x, y, w, seg_h, pill_r, ath_tokens::GLASS_POPOVER_DARK.tint);
        canvas.fill_rounded_rect(x, y, w, seg_h, pill_r, ath_tokens::GLASS_POPOVER_DARK.frost);
        for (i, mode) in PerfMode::ALL.iter().enumerate() {
            ...
            let ly = y as i32 + (seg_h as i32 - ath_tokens::TYPE_CAPTION.line_height as i32) / 2;
            canvas.draw_text_aa(
                lx,
                ly,
                label,
    ...
    fn render_footer(&self, canvas: &mut athgfx::Canvas, x: usize, y: usize, w: usize) {
        let p = PALETTE;
        // Avatar + name (left).
        let av = 24usize;
        canvas.fill_rounded_rect(
            ...
            av,
            av,
            ath_tokens::radius_pill(av as u32) as usize,
            accent().subtle,
        );
        canvas.draw_text_aa(
            (x + av + SPACE_2 as usize) as i32,
            (y + (FOOTER_HEIGHT - ath_tokens::TYPE_LABEL.line_height as usize) / 2) as i32,
            "AthenaOS",
            ath_tokens::TYPE_LABEL,
            p.text_primary,
            athgfx::text::FontFamily::Sans,
    ...
    fn render_focus_ring(&self, canvas: &mut athgfx::Canvas, x: usize, y: usize, w: usize) {
        ...
        let ring = ath_tokens::active_focus_ring(accent().base);
...
//    are reported as follow-ups for the athgfx icon agent to promote into the
...
fn stroke(canvas: &mut athgfx::Canvas, x0: i32, y0: i32, x1: i32, y1: i32, color: u32) {
...
fn draw_power(canvas: &mut athgfx::Canvas, x: usize, y: usize, w: usize, h: usize, color: u32) {
    let cx = (x + w / 2) as i32;
    let cy = (y + h / 2) as i32;
...
fn draw_sun(canvas: &mut athgfx::Canvas, x: usize, y: usize, w: usize, h: usize, color: u32) {
    let cx = (x + w / 2) as i32;
    let cy = (y + h / 2) as i32;
...
fn draw_lock(canvas: &mut athgfx::Canvas, x: usize, y: usize, w: usize, h: usize, color: u32) {
...
fn draw_chevron_up(canvas: &mut athgfx::Canvas, x: usize, y: usize, box_: usize, color: u32) {
    let bx = x as i32;
    let by = y as i32;
    let s = box_ as i32;
    let apex_x = bx + s / 2;
...
pub fn control_center_proof() -> ControlCenterProof {
    ...
    let slider_is_accent_base = a.base == ath_tokens::derive_accent(crate::active_accent(), p).base;

    // §5 cohesion: the panel accent == derive_accent(active_seed).base.
    let want = ath_tokens::derive_accent(crate::active_accent(), p).base;
...
mod tests {
    ...
    fn enabled_tile_label_uses_dark_on_accent_ink_not_white() {
        ...
        let cr = ath_tokens::contrast_ratio(cc.tile_label_ink(&on), fill);
        assert!(
            cr >= 4.5,
        ...
        let white_cr = ath_tokens::contrast_ratio(p.text_primary, fill);
        assert!(
            white_cr < 4.5,
    ...
    fn control_center_repaints_in_high_contrast() {
        ...
        assert_eq!(*crate::active_palette(), ath_tokens::DARK, "off -> normal");
        ...
        let mut canvas = unsafe { athgfx::Canvas::new(buf.as_mut_ptr(), 400, 720, 4) };
    ...
    fn inline_vector_glyphs_paint_ink_no_letters() {
        ...
        let cases: [(&str, fn(&mut athgfx::Canvas)); 8] = [
    ...
    fn renders_without_panic_and_paints_ink() {
        ...
        let mut canvas = unsafe { athgfx::Canvas::new(buf.as_mut_ptr(), 640, 720, 4) };
        cc.render(&mut canvas);
