...
//! `athshell` with the rest of the shell chrome; the compositor calls that act on
...
pub fn selection_ring(seed: u32) -> u32 {
    ath_tokens::derive_accent(seed, &ath_tokens::DARK).base
}
...
pub fn selection_glow(seed: u32) -> u32 {
    ath_tokens::derive_accent(seed, &ath_tokens::DARK).glow
}
...
pub fn run_boot_smoketest() -> (bool, bool, bool) {
    ...
    let expected = ath_tokens::derive_accent(seed, &ath_tokens::DARK).base;
