...
/// `crate::thermal` (CPU/GPU temp) in the kernel; athshell stays decoupled from
...
pub struct GameBar {
    ...
    /// by the smoketest + `/proc/athena/gaming` to prove the toggle fires.
...
impl GameBar {
    ...
    pub fn render_compact_corner(&self, canvas: &mut athgfx::Canvas) {
    ...
    pub fn render_live_overlay(&self, canvas: &mut athgfx::Canvas) {
        if !self.visible {
            return;
        }
        let (ox, oy) = self.overlay_origin();
    ...
    fn render_quick_actions(&self, canvas: &mut athgfx::Canvas) {
    ...
    fn render_ping(&self, canvas: &mut athgfx::Canvas, x: usize, y: usize, w: usize, hl: bool) {
        if hl {
            canvas.fill_rect(x - 4, y - 2, w + 8, 14, 0x44_4E_9C_FF);
        }
    ...
    pub fn render_recording_indicator(&self, canvas: &mut athgfx::Canvas) {
        if !self.recording {
            return;
        }
    ...
    pub fn render_voice_chat(&self, canvas: &mut athgfx::Canvas, x: usize, y: usize, w: usize) {
    ...
    pub fn render_notifications(&self, canvas: &mut athgfx::Canvas) {
...
fn canvas_has_ink(canvas: &athgfx::Canvas, width: usize, height: usize) -> bool {
    for y in 0..height {
        for x in 0..width {
...
// ── Phase 4 host KATs: pure ring + FPS logic (cargo test -p athshell) ────────
