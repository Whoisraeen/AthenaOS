...
//!
//! Concept alignment (RaeenOS_Concept.md §Gaming):
...
//!   normalization → `RaeGamepadEvent` → kernel `SCHED_GAME` dispatch queue.
//!
...
/// Normalized gamepad event emitted by the raeinput daemon to the kernel.
/// Sent via SYS_GAMEPAD_EVENT_PUSH syscall into the SCHED_GAME input queue.
...
pub mod daemon {
    ...
    //! and forwards to the kernel via the SCHED_GAME IPC channel.
    //!
    //! Stub implementation — full daemon wiring is Phase 12.2.
    //! When gilrs is available: call daemon::run() from the raeinput process main().
