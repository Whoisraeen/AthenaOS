...
//! (`PLAIN` / `LOGIN`, base64 via `rae_encode`), `MAIL FROM` / `RCPT TO` / `DATA`
...
impl SmtpClient {
    ...
    /// `raenet` integration step. Requires the server to have advertised STARTTLS.
    ...
    pub fn auth_login<T: MailTransport>(
        ...
        send_line(t, &rae_encode::base64_encode(user.as_bytes()))?;
        let r2 = read_reply(t)?;
        if r2.code != 334 {
            return Err(SmtpError::AuthFailed(r2.code));
        }
        send_line(t, &rae_encode::base64_encode(pass.as_bytes()))?;
        let r3 = read_reply(t)?;
        if r3.code == 235 {
            Ok(())
        } else {
...
impl OutgoingMessage {
    ...
    /// (reusing `rae_encode`). Otherwise a simple `text/plain` message.
    pub fn serialize(&self) -> String {
        ...
        match &self.attachment {
            ...
            Some((fname, ctype, bytes)) => {
                ...
                let b64 = rae_encode::base64_encode(bytes);
...
mod tests {
    ...
    fn full_successful_send_emits_exact_command_sequence() {
        ...
        let expected_b64 = rae_encode::base64_encode(b"\0user\0pass");
    ...
    fn auth_plain_base64_is_exactly_right() {
        ...
        let expected = rae_encode::base64_encode(blob);
        // decode it back to be sure the test oracle is itself right
        let round = rae_encode::base64_decode(&expected).unwrap();
