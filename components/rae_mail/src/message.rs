...
//!
//! `rae_mime` is a file-association resolver, not a message parser, so the parse
...
//! here. We reuse `rae_mime` to *classify* a part by its declared `Content-Type`
//! / filename, and `rae_encode` for the base64 codec.
//!
...
impl Part {
    ...
    /// Reuse `rae_mime` to classify this part: returns the MIME type `rae_mime`
    ...
    /// `rae_mime` has nothing better.
    pub fn classify(&self) -> &str {
        // Prefer filename+magic resolution from rae_mime; fall back to declared.
        let magic = if self.body.is_empty() {
            None
        } else {
            Some(self.body.as_slice())
        };
        let resolved = rae_mime::resolve_mime(&self.filename, magic);
        if resolved == rae_mime::OCTET_STREAM && !self.content_type.is_empty() {
            &self.content_type
        } else {
            resolved.as_str()
        }
...
fn decode_transfer(encoding: &str, body: &[u8]) -> Vec<u8> {
    match encoding {
        "base64" => {
            ...
            match rae_encode::base64_decode(&s) {
                Ok(bytes) => bytes,
                Err(_) => body.to_vec(),
            }
        }
