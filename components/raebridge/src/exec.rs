...
/// KAT. The spec (raebridge-real-crt-abi.md §4) mandates the exact order:
...
// actually import. See `docs/components/raebridge-wine-strategy.md` §7.
...
/// plus one grep-able `[raebridge] import-gap: <DLL>!<name> xN` line per name,
...
pub fn log_missing_import_report() -> usize {
    ...
    let header = format!(
        "[raebridge] import-coverage gap (cumulative): {} distinct missing import(s)\n",
        distinct
    );
    ...
    unsafe {
        ...
        for (name, count) in &ranked {
            let line = format!("[raebridge] import-gap: {} x{}\n", name, count);
...
extern "win64" fn missing_import_handler(idx: u32) -> ! {
    ...
    let msg = format!("[raebridge] FATAL: call into unresolved import: {}\n", name);
...
///
/// Sequence (raebridge-real-crt-abi.md §4, recorded into `load_steps`):
/// mmap RW → copy/reloc/IAT → mprotect `.text` RX → build TEB/PEB →
...
pub fn load_pe_executable(data: &[u8]) -> Result<ExecutablePe, ExecError> {
    ...
    // (docs/components/raebridge-wine-strategy.md §7). Silent when the image
