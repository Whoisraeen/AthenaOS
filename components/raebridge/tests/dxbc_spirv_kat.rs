...
use raebridge::dxbc_spirv::{translate, ShaderError, SpirvBuiltIn, TranslateOpts};
use raegfx::ShaderStage;
...
fn try_spirv_val(spirv: &[u8], label: &str) {
    ...
    tmp.push(format!("raeen_dxbc_kat_{label}_{}.spv", std::process::id()));
    {
...
fn dxbc_spirv_self_test_passes() {
    ...
    assert!(raebridge::dxbc_spirv::run_self_test());
    assert!(raebridge::dxbc_spirv::self_test_bound() > 1);
