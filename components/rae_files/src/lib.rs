...
//! `apps/files` is a `#![no_std] #![no_main]` bin that links raekit's
...
//! only under `cfg(test)` for the harness (the `rae_tokens` / `rae_calc`
//! pattern), giving FAIL-able proofs: `cargo test -p rae_files`.
//!
