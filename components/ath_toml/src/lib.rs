...
//! primary proof (`cargo test -p ath_toml`).
//!
//! ## What it is
...
// ── Host KATs (the FAIL-able proof: `cargo test -p ath_toml`) ────────────────
...
"#;
    ...
    //
    // ath_toml parses user config (file_assoc.toml, app prefs) that can be
