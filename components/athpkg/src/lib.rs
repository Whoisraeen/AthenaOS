//! athpkg — the host-side packer for AthenaOS application bundles (the `athkit bundle`
//! producer half).
//!
...
mod tests {
    ...
    fn round_trip_with_deps() {
        let m = Manifest::new("photos", pack_semver(2, 1, 0))
            .with_dep("ath_image", pack_semver(1, 0, 0), [0xAB; 32])
            .with_dep("athkit", pack_semver(0, 9, 4), [0x11; 32]);
        let bytes = m.pack();
