...
//! series), exactly the rae_calc / raefont pattern. The IDCT itself is then pure
...
//! bottom of this file is the primary proof (run `cargo test -p raemedia`).
...
mod tests {
    ...
    // matching the rae_crypto pattern, so the crate never links std and the
