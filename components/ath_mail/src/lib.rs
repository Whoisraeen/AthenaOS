...
//! socket from `athnet`; in the host KATs it is a [`ScriptedTransport`] that
...
//! server, no network, no QEMU (`cargo test -p ath_mail`). **The live TLS/TCP
//! wiring over `athnet` is a documented LATER integration step** — this crate has
//! no `athnet` dependency and opens no sockets.
//!
//! ## What it implements
...
//!   `ath_mime` to classify parts) so a client can list and read mail.
...
//!
//! ## REUSE vs reimplementation (important — `ath_mime` is NOT a message parser)
//! The task brief assumed `ath_mime` parses MIME *messages*. It does not:
//! `ath_mime` is a file-association resolver (extension/magic → MIME type → app).
...
//! [`message`]. We still reuse `ath_mime` for what it *is* good at — classifying a
//! decoded attachment part by its `Content-Type` / filename — and we reuse
//! `ath_encode` for base64 so no base64 codec is reimplemented here.
//!
...
/// `athnet` socket; the test impl ([`ScriptedTransport`]) replays canned bytes.
///
/// ## Contract
