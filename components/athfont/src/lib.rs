...
// rasterizer host KATs can link (the ath_tokens pattern — see CLAUDE.md §15 and
...
impl FontHandle {
    ...
    /// in that case. This is the single entry point `athgfx` uses per glyph.
...
// alongside in components/athfont/assets/ (see assets/README.md).
//
// RaeSans  = Inter (variable .ttf — the glyf table carries the default-instance
//            Regular outlines, which athfont rasterizes directly; gvar deltas /
...
// S1). Run with `cargo test -p athfont` (per-crate; never `--workspace` —
...
mod tests {
    ...
    fn rasterize_inter_A_at_title() {
        ...
        println!(
            "[athfont-kat] 'A'@22 W={} H={} cov_sum={} mid_row_ink={} (>0)",
    ...
    fn coverage_is_grayscale_not_binary() {
        ...
        println!("[athfont-kat] 'o'@24 has partial-coverage AA edges");
    }
    ...
    fn jetbrains_mono_parses_and_renders() {
        ...
        println!("[athfont-kat] RaeMono 'M'@16 cov_sum={}", cov);
    }
    ...
    fn space_has_no_outline_but_advances() {
        ...
        println!("[athfont-kat] space: no outline, advance>0 OK");
    }
    ...
    fn ui_punctuation_renders_real_ink_in_raesans() {
        let sans = load_sans();
        // (codepoint, name) — drawn from a grep of athshell/apps/ath_tokens.
        let used: &[(u32, &str)] = &[
            (0x2014, "em-dash"),
            (0x2013, "en-dash"),
        ...
        println!(
            "[athfont-kat] RaeSans renders {} UI punctuation/symbol codepoints with real ink",
            used.len()
        );
    }
    ...
    fn supplemented_symbols_render_real_ink() {
        ...
        for &(cp, name) in supp {
            ...
            println!(
                "[athfont-kat] supplement U+{:04X} {:<32} W={} H={} ink={} adv={}",
    ...
    fn uncovered_codepoint_is_visible_notdef_box() {
        ...
        println!(
            "[athfont-kat] uncovered U+{:04X} -> visible notdef box W={} H={} ink={} (FAIL-able demo)",
            cjk,
            g.width,
            g.height,
            ink(&g)
    ...
    fn blank_codepoints_do_not_draw_tofu() {
        ...
        println!("[athfont-kat] whitespace/format codepoints stay ink-free (no false tofu)");
    }
    ...
    fn ui_punctuation_renders_in_raemono() {
        ...
        println!("[athfont-kat] RaeMono renders UI punctuation incl. native U+2715");
    }
}
