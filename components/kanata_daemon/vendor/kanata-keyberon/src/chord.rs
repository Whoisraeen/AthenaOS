...
use alloc::collections::BTreeMap as FxHashMap;
...
impl<'a, T> core::fmt::Debug for ChordV2<'a, T> {
...
pub struct ChordsForKey<'a, T> {
    ...
    pub chords: alloc::vec::Vec<&'a ChordV2<'a, T>>,
}
...
impl<T> core::fmt::Debug for ChordsV2<'_, T> {
            ...
            let count_possible = if prev_count == chord_candidates.len() {
                ...
                for chc in chord_candidates.iter() {
                    if chc.pending_duration > since {
                        min_timeout = core::cmp::min(min_timeout, chc.pending_duration);
                    }
                }
                    ...
                    .filter(|pch| {
                            ...
                            && accumulated_presses[0].since - press.since <= pch.pending_duration
                        {
                            ...
                            if pch.pending_duration > since {
                                min_timeout = core::cmp::min(min_timeout, pch.pending_duration);
                            }
                            true
                        } else {
                            false
