...
impl<'a, T> MultiKeyBuffer<'a, T> {
    ...
    pub(crate) unsafe fn new() -> Self {
        Self {
            ...
            size: 0,
            ptr: alloc::boxed::Box::leak(alloc::boxed::Box::new(slice::from_raw_parts(
                core::ptr::NonNull::dangling().as_ptr(),
                0,
            ))),
            ac: alloc::boxed::Box::leak(alloc::boxed::Box::new(Action::NoOp)),
        }
    }
...
impl<T> Drop for MultiKeyBuffer<'_, T> {
    fn drop(&mut self) {
        unsafe {
            drop(alloc::boxed::Box::from_raw(self.ac));
            drop(alloc::boxed::Box::from_raw(self.ptr));
        }
    }
}
