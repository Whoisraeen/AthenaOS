...
//! This crate is that vault; it pairs with [`ath_otp`]'s TOTP secrets and
//! `athid`'s passkeys to cover the credential side of modern auth.
//!
...
//! delegated to [`ath_crypto`]** — Argon2id (RFC 9106) for the passphrase KDF and
...
//!   [`ath_crypto`]; secret *lookup* is by `(service, account)` key (not secret
...
//! 8 MiB-class cost in `ath_crypto`'s docs).
//!
//! The host KAT suite at the bottom is the FAIL-able proof
//! (`cargo test -p ath_keychain`): the load-bearing round-trip (binary/empty/long
...
use ath_crypto::{argon2id_derive, blake2b, chacha20poly1305};
...
/// `ath_crypto` (`max(1)` / `max(8*p)`), so even a degenerate header cannot
...
fn derive_salt_nonce(seed: &[u8]) -> ([u8; SALT_LEN], [u8; NONCE_LEN]) {
    ...
    salt_in.extend_from_slice(b"ath_keychain-salt");
    salt_in.extend_from_slice(seed);
    let mut nonce_in = Vec::with_capacity(seed.len() + 16);
    nonce_in.extend_from_slice(b"ath_keychain-nonce");
...
// ===========================================================================
// Host KAT suite — the FAIL-able proof (cargo test -p ath_keychain)
// ===========================================================================
