...
//! transport is abstract, so the deterministic host KAT drives it with raenet's
...
//! raenet APIs). `no_std + alloc`, never panics: every transport/HTTP error becomes a
//! [`LoadError`] the shell can render as an error page.

use alloc::string::String;
use alloc::vec::Vec;
use raenet::http1::{self, HttpTransport, Limits, Method};
...
/// Fetch `url` over the real `raenet::http1` client and classify the result.
///
...
mod loader_text_tests {
    ...
    impl HttpTransport for RoutingMock {
        fn connect(&mut self, _host: &str, _port: u16) -> raenet::http1::Http1Result<()> {
            Ok(())
        }
