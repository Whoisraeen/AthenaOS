...
//! the JS↔DOM bridge (it depends on both `raeweb` and `rae_js`), wrapping a `DomDocument` in
//! a `rae_js` host object whose `getElementById`/`textContent` calls land here. Keeping the
//! mutable primitive here (and the binding in the app) preserves the layering: `raeweb` never
//! depends on `rae_js`.
//!
...
impl DomDocument {
    ...
    /// `callback_id`. This populates raeweb's [`crate::EventListener`] seam so the engine's own
