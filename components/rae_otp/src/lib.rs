...
//! clock, or rate-limit verification — those belong to the caller (`raeid` / a
//! vault UI). All cryptography is delegated to [`rae_crypto`] (HMAC-SHA-1 /
...
//! SHA-256), plus base32 / verify / never-panic cases. Run `cargo test -p rae_otp`.
...
use rae_crypto::hmac::{hmac_sha1, hmac_sha256};
...
/// and [`rae_crypto`] does not yet expose HMAC-SHA-512. Adding it later is a
/// one-arm extension here plus the primitive in `rae_crypto`.
...
// ===========================================================================
// Host KAT suite — the FAIL-able proof (cargo test -p rae_otp)
// ===========================================================================
