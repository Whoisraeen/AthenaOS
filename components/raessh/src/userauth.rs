...
//!
//! Pure logic on `rae_crypto::ed25519`: parse a request, rebuild the exact
...
mod tests {
    ...
    /// happy path, and the signature genuinely verifies via rae_crypto.
    #[test]
    fn authorized_signed_attempt_succeeds() {
        ...
        let query = build_publickey_request("raeen", SERVICE_CONNECTION, &pubkey, None);
        ...
        let attempt = build_publickey_request("raeen", SERVICE_CONNECTION, &pubkey, Some(&sig));
    ...
    fn signature_from_another_session_fails() {
        ...
        let attempt = build_publickey_request("raeen", SERVICE_CONNECTION, &pubkey, Some(&sig_a));
        let req = parse_publickey_request(&attempt).unwrap();
        assert_eq!(
    ...
    fn wrong_service_fails() {
        ...
        .unwrap();
        let sig = ed25519::sign(&seed, &signed_data(&sid, &base));
        let attempt = build_publickey_request("raeen", "ssh-userauth", &pubkey, Some(&sig));
        let req = parse_publickey_request(&attempt).unwrap();
        assert_eq!(authenticate(&sid, &req, &authorized), AuthOutcome::Failure);
    }
