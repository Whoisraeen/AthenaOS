...
//! lays out HTML/CSS itself and paints through [`athgfx::Canvas`] (the same crisp-AA
//! path as every AthUI surface), with the resource loader riding the real
//! `athnet::http1` client. No JS in Phase 1 (the `EventListener` hooks stay inert) —
...
impl MediaQuery {
    ...
    /// (athweb renders to a screen), everything else (e.g. `print`) does not.
...
///
/// athweb resolves against a single GLOBAL custom-property pool (last declaration
...
/// a Length) since athgfx's gradient fill is row-interpolated (vertical) only.
...
/// Engine status for `/proc/athena/web` (procfs-style line, R10).
///
...
impl EngineStatus {
    pub fn render_line(&self) -> String {
        alloc::format!(
            "engine=athweb tabs={} last_url={} dom_nodes={} paint_cmds={} js=disabled",
            self.tabs,
...
pub struct RenderSmoketest {
    /// The document was fetched over the real `athnet::http1` path.
    pub fetched: bool,
    pub dom_nodes: usize,
    pub styled: usize,
    pub laid_out: usize,
...
/// modes:
///   * host KAT / boot smoketest: paint into a real [`athgfx::Canvas`].
///
/// It fetches a trivial styled HTML doc (a heading + a link) over the **real**
/// `athnet::http1` client via the supplied transport, runs the full DOM → cascade →
    ...
    P: FnMut(&DisplayList) -> backend::PaintStats,
{
    ...
    // 1. Fetch over the real athnet path (transport may be a mock or a live socket).
...
mod tests {
    ...
    /// Paint sink: a real athgfx Canvas backed by an owned buffer (so the test
    ...
    fn render_smoketest_passes_full_pipeline() {
        ...
        assert!(result.fetched, "fetch over athnet::http1 failed");
    ...
    fn engine_status_line_format() {
        ...
        assert!(line.contains("engine=athweb"));
    ...
    fn css_text_transform_applies() {
        ...
        // athweb fragments text into per-character nodes.
