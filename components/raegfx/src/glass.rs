...
//! framebuffer through `Canvas`). All token values come from `rae_tokens`
...
/// long-term home is the chrome tier's token recipe — route to raeen-ui when
/// `rae_tokens` frees.
...
/// `rae_tokens::glass_tier_interior` KAT exactly, so this draw, the kernel mirror,
...
/// decide.
///
/// `tier` is one of the three `rae_tokens::GLASS_*` tiers. The backdrop under the
...
/// weight `rae_tokens`'s private `mean_luma` uses for the legibility cap (Rec.709
/// 0.2126 / 0.7152 / 0.0722), so this render-side clamp lands on the identical line
/// the WCAG audit measures. `rae_tokens::mean_luma` / `clamp_interior_luma` are not
...
/// [`pixel_mean_luma`] ≤ `ceil`. Identical to `rae_tokens::clamp_interior_luma` but
...
/// change in the catalog itself is a `rae_tokens` edit — NOTE it to raeen-ui.)
#[inline]
...
mod tests {
    ...
    fn rim_renders_chromatic_pixels() {
        ...
        let (mut px, w, h) = fb(360, 360);
        {
            ...
            draw_glass_surface(&mut c, sx, sy, sw, sh, rad, rae_tokens::GLASS_PANEL_DARK);
        }
        ...
        let (mut px2, w2, _h2) = fb(360, 360);
        {
            ...
            c.fill_rounded_rect(sx, sy, sw, sh, rad, rae_tokens::GLASS_PANEL_DARK.tint);
    ...
    fn glass_thins_over_dark_aurora() {
        let (mut px, w, h) = fb(300, 200);
        {
            ...
            assert!(
                luma < rae_tokens::GLASS_LUMA_LO,
                "aurora base must read dark: {luma}"
            );
            let adj = glass_luma_adjust(rae_tokens::GLASS_PANEL_DARK, luma);
            let base_a = (rae_tokens::GLASS_PANEL_DARK.tint >> 24) & 0xFF;
    ...
    /// pins the bug raeen-ui found: the WCAG audit calls the capped `glass_tier_interior`
    ...
    fn glass_interior_capped_over_bright_not_over_dark() {
        ...
        let (mut bpx, bw, bh) = fb(200, 200);
        {
            ...
            draw_glass_surface(&mut c, 20, 20, 160, 160, 16, rae_tokens::GLASS_POPOVER_DARK);
        }
        ...
        let raw = rae_tokens::glass_tier_interior_raw(
    ...
    fn chrome_is_more_see_through_than_panel() {
        ...
        for &bk in backdrops.iter() {
            let (mut px, w, h) = fb(400, 200);
            {
                ...
                draw_glass_surface(&mut c, 220, 40, 160, 120, 14, rae_tokens::GLASS_PANEL_DARK);
            }
            let bkl = luma601(bk);
            ...
            chrome_dev_sum += chrome - luma601(rae_tokens::GLASS_CHROME_DARK.tint);
            panel_dev_sum += panel - luma601(rae_tokens::GLASS_PANEL_DARK.tint);
        }
    ...
    fn popover_white_text_clears_aa_over_any_bright_backdrop() {
            ...
            |px: &[u32], w: usize, x: usize, y: usize, sw: usize, sh: usize, rad: usize| -> f32 {
                ...
                for py in y..(y + sh) {
                    for pxn in x..(x + sw) {
                        ...
                        let cr = rae_tokens::contrast_ratio(TEXT_PRIMARY_DARK, p);
                        if cr < worst {
                            worst = cr;
                        }
                    }
        ...
        let (sx, sy, sw, sh, rad) = (30usize, 30usize, 300usize, 300usize, 14usize);
        {
            ...
            draw_glass_surface(&mut c, sx, sy, sw, sh, rad, rae_tokens::GLASS_POPOVER_DARK);
        }
        ...
        for &bk in &[
            ...
            let (mut bpx, bw, bh) = fb(160, 160);
            {
                ...
                draw_glass_surface(&mut c, 16, 16, 128, 128, 12, rae_tokens::GLASS_POPOVER_DARK);
            }
    ...
    fn start_popover_frost_carries_chroma() {
        ...
        let colored = 0xFF_1E_8C_A0u32;
        {
            ...
            draw_glass_surface(&mut c, 20, 20, 180, 180, 16, rae_tokens::GLASS_POPOVER_DARK);
        }
