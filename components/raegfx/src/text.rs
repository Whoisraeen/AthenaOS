...
//! black (the `raeui::text::blit_glyph` bug this path deliberately avoids).
...
use raefont::{FontHandle, RasterizedGlyph};
...
impl TextEngine {
    fn new() -> Self {
        ...
        // outlines, which raefont rasterizes directly. JetBrains Mono is static.
        let sans = FontHandle::from_bytes(raefont::builtin::rae_sans().to_vec());
        let mono = FontHandle::from_bytes(raefont::builtin::rae_mono().to_vec());
        TextEngine {
            sans,
            mono,
            cache: Vec::new(),
...
fn engine() -> Option<&'static mut TextEngine> {
    if ENGINE_INIT.load(Ordering::SeqCst) {
        ...
        // bounded cache. Matches the `raefont::FontEngine` global discipline.
...
impl Canvas {
    ...
    /// `raeui::text::blit_glyph`-against-black bug). Falls back to the 8×8 bitmap
...
/// (raegfx is `no_std` with no serial of its own).
