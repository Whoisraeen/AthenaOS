...
//!   1. Each bundled effect shader (`raegfx/shaders/*.wgsl`) compiles to SPIR-V.
...
//!
//! Run: `cargo test -p raegfx --features wgsl`
//!      (force the oracle with `RAEEN_SPIRV_VAL=1`).
#![cfg(feature = "wgsl")]

use raegfx::shader::{ShaderStage, SpirVExecutionModel, SpirVModule};
use raegfx::wgsl::{
...
fn try_spirv_val(spirv: &[u8], label: &str) {
    ...
    tmp.push(format!("raeen_wgsl_kat_{label}_{}.spv", std::process::id()));
    {
...
fn reflect_reports_glass_bindings() {
    let r = reflect_wgsl(raegfx::wgsl::GLASS_FS_WGSL).expect("reflect glass.fs");
...
fn reflect_fullscreen_has_no_bindings() {
    let r = reflect_wgsl(raegfx::wgsl::FULLSCREEN_VS_WGSL).expect("reflect fullscreen.vs");
    assert!(
        r.bindings.is_empty(),
...
fn reflect_vibe_single_uniform() {
    let r = reflect_wgsl(raegfx::wgsl::VIBE_TINT_FS_WGSL).expect("reflect vibe_tint.fs");
...
fn wrong_stage_is_error() {
    let r = compile_wgsl(raegfx::wgsl::GLASS_FS_WGSL, "fs_main", ShaderStage::Vertex);
...
fn corrupted_spirv_is_rejected_by_oracle() {
    ...
    let mut tmp = std::env::temp_dir();
    tmp.push(format!("raeen_wgsl_kat_corrupt_{}.spv", std::process::id()));
