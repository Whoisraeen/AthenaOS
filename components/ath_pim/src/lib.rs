...
//! (`cargo test -p ath_pim`): a 2-VEVENT `.ics` (exact UID/SUMMARY/DTSTART
...
BEGIN:VCALENDAR\r
VERSION:2.0\r
PRODID:-//AthenaOS//ath_pim//EN\r
BEGIN:VEVENT\r
UID:event-001@athena.os\r
...
STATUS:CONFIRMED\r
ORGANIZER:mailto:lead@athena.os\r
ATTENDEE:mailto:a@athena.os\r
ATTENDEE:mailto:b@athena.os\r
CATEGORIES:WORK,PLANNING\r
END:VEVENT\r
BEGIN:VEVENT\r
UID:event-002@athena.os\r
...
";
    ...
    fn ics_two_events_exact_fields() {
        ...
        assert_eq!(cal.prodid, "-//AthenaOS//ath_pim//EN");
        assert_eq!(cal.events.len(), 2);

        let e0 = &cal.events[0];
        assert_eq!(e0.uid, "event-001@athena.os");
        assert_eq!(e0.summary, "Sprint planning");
        assert_eq!(e0.status, "CONFIRMED");
        assert_eq!(e0.organizer, "mailto:lead@athena.os");
        assert_eq!(e0.attendees.len(), 2);
        assert_eq!(e0.attendees[1], "mailto:b@athena.os");
    ...
    fn ics_roundtrip_important_fields_failable() {
        ...
        assert_eq!(
            observed,
            "event-001@athena.os|Sprint planning|20260615T093000|Room 4, Building B"
        );
    }
