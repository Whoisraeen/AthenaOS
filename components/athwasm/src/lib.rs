//! athwasm — the sandboxed WebAssembly runtime for AthenaOS (Concept §AthGuard —
...
//! instead of panicking. The capability gate is the [`HostEnv`] trait: athwasm stays
...
/// capability check lives. athwasm stays kernel-free: it resolves an imported `call`
