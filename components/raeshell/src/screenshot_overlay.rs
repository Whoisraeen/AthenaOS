...
//! engine lives in the kernel compositor. All colors come from `rae_tokens` +
...
//! hardcoded `SELECTION_BORDER`. The scrim is `rae_tokens::SCRIM_CAPTURE`.
...
impl CaptureOverlay {
    ...
    fn render_dimensions_pill(
        ...
        let tw = canvas
            .measure_text_aa(&label, TYPE_CAPTION, raegfx::text::FontFamily::Sans)
            .max(0) as usize;
        let pill_h = 22usize;
    ...
    fn render_action_bar(
        ...
        for (i, (_, lbl)) in buttons.iter().enumerate() {
            let tw = canvas
                .measure_text_aa(lbl, TYPE_LABEL, raegfx::text::FontFamily::Sans)
                .max(0) as usize;
    ...
    fn render_hint(&self, canvas: &mut raegfx::Canvas) {
        let hint = "Drag to select a region  ·  Enter to capture  ·  Esc to cancel";
        let tw = canvas
            .measure_text_aa(hint, TYPE_LABEL, raegfx::text::FontFamily::Sans)
            .max(0) as usize;
