...
/// `raegfx` line-icon — the SAME set the rest of the chrome consumes (visual-QA
...
pub(crate) fn tray_line_icon(app_id: &str) -> raegfx::icon::Icon {
...
impl Taskbar {
    ...
    /// `rae_tokens` through the LIVE seed accent shared with Start/CC/Settings.
    pub fn render(&self, canvas: &mut raegfx::Canvas) {
        ...
        let line_top = |style: rae_tokens::TypeStyle| -> i32 {
        ...
        for btn in &self.task_buttons {
            ...
            // Real raegfx line-icon per running app (visual-QA Round-7 #1: retire
...
fn taskbar_identity_proof() -> TaskbarIdentityProof {
    ...
    let chrome_luma_dark = {
        ...
        {
            let mut c = unsafe { raegfx::Canvas::new(p.as_mut_ptr() as *mut u8, sw, sh, 4) };
...
mod identity_tests {
    ...
    fn taskbar_pills_and_tray_use_real_line_icons_not_letters() {
        ...
        // real raegfx Icon (the old R/F/W/T/M + N/V/B letters drew as initials).
        use raegfx::icon::Icon;
    ...
    fn taskbar_config_drops_legacy_opaque_glass_default() {
        ...
        let chrome = (rae_tokens::GLASS_CHROME_DARK.tint >> 24) & 0xFF;
        let panel = (rae_tokens::GLASS_PANEL_DARK.tint >> 24) & 0xFF;
        let popover = (rae_tokens::GLASS_POPOVER_DARK.tint >> 24) & 0xFF;
        assert!(
...
pub fn init() {
    ...
    unsafe {
        ...
        taskbar.pin_app("settings", "Settings", 'S', "/usr/bin/raesettings");
