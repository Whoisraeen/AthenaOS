...
impl SettingsCategory {
    ...
    /// The real `raegfx` line-icon for this nav category — replacing the `?`/emoji
    /// `char` placeholders the bitmap font can't render (visual-QA Round-7 #1).
    /// Reuses the SAME `raegfx::icon::Icon` set Control Center / Files consume
    ...
    pub fn line_icon(&self) -> raegfx::icon::Icon {
...
/// panels. The kernel pushes the raw `/proc/raeen/*` text via
...
/// banners), trimmed. Used to turn `/proc/version` / `/proc/raeen/hardware` into
...
/// Parse the live `/proc/raeen/*` dumps the kernel pushes into a render-ready
...
/// - `cpu`      ← `/proc/raeen/cpu`          → processor (brand/vendor)
/// - `smp`      ← `/proc/raeen/smp`          → logical-CPU count (`cpuN:` rows)
/// - `hardware` ← `/proc/raeen/hardware`     → board/DMI
/// - `memory`   ← `/proc/raeen/memory`       → `physical_total_bytes`
/// - `storage`  ← `/proc/raeen/storage`      → `mounted:` gate + total/free/used
pub fn parse_system_info(
    ...
    // The kernel's /proc/raeen/smp emits one `cpuN: ticks=.. task_picks=.. ..`
...
pub struct ControlPanel {
    ...
    /// Live About/Storage facts (parsed from `/proc/raeen/*`, pushed by the
...
// ── Design tokens (docs/design/settings.md + design-language.md, rae_tokens) ─
//
...
/// swap is the one remaining value change here. No `rae_abi` dependency is
/// needed — the seed flows through the crate-local `ACTIVE_ACCENT` static.
#[inline]
fn cp_palette() -> &'static rae_tokens::Palette {
    &rae_tokens::DARK
}
...
#[inline]
fn cp_accent() -> rae_tokens::AccentRamp {
    rae_tokens::derive_accent(cp_accent_seed(), cp_palette())
}
...
fn draw_content_card(
    ...
    w: usize,
    h: usize,
    elev: rae_tokens::Elevation,
) {
    if w == 0 || h == 0 {
        return;
    }
    let r = rae_tokens::RADIUS_MD as usize;
    ...
    raegfx::glass::draw_glass_surface(canvas, x, y, w, h, r, rae_tokens::GLASS_POPOVER_DARK);
}

/// Opaque per-channel blend `a*(den-num)/den + b*num/den` (matches
/// raeshell::lib `blend_opaque`; duplicated here only because that helper is
...
impl ControlPanel {
    pub fn render(&self, canvas: &mut raegfx::Canvas, ox: usize, oy: usize, w: usize, h: usize) {
        ...
        use rae_tokens::{RADIUS_LG, SPACE_4, SPACE_5, SPACE_6};
        ...
        canvas.fill_rounded_rect_shadow(
            ...
            h,
            win_r,
            rae_tokens::ELEV_5.color,
            rae_tokens::ELEV_5.radius as usize,
            rae_tokens::ELEV_5.offset_y,
        );
        ...
        canvas.draw_text_aa(
            ...
            (oy + (titlebar_h.saturating_sub(rae_tokens::TYPE_SUBTITLE.line_height as usize)) / 2)
                as i32,
            "Settings",
        ...
        // Close affordance — REAL raegfx Close icon (spec bullet 8: no tofu glyph),
        // resting text.secondary. 16px, centred on the 32px title bar.
        let close_sz = 16usize;
        canvas.draw_icon(
            raegfx::icon::Icon::Close,
    ...
    fn render_sidebar(
        ...
        use rae_tokens::{RADIUS_SM, RADIUS_XS, SPACE_1, SPACE_2, SPACE_3, SPACE_4};
        let p = cp_palette();
        ...
        let field_h = rae_tokens::HIT_TARGET_POINTER as usize;
        let fx = sx + pad;
        let fy = sy + pad;
        ...
        // Search magnifier — REAL raegfx line-icon (spec bullet 8: no tofu glyph;
        ...
        let field_text_y =
            fy + (field_h.saturating_sub(rae_tokens::TYPE_BODY.line_height as usize)) / 2;
        canvas.draw_text_aa(
        ...
        for cat in SettingsCategory::all().iter() {
            ...
            // Real raegfx line-icon (visual-QA Round-7 #1: retire the emoji-char
            ...
            let label_y =
                iy + (row_h.saturating_sub(rae_tokens::TYPE_LABEL.line_height as usize)) / 2;
            canvas.draw_text_aa(
    ...
    fn render_category_landing(
        ...
        use rae_tokens::{SPACE_3, SPACE_4, SPACE_5};
        let p = cp_palette();
        let accent = cp_accent();
        ...
        if let Some(cat) = self.navigation.current_category {
            ...
            for page in pages.iter() {
                ...
                draw_content_card(canvas, cx, py, cw, card_h, rae_tokens::ELEV_1);
                let gy = py + SPACE_4 as usize;
                // Leading icon — REAL raegfx line-icon for the category (the page's
    ...
    fn render_page(
        ...
        use rae_tokens::{SPACE_4, SPACE_5};
        let p = cp_palette();
        ...
        canvas.draw_text_aa(
            cx as i32,
            (title_y + 26) as i32,
            fit_text_aa(canvas, &page.description, cw, rae_tokens::TYPE_BODY),
            rae_tokens::TYPE_BODY,
            p.text_secondary,
            raegfx::text::FontFamily::Sans,
        );
        ...
        // `/proc/raeen/*` facts as glass cards above their (few) controls.
        ...
        draw_content_card(canvas, card_x, card_y, card_w, card_h, rae_tokens::ELEV_2);
    ...
    /// a System label/value card sourced from the live `/proc/raeen/*` push
    ...
    fn render_about_panel(
        ...
        use rae_tokens::{SPACE_2, SPACE_3, SPACE_4};
        let p = cp_palette();
        let accent = cp_accent();
        ...
        if cy + hero_h <= cy + ch {
            ...
            canvas.draw_text_aa(
                tx,
                (cy + SPACE_3 as usize + rae_tokens::TYPE_SUBTITLE.line_height as usize + 2) as i32,
                board_line,
                rae_tokens::TYPE_CAPTION,
                cp_card_caption_ink(), // in-card caption (popover plane)
                raegfx::text::FontFamily::Sans,
        ...
        draw_content_card(canvas, cx, card_y, cw, card_h, rae_tokens::ELEV_1);
        ...
        for (label, value) in rows.iter() {
            ...
            first = false;
            let ty = (ry + (row_h.saturating_sub(rae_tokens::TYPE_BODY.line_height as usize)) / 2)
                as i32;
            canvas.draw_text_aa(
                (cx + inner) as i32,
                ty,
            ...
            let value = fit_text_aa(canvas, value, val_avail, rae_tokens::TYPE_BODY);
            canvas.draw_text_aa(
    ...
    fn render_storage_panel(
        ...
        use rae_tokens::{radius_pill, RADIUS_MD, RADIUS_XS, SPACE_2, SPACE_3, SPACE_4};
        let p = cp_palette();
        let accent = cp_accent();
        let info = &self.system_info;
        let inner = SPACE_4 as usize;
        ...
        let cap_h = 72usize;
        draw_content_card(canvas, cx, cy, cw, cap_h, rae_tokens::ELEV_2);
        ...
        if !visible.is_empty() && ctrl_y + row_h + 2 * inner <= cy + ch {
            ...
            draw_content_card(canvas, cx, ctrl_y, cw, ctrl_h, rae_tokens::ELEV_2);
    ...
    fn render_setting_row(
        ...
        use rae_tokens::{RADIUS_SM, RADIUS_XS, SPACE_1, SPACE_2, SPACE_3};
        let p = cp_palette();
        let accent = cp_accent();
        ...
        let label_y = if item.description.is_empty() {
            ry + (rh.saturating_sub(rae_tokens::TYPE_LABEL.line_height as usize)) / 2
        } else {
            ry + SPACE_2 as usize
        };
        canvas.draw_text_aa(
        ...
        if !item.description.is_empty() {
            ...
            let desc = fit_text_aa(canvas, &item.description, rw / 2, rae_tokens::TYPE_CAPTION);
            canvas.draw_text_aa(
                rx as i32,
                (label_y + 16) as i32,
                desc,
        ...
        match &item.control {
            ...
            SettingControl::Slider {
                ...
                canvas.draw_text_aa(
                    vs_x,
                    (cy_mid.saturating_sub(rae_tokens::TYPE_CAPTION.line_height as usize / 2))
                        as i32,
                    vs,
                    rae_tokens::TYPE_CAPTION,
                    p.text_secondary,
            ...
            | SettingControl::RadioGroup { selected, options } => {
                ...
                if let Some(opt) = options.get(*selected) {
                    canvas.draw_text_aa(
                        (fx + SPACE_2 as usize) as i32,
                        (fy + (field_h.saturating_sub(rae_tokens::TYPE_BODY.line_height as usize))
                            / 2) as i32,
                        fit_text_aa(
                            canvas,
                            opt,
            ...
            SettingControl::Button { label, .. } => {
                ...
                let bh = rae_tokens::HIT_TARGET_POINTER as usize;
                ...
                canvas.draw_text_aa(
                    ...
                    (by + (bh.saturating_sub(rae_tokens::TYPE_LABEL.line_height as usize)) / 2)
                        as i32,
                    label,
                    rae_tokens::TYPE_LABEL,
                    if disabled {
            ...
            SettingControl::Link { label, .. } => {
                ...
                canvas.draw_text_aa(
                    lx,
                    (cy_mid.saturating_sub(rae_tokens::TYPE_LABEL.line_height as usize / 2)) as i32,
                    label,
                    rae_tokens::TYPE_LABEL,
                    accent.text,
                    raegfx::text::FontFamily::Sans,
            ...
            SettingControl::CheckboxGroup { items } => {
                ...
                let suffix = " selected";
                let ty = (cy_mid.saturating_sub(rae_tokens::TYPE_CAPTION.line_height as usize / 2))
                    as i32;
            ...
            SettingControl::KeyBinding { display, .. } => {
                ...
                let chip_h = rae_tokens::TYPE_CAPTION.line_height as usize + SPACE_2 as usize;
                ...
                canvas.draw_text_aa(
                    (chip_x + SPACE_2 as usize) as i32,
                    (chip_y
                        + (chip_h.saturating_sub(rae_tokens::TYPE_CAPTION.line_height as usize))
                            / 2) as i32,
                    display,
                    rae_tokens::TYPE_CAPTION,
                    p.text_primary,
            ...
            SettingControl::InfoBar { message, severity } => {
                ...
                canvas.draw_text_aa(
                    ...
                    (cy_mid.saturating_sub(rae_tokens::TYPE_BODY.line_height as usize / 2)) as i32,
                    fit_text_aa(
                        canvas,
                        message,
    ...
    fn render_breadcrumb(&self, canvas: &mut raegfx::Canvas, bx: usize, by: usize, _bw: usize) {
        use rae_tokens::SPACE_2;
        let p = cp_palette();
    ...
    fn render_search_overlay(
        ...
        use rae_tokens::{RADIUS_MD, RADIUS_XS, SPACE_2, SPACE_3, SPACE_4};
        let p = cp_palette();
        let accent = cp_accent();
        let ox2 = cx;
        ...
        canvas.fill_rounded_rect_shadow(
            ...
            oh,
            RADIUS_MD as usize,
            rae_tokens::ELEV_3.color,
            rae_tokens::ELEV_3.radius as usize,
            rae_tokens::ELEV_3.offset_y,
        );
        raegfx::glass::draw_glass_surface(
            canvas,
            ox2,
        ...
        if self.search.results.is_empty() {
            ...
            canvas.draw_text_aa(
                (ox2 + SPACE_4 as usize) as i32,
                (oy2 + SPACE_4 as usize + rae_tokens::TYPE_SUBTITLE.line_height as usize + 4)
                    as i32,
        ...
        for (idx, result) in self.search.results.iter().enumerate() {
            ...
            canvas.draw_text_aa(
                text_x,
                (ry + 6 + rae_tokens::TYPE_BODY.line_height as usize + 2) as i32,
                crumb,
                rae_tokens::TYPE_CAPTION,
                cp_card_caption_ink(), // over the popover overlay plane
                raegfx::text::FontFamily::Sans,
    ...
    fn draw_label_with_match(
        ...
        let advance = |s: &str| -> i32 {
            ...
            (s.chars().count() as i32) * (rae_tokens::TYPE_BODY.px as i32 * 6 / 10).max(1)
        };
        match span {
    ...
    fn render_appearance_showcase(
        ...
        use rae_tokens::{RADIUS_SM, SPACE_2, SPACE_3, SPACE_4};
        let p = cp_palette();
        let accent = cp_accent();
        if ch < 48 {
            return;
        ...
        for (i, (label, col)) in ramp.iter().enumerate() {
            ...
            canvas.draw_text_aa(
                ...
                fit_text_aa(canvas, label, chip_w, rae_tokens::TYPE_CAPTION),
                rae_tokens::TYPE_CAPTION,
                cp_caption_ink(),
                raegfx::text::FontFamily::Sans,
            );
        ...
        for (i, preset) in presets.iter().enumerate() {
            ...
            draw_content_card(canvas, tx, ty, tile_w, tile_h, rae_tokens::ELEV_1);
            // Mini swatch trio.
            let sw_r = 6usize;
            ...
            canvas.draw_text_aa(
                ...
                (sw_cy.saturating_sub(rae_tokens::TYPE_LABEL.line_height as usize / 2)) as i32,
                fit_text_aa(
                    canvas,
                    name,
...
fn fit_text_aa<'a>(
    ...
    let sans = raegfx::text::FontFamily::Sans;
...
/// Push the live `/proc/raeen/*` dumps into the Settings About + Storage panels
...
pub fn settings_design_proof() -> SettingsDesignProof {
    let want_accent = rae_tokens::derive_accent(cp_accent_seed(), cp_palette()).base;
    ...
    let card_radius = rae_tokens::RADIUS_MD;
    let window_radius = rae_tokens::RADIUS_LG;
    ...
    let pass = accent_base == want_accent
        ...
        && card_radius == rae_tokens::RADIUS_MD
        && window_radius == rae_tokens::RADIUS_LG
        && vibe_tiles > 0;
    SettingsDesignProof {
        panes: 2,
        accent_base,
...
/// the kernel's `/proc/raeen/*` push). Single authority for the asserted values.
...
mod design_tests {
    ...
    fn settings_chrome_text_is_raesans_proportional_not_mono_bitmap() {
        ...
        let c = unsafe { raegfx::Canvas::new(px.as_mut_ptr() as *mut u8, sw, sh, 4) };
        let narrow = c.measure_text_aa("iiii", rae_tokens::TYPE_LABEL, FontFamily::Sans);
        let wide = c.measure_text_aa("WWWW", rae_tokens::TYPE_LABEL, FontFamily::Sans);
        assert!(
            wide > narrow,
        ...
        let wfit = fit_text_aa(&c, "WWWWWWWW", budget, rae_tokens::TYPE_LABEL);
        let ifit = fit_text_aa(&c, "iiiiiiii", budget, rae_tokens::TYPE_LABEL);
        assert!(
            ifit.len() > wfit.len(),
        ...
        let mut c = unsafe { raegfx::Canvas::new(px.as_mut_ptr() as *mut u8, sw, sh, 4) };
    ...
    fn accent_is_token_derived_not_hardcoded() {
        ...
        let want = rae_tokens::derive_accent(cp_accent_seed(), cp_palette()).base;
        assert_eq!(
            cp_accent().base,
            want,
        ...
        assert_eq!(want, rae_tokens::RAEBLUE, "default seed is RaeBlue");
    }
    ...
    fn settings_uses_three_distinct_translucency_planes() {
        ...
        // as different planes.
        let panel_a = (rae_tokens::GLASS_PANEL_DARK.tint >> 24) & 0xFF;
        let popover_a = (rae_tokens::GLASS_POPOVER_DARK.tint >> 24) & 0xFF;
        assert!(
    ...
    fn proof_passes_when_wired() {
        ...
        assert_eq!(proof.card_radius, rae_tokens::RADIUS_MD);
        assert_eq!(proof.window_radius, rae_tokens::RADIUS_LG);
    }
    ...
    fn nav_rows_map_to_real_line_icons_not_letters() {
        // visual-QA Round-7 #1: every Settings nav row must resolve a real raegfx
    ...
    fn cp_palette_consts_are_retired() {
        ...
        assert_eq!(
            cp_accent().subtle,
            rae_tokens::derive_accent(rae_tokens::RAEBLUE, &rae_tokens::DARK).subtle
        );
    }
}
