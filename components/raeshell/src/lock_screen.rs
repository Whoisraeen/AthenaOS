...
impl LockScreen {
    ...
    pub fn render(&self, canvas: &mut [u32], stride: usize) {
        ...
        let mut c =
            unsafe { raegfx::Canvas::new(canvas.as_mut_ptr() as *mut u8, stride, height, 4) };
        self.render_canvas(&mut c);
    }

    /// Glass-based render through a [`raegfx::Canvas`] — host-callable (the UI
    /// screenshot harness drives this over a `HostFb`).
    pub fn render_canvas(&self, c: &mut raegfx::Canvas) {
        ...
        let accent = rae_tokens::derive_accent(crate::active_accent(), p);
        ...
        let clock_style = rae_tokens::TYPE_DISPLAY;
        ...
        if self.clock.show_date {
            ...
            let dstyle = rae_tokens::TYPE_CAPTION;
        ...
        let istyle = rae_tokens::TYPE_TITLE;
        ...
        let nstyle = rae_tokens::TYPE_LABEL;
        ...
        match &self.auth_method {
            ...
            _ => {
                let pstyle = rae_tokens::TYPE_BODY;
        ...
        let status_style = rae_tokens::TYPE_CAPTION;
...
pub struct PasskeyCredential {
    ...
    /// `rae_crypto::p256_ecdsa::verify` accepts. This is what an assertion is
...
impl PasskeyManager {
    ...
    pub fn verify_response(
        ...
        let ok = rae_crypto::p256_ecdsa::verify(&cred.public_key, &signed, signature);
...
// flat hardcoded-hex render. These run on the host (`cargo test -p raeshell`)
// over a raw ARGB buffer wrapped in the SAME `raegfx::Canvas` the kernel uses.
#[cfg(test)]
mod glass_identity_tests {
    ...
    fn lock_time_renders_proportional_raesans_not_block_glyph() {
        ...
        let narrow = c.measure_text_aa("iiii", rae_tokens::TYPE_DISPLAY, FontFamily::Sans);
        let wide = c.measure_text_aa("WWWW", rae_tokens::TYPE_DISPLAY, FontFamily::Sans);
        assert!(
            wide > narrow,
    ...
    fn lock_card_uses_popover_glass_tier() {
        ...
        let tier = rae_tokens::GLASS_POPOVER_DARK;
