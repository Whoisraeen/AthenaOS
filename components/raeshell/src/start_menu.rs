...
/// CC-local helper (`rae_tokens::with_alpha` is private to that crate).
#[inline]
...
/// `glass.popover` interior). Pure token math (mirrors `rae_tokens`' tier model)
/// so the raised-card KAT can compare a tile face against the gap it floats above.
fn start_gap_interior(backdrop: u32) -> u32 {
    let t = rae_tokens::GLASS_POPOVER_DARK;
    rae_tokens::flatten_over(t.frost, rae_tokens::flatten_over(t.tint, backdrop))
}
...
/// `rae_tokens` private `mean_luma` (kept private to that crate).
...
/// Map a Start-menu app (by id + name) to the closest shipped `raegfx` line-icon
...
pub(crate) fn app_line_icon(app_id: &str, name: &str) -> raegfx::icon::Icon {
...
pub(crate) fn entry_line_icon(name: &str, path: &str) -> raegfx::icon::Icon {
    use raegfx::icon::Icon;
...
/// Map a `ContextAction` to the closest shipped `raegfx` line-icon — the SAME
...
pub(crate) fn context_action_icon(action: ContextAction) -> raegfx::icon::Icon {
    use raegfx::icon::Icon;
    match action {
...
impl ContextMenu {
    ...
    pub fn relayout(&mut self) {
        let label_style = rae_tokens::TYPE_LABEL;
        let caption_style = rae_tokens::TYPE_CAPTION;
    ...
    /// `raegfx` line-icon and an optional right-aligned `type.caption` shortcut
    ...
    pub fn render(&self, canvas: &mut raegfx::Canvas, x: i32, y: i32) {
        ...
        let radius = rae_tokens::RADIUS_MD as usize;
        ...
        for (i, it) in self.items.iter().enumerate() {
            ...
            // Leading line-icon (real raegfx vector, never a letter).
...
impl StartMenuTheme {
    /// Dark theme, derived from `rae_tokens` through the LIVE seed accent (Vibe
    ...
    pub fn dark() -> Self {
        ...
        Self {
            mode: ThemeMode::Dark,
            background_color: rae_tokens::GLASS_POPOVER_DARK.tint,
            text_color: p.text_primary,
            accent_color: a.base,
            hover_color: rae_tokens::GLASS_POPOVER_DARK.frost,
            selected_color: a.subtle,
            dimmed_color: p.text_tertiary,
            border_color: a.base,
            blur_enabled: true,
            transparency: 0.85,
            corner_radius: rae_tokens::RADIUS_LG,
        }
    }
...
impl StartMenu {
    ...
    pub fn render(&self, canvas: &mut raegfx::Canvas) {
        ...
        let heading_style = rae_tokens::TYPE_SUBTITLE;
        ...
        let _ = on_accent; // (selection ink used in tile/row branches below)
                           // Leading magnifier — a REAL raegfx line-icon (Round-8 residual icon gap:
        ...
        match self.active_section {
            StartMenuSection::Search => {
                ...
                for (i, result) in self.search_engine.results.iter().take(10).enumerate() {
                    ...
                    let ricon = match result.kind {
                        ...
                        SearchResultKind::Setting => raegfx::icon::Icon::Gear,
                        SearchResultKind::Web => raegfx::icon::Icon::WiFi,
                        SearchResultKind::Calculator => raegfx::icon::Icon::Doc,
                    };
            ...
            _ => {
                ...
                for (i, app) in pinned.iter().take(max_tiles).enumerate() {
                    ...
                    let tile_w = cell_w - 4;
                    let tile_h = cell_h - 4;
                    let tile_r = rae_tokens::RADIUS_MD as usize;
                    ...
                    // Real raegfx line-icon (Round-7 #1) instead of the accent
                ...
                if rec_y + heading_lh < body_bottom {
                    ...
                    for (i, rec) in self.recommended.iter().take(4).enumerate() {
                        ...
                        let row_w = w - 24;
                        let row_h = entry_h - 4;
                        let row_r = rae_tokens::RADIUS_SM as usize;
                        canvas.fill_rounded_rect_shadow(
                            row_x,
                            ey,
                            row_w,
        ...
        canvas.draw_icon(
            raegfx::icon::Icon::Accessibility,
            (av_x + 5) as i32,
            (av_y + 5) as i32,
            (av_d - 10) as i32,
            t.text_color,
...
mod identity_tests {
    ...
    fn start_tiles_map_to_real_line_icons_not_letters() {
        // visual-QA Round-7 #1: each pinned app tile must resolve a real raegfx
    ...
    fn start_theme_derives_from_tokens_not_hardcoded_hex() {
        // The dark theme must resolve from rae_tokens (the glass.popover tier +
    ...
    fn start_uses_translucent_popover_glass() {
        ...
        let tint = rae_tokens::GLASS_POPOVER_DARK.tint;
        let a = (tint >> 24) & 0xFF;
        assert!(
    ...
    fn start_search_field_uses_real_magnifier_icon_not_question_mark() {
        ...
        // bitmap font's missing-glyph box). The field must draw a REAL raegfx line
    ...
    fn context_menu_is_glass_with_separators_and_real_icons() {
        ...
        //   (3) every item maps to a REAL raegfx line-icon, never a letter glyph.
    ...
    fn render_honors_position_and_clamps() {
        ...
        {
            let mut canvas = unsafe { raegfx::Canvas::new(px.as_mut_ptr() as *mut u8, sw, sh, 4) };
    ...
    fn context_menu_renders_glass_and_accent_hover_over_backdrop() {
        ...
        assert!(raegfx::text::ensure_init());
        ...
        {
            let mut c = unsafe { raegfx::Canvas::new(px.as_mut_ptr() as *mut u8, w, h, 4) };
            m.render(&mut c, 12, 12);
        }
        let a = rae_tokens::derive_accent(crate::active_accent(), crate::active_palette());
