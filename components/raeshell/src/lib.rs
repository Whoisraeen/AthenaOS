...
// from ONE seed accent flowed through the shared `rae_tokens` palette, so a
// Vibe-Mode re-skin (one seed change) recolours the whole shell coherently —
// the duplication this crate was built to kill (rae_tokens crate docstring).
...
/// `rae_tokens::active_palette()` (the kernel a11y on-switch flips the flag).
#[inline]
pub fn active_palette() -> &'static rae_tokens::Palette {
    rae_tokens::active_palette()
}
...
/// place" promise and the payoff of the rae_tokens system.
///
...
static ACTIVE_ACCENT: core::sync::atomic::AtomicU32 =
    core::sync::atomic::AtomicU32::new(rae_tokens::RAEBLUE);
...
fn accent() -> rae_tokens::AccentRamp {
    rae_tokens::derive_accent(active_accent(), PALETTE)
}
...
impl StartMenu {
    ...
    pub fn new(screen_width: usize, screen_height: usize) -> Self {
        ...
        let inset = rae_tokens::SPACE_2 as usize;
        let avail_h = screen_height
            .saturating_sub(TASKBAR_HEIGHT)
            .saturating_sub(rae_tokens::SPACE_4 as usize)
    ...
    pub fn render(&self, canvas: &mut raegfx::Canvas) {
        ...
        use rae_tokens::{RADIUS_LG, RADIUS_SM, SPACE_2, SPACE_3, SPACE_4};
        let accent = accent();
        let p = PALETTE;
        ...
        // `raegfx::glass::draw_glass_surface`, which composites the FULL stack:
        ...
        canvas.draw_text_aa(
            ...
            (search_y + (search_h.saturating_sub(rae_tokens::TYPE_BODY.line_height as usize)) / 2)
                as i32,
            search_text,
            ...
            .enumerate()
        {
            ...
            let name_y = item_y
                + (item_height.saturating_sub(rae_tokens::TYPE_LABEL.line_height as usize)) / 2;
            canvas.draw_text_aa(
            ...
            if !cat_str.is_empty() {
                ...
                let tag_y = item_y
                    + (item_height.saturating_sub(rae_tokens::TYPE_CAPTION.line_height as usize))
                        / 2;
                canvas.draw_text_aa(
                    tag_x,
                    tag_y as i32,
...
impl SystemTray {
    ...
    pub fn total_width(&self) -> usize {
        let gap = rae_tokens::SPACE_1 as usize;
        let icons_w = self.icons.len() * (TRAY_ICON_SIZE + gap);
        let clock_w = self.clock_text.len() * GLYPH_W + 2 * rae_tokens::SPACE_2 as usize;
        icons_w + clock_w + rae_tokens::SPACE_3 as usize
    }
    ...
    pub fn render(&self, canvas: &mut raegfx::Canvas, x: usize, y: usize, height: usize) {
        let p = PALETTE;
        let gap = rae_tokens::SPACE_1 as usize;
        ...
        cx += rae_tokens::SPACE_2 as usize;
        let clock_y =
            y + (height.saturating_sub(rae_tokens::TYPE_CAPTION.line_height as usize)) / 2;
        canvas.draw_text_aa(
            cx as i32,
            clock_y as i32,
            &self.clock_text,
...
impl NotificationCenter {
    ...
    pub fn render_toasts(&self, canvas: &mut raegfx::Canvas, screen_width: usize) {
        ...
        use rae_tokens::{RADIUS_MD, SPACE_2, SPACE_4};
        let accent = accent();
        let p = PALETTE;
...
impl SettingsPanel {
    ...
    pub fn render(&self, canvas: &mut raegfx::Canvas) {
        ...
        canvas.draw_text_aa(
            r.x + 12,
            r.y + (32 - rae_tokens::TYPE_TITLE.line_height as i32) / 2,
            "Settings",
        ...
        canvas.draw_text_aa(
            r.x + r.w as i32 - 20,
            r.y + (32 - rae_tokens::TYPE_LABEL.line_height as i32) / 2,
            "x",
            rae_tokens::TYPE_LABEL,
            dimmed_fg,
            raegfx::text::FontFamily::Sans,
        ...
        for (i, (page, label)) in pages.iter().enumerate() {
            ...
            let label_y = item_y + (28 - rae_tokens::TYPE_LABEL.line_height as usize) / 2;
...
impl DesktopShell {
    ...
    pub fn taskbar_tray_x(&self) -> usize {
        self.screen_width
            .saturating_sub(self.system_tray.total_width() + rae_tokens::SPACE_2 as usize)
    }
    ...
    pub fn render(&self, canvas: &mut raegfx::Canvas) {
        ...
        raegfx::glass::draw_glass_surface_docked(
            canvas,
            tb_x,
            tb_y,
            tb_w,
        ...
        let pill_r = rae_tokens::radius_pill(sr.h) as usize;
        ...
        for (i, btn) in self.taskbar_buttons().iter().enumerate() {
            ...
            if btn.focused {
                ...
                canvas.fill_rounded_rect_shadow(
                    ...
                    rae_tokens::GLOW_ACCENT_BLUR as usize,
                    0,
                );
                canvas.fill_rounded_rect(
                    bx,
...
pub fn shell_design_proof() -> ShellDesignProof {
    let want_accent = rae_tokens::derive_accent(active_accent(), PALETTE).base;
...
mod design_tests {
    ...
    fn accent_is_token_derived_not_hardcoded() {
        ...
        let want = rae_tokens::derive_accent(active_accent(), PALETTE).base;
        assert_eq!(
            accent().base,
            want,
        ...
        assert_eq!(want, rae_tokens::RAEBLUE);
    }
    ...
    fn accent_tracks_the_live_seed() {
        ...
        let ramp = rae_tokens::derive_accent(ORANGE, PALETTE);
        ...
        assert_eq!(active_accent(), rae_tokens::RAEBLUE);
        assert_eq!(accent().base, rae_tokens::RAEBLUE);
    }
    ...
    fn start_menu_panel_is_visible_over_bright_aurora() {
        ...
        let band_y = ry + (rae_tokens::SPACE_4 as usize) + 40 + (rae_tokens::SPACE_4 as usize) + 4;
        ...
        let cr = rae_tokens::contrast_ratio(PALETTE.text_primary, interior_px);
        assert!(
            cr >= 4.5,
    ...
    fn proof_passes_when_wired() {
        ...
        assert_eq!(proof.accent_base, rae_tokens::RAEBLUE);
    }
}
