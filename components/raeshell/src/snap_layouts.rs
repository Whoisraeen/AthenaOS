...
//! correctness is proven by a host KAT (`cargo test -p raeshell`) and its look
...
impl SnapOverlay {
    ...
    fn grid(&self) -> Grid {
        let cols = 3usize;
        let gap = rae_tokens::SPACE_3 as usize; // 12
        let pad = rae_tokens::SPACE_4 as usize; // 16
        let title_h = rae_tokens::SPACE_5 as usize; // 24
