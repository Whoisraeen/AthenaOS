...
// backdrop (visual-QA Round-4 P0 #2 — they were dark raeshell slates punched into
...
fn draw_frosted_card(
    ...
    canvas.fill_rounded_rect(x, y, w, h, radius, rae_tokens::GLASS_POPOVER_DARK.tint);
    canvas.fill_rounded_rect(x, y, w, h, radius, rae_tokens::GLASS_POPOVER_DARK.frost);
}

/// Replace the alpha channel of an ARGB colour, keeping RGB (surface-local; the
/// rae_tokens `with_alpha` is private). Used for the critical-toast glow halo.
#[inline]
...
const ND_RADIUS: usize = rae_tokens::RADIUS_LG as usize;

// ── Rendering ────────────────────────────────────────────────────────────

impl NotificationDaemon {
    pub fn render_toasts(&self, canvas: &mut raegfx::Canvas) {
        ...
        for (notif, geo) in &toasts {
            ...
            // the Round-4 lift). raeen-ui confirmed: PROMOTE to `text.primary` over
            ...
            if let Some(progress) = notif.progress {
                let bar_y = y + h - 8;
                let bar_w = w - 24;
                let bar_r = rae_tokens::radius_pill(4) as usize;
            ...
            if !notif.actions.is_empty() {
                ...
                for action in notif.actions.iter().take(3) {
                    ...
                    let pill_r = rae_tokens::radius_pill(ah as u32) as usize;
    ...
    pub fn render_center(
        ...
        canvas.fill_rounded_rect(ox, oy, w, h, ND_RADIUS, rae_tokens::GLASS_PANEL_DARK.tint);
        canvas.fill_rounded_rect(ox, oy, w, h, ND_RADIUS, rae_tokens::GLASS_PANEL_DARK.frost);
        ...
        for (i, notif) in items.iter().take(max_visible).enumerate() {
            ...
            let frost = if notif.read {
                rae_tokens::GLASS_CHROME_DARK.frost
            } else {
            ...
            if !notif.body.is_empty() {
                ...
                // chrome frost) where secondary fails 4.5:1. raeen-ui: promote, don't
                // darken the token.
                canvas.draw_text_aa(
                    (ox + 14) as i32,
                    (ny + 36) as i32,
