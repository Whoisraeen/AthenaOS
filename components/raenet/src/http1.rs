...
//!   raenet/raekit TCP socket + DNS resolve when that wrapper lands.
//!
//! HTTPS/TLS is a DEFERRED follow-up: TLS 1.3 building blocks exist behind the
//! crate's `tls13` feature (`raenet::tls_crypto`), but a full handshake-driving
...
/// bomb-bounded `rae_deflate` decoder; `identity` (and an absent header) pass the
...
fn decode_one_coding(coding: &str, body: Vec<u8>, limits: &Limits) -> Http1Result<Vec<u8>> {
    let decoded = if coding.eq_ignore_ascii_case("identity") {
        ...
        rae_deflate::gzip_decompress(&body).map_err(|_| Http1Error::BadContentEncoding)?
        ...
        match rae_deflate::zlib_decompress(&body) {
            Ok(v) => v,
            Err(_) => rae_deflate::inflate(&body).map_err(|_| Http1Error::BadContentEncoding)?,
        }
    ...
    // Defense in depth: rae_deflate already caps output, but enforce the response
...
/// wraps the raenet/raekit TCP socket + DNS resolve once a userspace socket
...
/// raenet/raekit socket adapter must satisfy.
#[derive(Debug, Clone)]
...
// ---------------------------------------------------------------------------
// Tests — host KATs (cargo test -p raenet)
// ---------------------------------------------------------------------------
...
\r\n"
    ...
    /// compatibility, independent of rae_deflate's own writer.
    #[test]
    ...
    fn deflate_content_encoding_decoded() {
        ...
        let z = rae_deflate::zlib_compress(plain);
        let mut raw: Vec<u8> = Vec::new();
    ...
    fn corrupt_gzip_body_is_error_not_panic() {
        ...
        let mut gz = rae_deflate::gzip_compress(plain);
