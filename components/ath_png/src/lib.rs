...
//! the from-scratch PNG path the Photos viewer and `athplay` thumbnailer sit on.
//!
//! Output is a flat ARGB8888 `Vec<u32>` (`0xAARRGGBB`) — the AthGFX compositor /
//! Canvas pixel format, **matching the [`ath_bmp`]/[`ath_gif`] decoders** so a
...
//! - `IDAT` concatenation → zlib inflate via [`ath_deflate::zlib_decompress`] →
...
//! types; `IDAT` is zlib via [`ath_deflate::zlib_compress`]. The encode→decode
...
/// identical to [`ath_bmp::BmpImage`] / a `ath_gif` frame so callers consume all
...
// ─── CRC-32 (PNG uses the same IEEE polynomial as zlib; reuse ath_deflate) ───
//
// ath_deflate::crc32 is the exact algorithm PNG specifies (§5.5): reflected,
...
pub fn decode_png(data: &[u8]) -> Result<PngImage, PngError> {
    ...
    while pos + 8 <= data.len() {
        ...
        let computed = ath_deflate::crc32(&data[pos + 4..data_end]);
    ...
    // ── Inflate the zlib-wrapped IDAT stream (REUSE ath_deflate) ─────────────
    let raw = ath_deflate::zlib_decompress(&idat).map_err(|_| PngError::InflateFailed)?;
...
// zlib stream is produced by ath_deflate::zlib_compress (NOT reimplemented), and
// each chunk's CRC-32 uses the decoder's ath_deflate::crc32 (no second impl).
...
fn encode_chunk(ty: &[u8; 4], data: &[u8], out: &mut Vec<u8>) {
    ...
    // CRC-32 over the type + data (REUSE the decoder's ath_deflate::crc32).
    let crc = {
        let mut crc_input = Vec::with_capacity(4 + data.len());
        crc_input.extend_from_slice(ty);
        crc_input.extend_from_slice(data);
        ath_deflate::crc32(&crc_input)
    };
...
///   filter types; the `IDAT` body is zlib via [`ath_deflate::zlib_compress`].
///
/// ## Errors
...
pub fn encode_png(img: &PngImage) -> Result<Vec<u8>, PngEncodeError> {
    ...
    // ── zlib-compress the filtered stream (REUSE ath_deflate) ────────────────
    let idat = ath_deflate::zlib_compress(&filtered);
...
// ════════════════════════════════════════════════════════════════════════════
// Host KAT suite — `cargo test -p ath_png`. FAIL-able by construction.
//
...
// (ath_deflate-independent on the encode side) so each pixel assert is concrete.
// ════════════════════════════════════════════════════════════════════════════
#[cfg(test)]
mod tests {
    use super::*;
    ...
    /// `ath_deflate::zlib_decompress` recovers `payload` exactly.
    fn zlib_stored(payload: &[u8]) -> Vec<u8> {
        ...
        out.extend_from_slice(&ath_deflate::adler32(payload).to_be_bytes());
        out
    }
    ...
    fn chunk(ty: &[u8; 4], data: &[u8]) -> Vec<u8> {
        ...
        crc_input.extend_from_slice(ty);
        crc_input.extend_from_slice(data);
        out.extend_from_slice(&ath_deflate::crc32(&crc_input).to_be_bytes());
        out
    }
