...
//!    trusted update key (held by raeen-security; we VERIFY, never mint trust).
...
use rae_diff::{apply_delta, DeltaOp, DiffError};

use crate::{PartitionSlot, UpdateError, Version};

// ===========================================================================
// 1. Trust input — the update-signing public key (owned by raeen-security).
// ===========================================================================

/// The Ed25519 public key the running system trusts for update payloads.
///
/// We do not generate or store this key here — raeen-security owns the
...
impl SignedDeltaPayload {
    ...
    pub fn verify(&self, key: &UpdateTrustKey) -> Result<(), VerifyError> {
        ...
        let actual = rae_hash::sha256(&self.payload);
        ...
        if !rae_crypto::ed25519::verify(&key.ed25519_public, &self.payload, &self.signature) {
    ...
    pub fn reconstruct(
        ...
        let result_hash = rae_hash::sha256(&new_image);
...
// ===========================================================================
//
// rae_diff defines DeltaOp but no wire form (it is a pure algorithm crate). The
...
// ===========================================================================
// 5. Host KATs — the FAIL-able proof (`cargo test -p raeupdate`).
// ===========================================================================
#[cfg(test)]
mod tests {
    use super::*;
    use alloc::vec;
    use rae_diff::byte_delta;
    ...
    fn make_payload(
        ...
        let payload_sha256 = rae_hash::sha256(&payload);
        let result_sha256 = rae_hash::sha256(new);
        let signature = rae_crypto::ed25519::sign(seed, &payload);
        let pubkey = rae_crypto::ed25519::derive_public_key(seed);
        (
            SignedDeltaPayload {
    ...
    fn tampered_payload_fails_signature() {
        ...
        payload.payload_sha256 = rae_hash::sha256(&payload.payload);
    ...
    fn wrong_key_fails_signature() {
        ...
        let wrong_key = UpdateTrustKey::new(rae_crypto::ed25519::derive_public_key(&other_seed));
        assert_eq!(
    ...
    fn out_of_range_copy_is_rejected() {
        ...
        let payload_sha256 = rae_hash::sha256(&bytes);
        let signature = rae_crypto::ed25519::sign(&SEED, &bytes);
        let key = UpdateTrustKey::new(rae_crypto::ed25519::derive_public_key(&SEED));
