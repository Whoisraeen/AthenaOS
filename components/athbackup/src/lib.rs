...
impl EncryptionConfig {
    ...
    pub fn encrypt_block(&self, data: &[u8], nonce: [u8; 12]) -> Vec<u8> {
        ...
        let sealed = ath_crypto::chacha20poly1305::seal(&self.backup_key, &nonce, &[], data);
    ...
    pub fn decrypt_block(&self, data: &[u8]) -> Option<Vec<u8>> {
        ...
        ath_crypto::chacha20poly1305::open(&self.backup_key, &nonce, &[], &data[12..])
    }
}

// Real cryptographic primitives via the shared `ath_crypto` crate. These
...
fn argon2id_hash(password: &[u8], salt: &[u8], iterations: u32) -> [u8; 32] {
    let mut out = [0u8; 32];
    ath_crypto::argon2id_derive(password, salt, iterations.clamp(1, 10), 8192, 1, &mut out);
    out
}
...
mod crypto_tests {
    ...
    fn sha256_hash_is_real() {
        assert_eq!(sha256_hash(b"abc"), ath_crypto::sha256::sha256(b"abc"));
    ...
    fn argon2id_hash_is_real() {
        // Matches ath_crypto Argon2id with the same cost parameters, and is
        // salt-sensitive (the old LCG barely depended on the salt).
        let mut expect = [0u8; 32];
        ath_crypto::argon2id_derive(b"pw", b"saltsalt", 3, 8192, 1, &mut expect);
