...
// Real cryptographic hashes via the shared `ath_crypto` crate. These replaced
...
impl PackageManager {
    pub fn new() -> Self {
        Self {
            ...
            cache: PackageCache::new(String::from("/var/cache/athpkg")),
            groups: Vec::new(),
...
mod crypto_tests {
    ...
    fn checksums_are_real_hashes() {
        ...
        assert_eq!(
            ...
            ath_crypto::sha256::sha256(b"abc").to_vec()
        );
        ...
        assert_eq!(
            ...
            ath_crypto::ed25519::sha512(b"abc").to_vec()
        );
        assert_eq!(compute_blake2b(b"abc"), ath_crypto::blake2b(64, b"abc"));
    }
    ...
    fn signed_package(
        ...
        let hash = ath_crypto::sha256::sha256(data).to_vec();
        let sig = ath_crypto::ed25519::sign(seed, &hash).to_vec();
    ...
    fn wrong_key_rejected() {
        ...
        keyring
            .import_key(GpgKey {
                ...
                trust: KeyTrust::Full,
                public_key: ath_crypto::ed25519::derive_public_key(&[9u8; 32]).to_vec(),
                created: 0,
                expires: 0,
            })
            .unwrap();
