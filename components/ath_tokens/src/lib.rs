...
//! (athui re-exports it as `athui::tokens`) share ONE token home. It is NOT an
//! ABI surface — `ath_abi` stays frozen; visual tokens churn here freely.
//!
//! Pure logic, host-KAT'd (`cargo test -p ath_tokens`): the crate is `no_std`
...
pub fn shipped_contrast_pairs() -> [ContrastPair; 24] {
    ...
    // SHIP-GATE a11y (athena-accessibility WCAG audit): the aurora BASE above is the
    ...
    let accent = RAEBLUE;
    // High-contrast palette (PARITY_MATRIX §J target; mirrors athui's HC values).
...
//
// Mirrors athui's `HighContrastPalette` (background black / foreground white /
// cyan focus ring) but promoted into `ath_tokens` so the whole stack reads ONE
// HC home (foundation §4 / plan §4: "promote it to a ath_tokens variant so the
...
/// High-contrast focus ring — cyan (mirrors athui `HighContrastPalette.focus_ring`).
...
///
/// Compositing order inside `draw_glass_surface` (athgfx): blurred backdrop →
...
/// (athgfx) as a sheen over the blurred-and-tinted backdrop. This is the
...
///
/// Lowered 0.6 → 0.38 (SHIP-GATE a11y, athena-accessibility WCAG audit): the
...
///
/// The dark-theme insight (SHIP-GATE a11y, athena-accessibility): on a DARK theme
...
/// ≈0.435; 0.40 keeps a deliberate margin. (≤ the "~L48" athena-accessibility
...
/// `draw_glass_surface` (athgfx) composites: backdrop → slate **tint** → **frost**
...
/// logic so the compositor, athgfx and the `tier_luminance_is_monotonic` KAT share
...
/// apply there. Pure logic shared by the compositor, athgfx and the
...
// the palette tokens; athena-gfx implements the `AuroraWallpaper: LiveWallpaper`
