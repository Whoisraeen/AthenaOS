...
//! package primitives live in `raepackage`.
#![no_std]
...
/// A section inside a `.raepkg` archive.
#[derive(Clone, Debug)]
...
/// Section table — the TOC at the start of a `.raepkg` file.
#[derive(Clone, Debug)]
...
// ── SHA-256 digest ──────────────────────────────────────────────────────
// Real SHA-256 (FIPS 180-4) via the shared `rae_crypto` crate. This replaced a
...
/// Errors specific to decoding the `.raepkg` byte container. These map onto
...
impl RaePackage {
    /// Serialize this package into the flat `.raepkg` container. The inverse of
    ...
    /// Parse a `.raepkg` byte container into an in-memory package. **Total over
...
/// The full security gate: parse untrusted `.raepkg` bytes AND verify them
...
mod crypto_tests {
    ...
    fn sha256_digest_is_real() {
        assert_eq!(sha256_digest(b"abc"), rae_crypto::sha256::sha256(b"abc"));
        // SHA-256("") = e3b0c442...
    ...
    fn wrong_and_revoked_keys_rejected() {
        ...
        kr.import(SigningKey {
            ...
            public_key: rae_crypto::ed25519::derive_public_key(&[8u8; 32]).to_vec(),
...
mod codec_tests {
    ...
    fn untrusted_key_rejected() {
        ...
        kr.import(SigningKey {
            ...
            owner: String::from("attacker"),
            public_key: rae_crypto::ed25519::derive_public_key(&[2u8; 32]).to_vec(),
            trust_level: KeyTrust::Full,
            created_at: 0,
            expires_at: 0,
        });
    ...
    fn unsigned_package_rejected_when_signature_required() {
        ...
        m.sha256_hash = rae_crypto::sha256::sha256(b"code");
        let mut pkg = RaePackage::new(m);
    ...
    fn unsigned_tampered_sideload_still_rejected() {
        ...
        let mut m = PackageManifest::new(AppId(1), String::from("x"), SemVer::new(1, 0, 0));
        m.sha256_hash = rae_crypto::sha256::sha256(b"good code");
        let mut pkg = RaePackage::new(m);
...
mod transaction_tests {
    ...
    fn entry(id: u64, version: SemVer, deps: Vec<Dependency>) -> RepoEntry {
        ...
        m.sha256_hash = rae_crypto::sha256::sha256(&[id as u8]);
        RepoEntry {
            manifest: m,
