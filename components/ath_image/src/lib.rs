...
//! from-scratch still-image decoders ([`ath_png`], [`ath_jpeg`], [`ath_bmp`],
//! [`ath_gif`], [`ath_webp`]), each producing the **identical** ARGB8888
...
//! bytes** (via [`ath_formats::detect`], NOT the file extension — criterion #6
//! security: never trust a name an attacker controls), dispatches to the matching
//! decoder, and re-homes its output into one unified [`Image`]. Photos, `athplay`,
//! and `athgfx` call `ath_image::decode(bytes)` and never branch on format.
//!
...
//! |---|---|---|---|
//! | PNG  | [`FileKind::Png`]  | [`ath_png::decode_png`]   | full color-type matrix |
//! | JPEG | [`FileKind::Jpeg`] | [`ath_jpeg::decode_jpeg`] | baseline DCT |
//! | BMP  | [`FileKind::Bmp`]  | [`ath_bmp::decode_bmp`]   | + ICO via ath_bmp directly |
//! | GIF  | [`FileKind::Gif`]  | [`ath_gif::decode_gif`]   | **first frame only** (see below) |
//! | WebP | [`FileKind::Webp`] | [`ath_webp::decode_webp`] | VP8L lossless |
//!
...
//! [`ath_gif::decode_gif`] directly; that multi-frame surface intentionally stays
//! on `ath_gif` rather than being flattened into this still-image type.
//!
//! ## What encodes ([`encode`])
//! Only the two formats that have in-tree encoders: [`ImageFormat::Png`] (via
//! [`ath_png::encode_png`]) and [`ImageFormat::Jpeg`] (via [`ath_jpeg::encode_jpeg`]
...
//! the seeded fuzz loop in the host KAT suite (`cargo test -p ath_image`).
...
pub enum ImageFormat {
    /// PNG, lossless. Encoded via [`ath_png::encode_png`].
    Png,
    /// JPEG, baseline lossy, with a 1..=100 quality. Encoded via
    /// [`ath_jpeg::encode_jpeg`].
    Jpeg { quality: u8 },
}
...
pub fn format_of(bytes: &[u8]) -> Option<&'static str> {
    match ath_formats::detect(bytes) {
...
/// format-specific path). Content only — see [`ath_formats::detect_with_hint`].
pub fn detect(bytes: &[u8]) -> FileKind {
    ath_formats::detect(bytes)
}

/// Decode any supported still image from its bytes into a unified [`Image`].
///
/// The format is chosen by **magic bytes** ([`ath_formats::detect`]), never the
...
pub fn decode(bytes: &[u8]) -> Result<Image, ImageError> {
    dispatch(ath_formats::detect(bytes), bytes)
}
...
/// [`ath_formats::detect_with_hint`]). **Content always wins** — the hint never
...
pub fn decode_with_hint(bytes: &[u8], filename_or_ext: Option<&str>) -> Result<Image, ImageError> {
    dispatch(ath_formats::detect_with_hint(bytes, filename_or_ext), bytes)
}
...
fn dispatch(kind: FileKind, bytes: &[u8]) -> Result<Image, ImageError> {
    match kind {
        FileKind::Png => ath_png::decode_png(bytes)
            .map(|img| Image {
                width: img.width,
                height: img.height,
                pixels: img.pixels,
        ...
        FileKind::Jpeg => ath_jpeg::decode_jpeg(bytes)
            .map(|img| Image {
                width: img.width,
                height: img.height,
                pixels: img.pixels,
        ...
        FileKind::Bmp => ath_bmp::decode_bmp(bytes)
            .map(|img| Image {
                width: img.width,
                height: img.height,
                pixels: img.pixels,
        ...
        FileKind::Gif => match ath_gif::decode_gif(bytes) {
            Ok(g) => {
                // First frame only — the still-image view of a GIF (thumbnail /
                // Quick Look). Full animation stays on ath_gif::decode_gif.
        ...
        FileKind::Webp => ath_webp::decode_webp(bytes)
            .map(|img| Image {
                width: img.width,
                height: img.height,
                pixels: img.pixels,
...
pub fn encode(img: &Image, format: ImageFormat) -> Result<Vec<u8>, ImageError> {
    match format {
        ImageFormat::Png => {
            let png = ath_png::PngImage {
                width: img.width,
                height: img.height,
                pixels: img.pixels.clone(),
            };
            ath_png::encode_png(&png).map_err(|e| ImageError::Encode {
                format: "png",
                reason: png_encode_reason(e),
            })
        }
        ImageFormat::Jpeg { quality } => {
            let jpeg = ath_jpeg::JpegImage {
                width: img.width,
                height: img.height,
                pixels: img.pixels.clone(),
            };
            ath_jpeg::encode_jpeg(&jpeg, quality).map_err(|e| ImageError::Encode {
                format: "jpeg",
...
fn png_reason(e: ath_png::PngError) -> &'static str {
    use ath_png::PngError::*;
    match e {
...
fn png_encode_reason(e: ath_png::PngEncodeError) -> &'static str {
    use ath_png::PngEncodeError::*;
    match e {
        DimensionsOutOfRange => "dimensions out of range",
...
fn jpeg_reason(e: ath_jpeg::JpegError) -> &'static str {
    use ath_jpeg::JpegError::*;
    match e {
        BadSignature => "bad signature",
        Truncated => "truncated",
...
fn jpeg_encode_reason(e: ath_jpeg::JpegEncodeError) -> &'static str {
    use ath_jpeg::JpegEncodeError::*;
    match e {
        DimensionsOutOfRange => "dimensions out of range",
        PixelCountMismatch => "pixel count mismatch",
...
fn bmp_reason(e: ath_bmp::BmpError) -> &'static str {
    use ath_bmp::BmpError::*;
    match e {
        Truncated => "truncated",
        BadSignature => "bad signature",
        DimensionsOutOfRange => "dimensions out of range",
...
fn gif_reason(e: ath_gif::GifError) -> &'static str {
    use ath_gif::GifError::*;
    match e {
        BadSignature => "bad signature",
        Truncated => "truncated",
        DimensionsOutOfRange => "dimensions out of range",
...
fn webp_reason(e: ath_webp::WebpError) -> &'static str {
    use ath_webp::WebpError::*;
    match e {
        Truncated => "truncated",
        NotWebp => "not webp",
        NoImageData => "no image data",
...
// ════════════════════════════════════════════════════════════════════════════
// Host KAT suite — `cargo test -p ath_image`. FAIL-able by construction.
//
...
mod tests {
    ...
    fn png_bytes() -> Vec<u8> {
        ...
        ath_png::encode_png(&png).expect("encode png fixture")
    }
    ...
    fn jpeg_bytes() -> Vec<u8> {
        ...
        ath_jpeg::encode_jpeg(&jpeg, 90).expect("encode jpeg fixture")
    }
    ...
    /// the EXACT bit layout ath_webp's own KAT corpus uses (a 1x1 image, no
    /// transforms, no color cache, no meta-Huffman, five single-symbol "simple"
    /// Huffman code groups). ath_webp has no encoder, so this hand-built fixture is
    ...
    fn decode_bmp_exact() {
        ...
        assert_eq!(ath_formats::detect(&bytes), FileKind::Bmp);
    ...
    fn decode_webp_vp8l() {
        ...
        assert_eq!(ath_formats::detect(&bytes), FileKind::Webp);
    ...
    fn decode_gif_first_frame() {
        let bytes = gif_bytes();
        assert_eq!(ath_formats::detect(&bytes), FileKind::Gif);
    ...
    fn encode_png_roundtrip() {
        ...
        assert_eq!(ath_formats::detect(&bytes), FileKind::Png);
    ...
    fn encode_jpeg_roundtrip() {
        ...
        assert_eq!(ath_formats::detect(&bytes), FileKind::Jpeg);
