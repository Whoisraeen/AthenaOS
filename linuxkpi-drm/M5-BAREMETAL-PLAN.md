...
unresolved against ath_linuxkpi, **on-path stubs = 0** (every symbol a real
...
### 1. Cross-build the object set + ath_linuxkpi for AthenaOS
- Compile all `BRINGUP*` .c with `-ffreestanding -mno-red-zone` for x86_64.
- Build ath_linuxkpi for `x86_64-unknown-none` (already the daemon/kernel target;
...
Today ath_linuxkpi's MMIO/PCI accessors split:
...
- a `live` cargo feature on ath_linuxkpi that swaps the drm_bringup device stubs
...
`ring_doorbell`), because those map 1:1 onto ath_linuxkpi's C-ABI accessors.
...
- `jiffies` must tick (ath_linuxkpi `lkpi_tick_jiffies`) so amdgpu's timeout loops
...
The Rust reimpl (`ath_amdgpu/src/bringup.rs`, ~6.5k lines over `GpuOps`) matches
