...
`ath_linuxkpi` export with correct semantics, amdgpu **links**.
...
| already provided by `ath_linuxkpi` | **182** | `amdgpu-symbols-covered.txt` |
...
| **C** | core LinuxKPI primitives (dma_fence / workqueue / xarray / idr / wait / timer / completion / kref) | 50 | **real work** — these get CALLED; extend the existing `ath_linuxkpi` facades |
...
The shims under `include/linux/*.h` *declare* these; `components/ath_linuxkpi` *defines*
...
# vs ath_linuxkpi exports:
grep -rhoE 'pub (unsafe )?extern "C" fn [a-z_0-9]+' components/ath_linuxkpi/src/ | sed -E 's/.*fn //' | sort -u
```
