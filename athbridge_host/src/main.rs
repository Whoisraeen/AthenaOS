...
//! on AthenaOS, calling Win32 APIs that land in `athbridge::winapi_shims`.
//!
...
//!      Verdict: `[athbridge] smoketest: hello-world exe -> stdout "Hello from
//!      Windows" + exit 0 PASS`.
//!   2. exit-code image — ExitProcess(42). The shim prints
//!      `[athbridge] guest ExitProcess(42) -> exit 42 PASS` and terminates the
...
//!      Verdict: `[athbridge] smoketest: gs-base exe -> gs:[0x30]==TEB survives
...
//!      lines in order — FAILs if the ctor line is missing): `[athbridge]
//!      smoketest: real MSVC /MT C++ exe -> static-ctor ran + main + exit 0 PASS`.
//!
//! Serial proof lines (grep targets):
//!   [athbridge] smoketest: hello-world exe -> stdout "Hello from Windows" + exit 0 PASS
//!   [athbridge] smoketest: gs-base exe -> gs:[0x30]==TEB survives reschedule + exit 0 PASS
//!   [athbridge] smoketest: real MSVC /MT exe -> all imports resolved to shims PASS
//!   [athbridge] smoketest: real MSVC /MT printf exe -> all imports resolved to shims PASS
//!   [athbridge] smoketest: real MSVC /MT C++ exe -> static-ctor ran + main + exit 0 PASS
...
// Pull in athkit for its #[global_allocator]; everything else here goes
// through athbridge's own syscall bindings.
#[allow(unused_imports)]
use athkit;

use athbridge::exec::load_pe_executable;
use athbridge::syscalls::{sys_debug_print, sys_exit, sys_open, sys_read};
use athbridge::testpe;
use athbridge::winapi_shims::{
...
unsafe fn run_returning_image(image: &[u8], what: &str) -> u64 {
    let loaded = match load_pe_executable(image) {
        Ok(l) => l,
        Err(e) => fail(
            &format!("[athbridge] host: FATAL: load {what}: {e:?}\n"),
            0xE3,
        ),
    };
    if loaded.resolved_imports == 0 {
        fail(
            &format!("[athbridge] host: FATAL: {what} resolved no imports\n"),
            0xE4,
        );
    }
...
unsafe fn run_result_image(image: &[u8], what: &str, expected_imports: usize) -> u64 {
    ...
    if loaded.resolved_imports != expected_imports || loaded.stubbed_imports != 0 {
        fail(
            &format!(
                "[athbridge] host: FATAL: {what} imports resolved={} stubbed={} (want {} / 0)\n",
...
pub extern "C" fn _start() -> ! {
    log("[athbridge] host: durable smoketest harness starting\n");
    ...
    let session = match FullCompatSession::new(
        ...
        Ok(s) => Box::new(s),
        Err(_) => fail(
            "[athbridge] host: FATAL: session construction failed\n",
            0xE1,
        ),
    };
    if install_host_context(session).is_some() {
        fail("[athbridge] host: FATAL: duplicate host context\n", 0xE2);
    }
    if !host_context_installed() {
        fail(
            "[athbridge] host: FATAL: host context not installed\n",
            0xE2,
        );
    }
    ...
    if captured == expected {
        log("[athbridge] smoketest: hello-world exe -> stdout \"Hello from Windows\" + exit 0 PASS\n");
    } else {
        // Fail loud: report what we actually captured so a marshaling bug is
        // diagnosable from the serial log alone.
        log(&format!(
            "[athbridge] smoketest: hello-world exe -> stdout MISMATCH (got {} bytes, want {}) FAIL\n",
            captured.len(),
            expected.len(),
        ));
    ...
    if api_code == testpe::API_PASS && console_ok {
        log("[athbridge] smoketest: api-exercise exe -> GetModuleHandle/GetProcAddress/Heap/TLS/Console all OK + exit 0 PASS\n");
    } else if api_code != testpe::API_PASS {
        // Name the exact step that failed (decodable from serial alone).
        log(&format!(
            "[athbridge] smoketest: api-exercise exe -> step {} FAILED (1=GetModuleHandle 2=GetProcAddress 3=HeapAlloc 4=HeapReAlloc 5=TLS 6=WriteConsole) FAIL\n",
            api_code
        ));
    } else {
        log(&format!(
            "[athbridge] smoketest: api-exercise exe -> steps OK but WriteConsoleW capture MISMATCH (got {} bytes, want {}) FAIL\n",
            api_console.len(),
    ...
    // be eyeballed, so we read the accumulated paint count).
    {
        ...
        let loaded = match load_pe_executable(gui_exe) {
            Ok(l) => l,
            Err(e) => fail(
                &format!("[athbridge] host: FATAL: load gui-window exe: {e:?}\n"),
                0xE5,
            ),
        };
        if loaded.stubbed_imports != 0 {
            log(&format!(
                "[athbridge] smoketest: gui-window exe -> {} unresolved import(s) (window/gdi shim gap) FAIL\n",
        ...
        if painted > 0 {
            log(&format!(
                "[athbridge] smoketest: gui-window exe -> RegisterClass+CreateWindow+UpdateWindow->WM_PAINT painted {painted} px (WndProc dispatch + reentrant gdi OK) PASS\n"
            ));
        } else {
            log("[athbridge] smoketest: gui-window exe -> WM_PAINT rendered 0 px (pump/dispatch/paint gap) FAIL\n");
        }
    }
    ...
    // typed bytes round-tripped: types + saves, proven end-to-end.
    {
        ...
        let loaded = match load_pe_executable(save_exe) {
            Ok(l) => l,
            Err(e) => fail(
                &format!("[athbridge] host: FATAL: load gui-save exe: {e:?}\n"),
                0xE6,
            ),
        };
        if loaded.stubbed_imports != 0 {
            log(&format!(
                "[athbridge] smoketest: gui-save exe -> {} unresolved import(s) FAIL\n",
                loaded.stubbed_imports
            ));
        }
        ...
        let mut buf = [0u8; 16];
        let vfs = athbridge::app_bucket_vfs_path("smoketest.exe", "C:\\out.txt");
        let fd = unsafe { sys_open(vfs.as_bytes(), 0) };
        // SYS_OPEN error codes are u64::MAX..=MAX-3 (MAX-1 = not found), not just MAX.
        if fd >= u64::MAX - 3 {
            log("[athbridge] smoketest: gui-save exe -> C:\\out.txt not created (save path gap) FAIL\n");
        } else {
            let n = unsafe { sys_read(fd, &mut buf) } as usize;
            let got = &buf[..n.min(buf.len())];
            if got == b"HI" {
                log("[athbridge] smoketest: gui-save exe -> typed 'HI' + CreateFileW/WriteFile -> C:\\out.txt readback 'HI' PASS\n");
            } else {
                log(&format!(
                    "[athbridge] smoketest: gui-save exe -> readback {} bytes, want 'HI' FAIL\n",
                    n
                ));
            }
        }
    ...
    match unsafe { gui_edit_control_selftest() } {
        ...
        Some((false, _)) => log(
            "[athbridge] smoketest: edit-control -> GetWindowTextW != 'HI' (builtin EDIT/dispatch gap) FAIL\n",
        ),
        Some((true, false)) => log(
            "[athbridge] smoketest: edit-control -> typed 'HI' but WM_PAINT rendered 0 px (EDIT paint gap) FAIL\n",
        ),
        None => log(
            "[athbridge] smoketest: edit-control -> CreateWindowEx(\"EDIT\") returned NULL (system-class create gap) FAIL\n",
        ),
    }
    ...
    // integrate end-to-end. FAIL-able: any piece breaking flips the verdict.
    {
        ...
        let loaded = match load_pe_executable(notepad) {
            Ok(l) => l,
            Err(e) => fail(
                &format!("[athbridge] host: FATAL: load gui-notepad exe: {e:?}\n"),
                0xE7,
            ),
        };
        if loaded.stubbed_imports != 0 {
            log(&format!(
                "[athbridge] smoketest: gui-notepad exe -> {} unresolved import(s) FAIL\n",
                loaded.stubbed_imports
            ));
        }
        reset_message_state();
        ...
        let mut buf = [0u8; 16];
        let vfs = athbridge::app_bucket_vfs_path("smoketest.exe", "C:\\note.txt");
        let fd = unsafe { sys_open(vfs.as_bytes(), 0) };
        if fd >= u64::MAX - 3 {
            log("[athbridge] smoketest: gui-notepad exe -> C:\\note.txt not created (menu/save flow gap) FAIL\n");
        } else {
            let n = unsafe { sys_read(fd, &mut buf) } as usize;
            if &buf[..n.min(buf.len())] == b"HI" {
                log("[athbridge] smoketest: gui-notepad exe -> window+EDIT+menu File->Save (GetSaveFileNameW) -> C:\\note.txt 'HI' PASS\n");
            } else {
                log(&format!(
                    "[athbridge] smoketest: gui-notepad exe -> readback {} bytes, want 'HI' FAIL\n",
                    n
                ));
            }
        }
    ...
    // loader's W^X flip + set_gs_base flags so a kernel-side miss is named.
    {
        ...
        let loaded = match load_pe_executable(&gs_exe) {
            Ok(l) => l,
            Err(e) => fail(
                &format!("[athbridge] host: FATAL: load gs-base exe: {e:?}\n"),
                0xE3,
            ),
        };
        if loaded.resolved_imports != testpe::GSBASE_IMPORTS.len() || loaded.stubbed_imports != 0 {
            fail(
                &format!(
                    "[athbridge] host: FATAL: gs-base exe imports resolved={} stubbed={} (want {} / 0)\n",
        ...
        if !loaded.gs_base_set {
            ...
            log("[athbridge] smoketest: gs-base exe -> SYS_SET_GS_BASE refused (kernel side missing?) FAIL\n");
        }
        ...
        if verdict == testpe::GSBASE_PASS && loaded.gs_base_set {
            log(&format!(
                "[athbridge] smoketest: gs-base exe -> gs:[0x30]==TEB survives reschedule + exit 0 PASS (text={wx})\n"
            ));
        } else {
            // Name the exact failing check (decodable from serial alone).
            log(&format!(
                "[athbridge] smoketest: gs-base exe -> check {} FAILED (1=TEB-zero 2=self-ptr 3=not-restored 4=sentinel) gs_set={} FAIL\n",
    ...
    // "[athbridge] guest ExitProcess(42) -> exit 42 PASS". We DON'T let it be
    ...
    // proven by Phase 3.)
    log("[athbridge] host: mapping exit-code exe (ExitProcess IAT check)\n");
    let exit_exe = testpe::build_exit_process_exe();
    match load_pe_executable(&exit_exe) {
        Ok(l) if l.resolved_imports > 0 => {
            log("[athbridge] smoketest: exit-code exe -> ExitProcess resolved to shim PASS\n");
        }
        Ok(_) => fail(
            "[athbridge] host: FATAL: ExitProcess did not resolve to a shim\n",
            0xE4,
        ),
        Err(e) => fail(
            &format!("[athbridge] host: FATAL: load exit-code exe: {e:?}\n"),
            0xE3,
        ),
    }
    ...
    log("[athbridge] host: loading REAL MSVC /MT WriteFile exe (import-resolve proof)\n");
    let real_exe = testpe::REAL_MSVC_MT_EXE;
    match load_pe_executable(real_exe) {
        Ok(l) => {
            log(&format!(
                "[athbridge] host: WriteFile exe mapped: resolved={} stubbed={} pdata_funcs={} wx={} gs={}\n",
                l.resolved_imports, l.stubbed_imports, l.runtime_functions.len(), l.wx_flip_ok, l.gs_base_set,
            ));
            if l.stubbed_imports != 0 {
                log(&format!(
                    "[athbridge] smoketest: real MSVC /MT exe -> {} imports UNRESOLVED (would fail-loud before main) FAIL\n",
                    l.stubbed_imports
                ));
            } else {
                log("[athbridge] smoketest: real MSVC /MT exe -> all imports resolved to shims PASS\n");
            }
        }
        Err(e) => fail(
            &format!("[athbridge] host: FATAL: load real MSVC exe: {e:?}\n"),
            0xE3,
        ),
    }
    ...
    log("[athbridge] host: loading REAL MSVC /MT printf exe (import-resolve proof)\n");
    let printf_exe = testpe::REAL_MSVC_MT_PRINTF_EXE;
    match load_pe_executable(printf_exe) {
        Ok(l) => {
            log(&format!(
                "[athbridge] host: printf exe mapped: resolved={} stubbed={} pdata_funcs={} wx={} gs={}\n",
                l.resolved_imports, l.stubbed_imports, l.runtime_functions.len(), l.wx_flip_ok, l.gs_base_set,
            ));
            if l.stubbed_imports != 0 {
                log(&format!(
                    "[athbridge] smoketest: real MSVC /MT printf exe -> {} imports UNRESOLVED (would fail-loud before main) FAIL\n",
                    l.stubbed_imports
                ));
            } else {
                log("[athbridge] smoketest: real MSVC /MT printf exe -> all imports resolved to shims PASS\n");
            }
        }
        Err(e) => fail(
            &format!("[athbridge] host: FATAL: load real MSVC printf exe: {e:?}\n"),
            0xE3,
        ),
    }
    ...
    // emits:
    //   [athbridge] smoketest: real MSVC /MT C++ exe -> static-ctor ran + main
    //     + exit 0 PASS
    // then terminates the host with exit 0. This is the process terminator.
    log("[athbridge] host: loading REAL MSVC /MT C++ exe (the C++-runtime milestone)\n");
    let cpp_exe = testpe::REAL_MSVC_MT_CPP_EXE;
    let loaded = match load_pe_executable(cpp_exe) {
        Ok(l) => l,
        Err(e) => fail(
            &format!("[athbridge] host: FATAL: load real MSVC C++ exe: {e:?}\n"),
            0xE3,
        ),
    };
    log(&format!(
        "[athbridge] host: C++ exe mapped: resolved={} stubbed={} pdata_funcs={} wx={} gs={}\n",
        loaded.resolved_imports,
        loaded.stubbed_imports,
    ...
    if loaded.stubbed_imports != 0 {
        ...
        fail(
            &format!(
                "[athbridge] smoketest: real MSVC /MT C++ exe -> {} imports UNRESOLVED (would fail-loud before static-init) FAIL\n",
                loaded.stubbed_imports
            ),
            0xE5,
        );
    ...
    log("[athbridge] host: jumping to REAL MSVC CRT entry point (C++ static-init)\n");
